(() => {
  ...
  /* ------------------------------------------------------------------ *
   * Strata that think — generative engine
   * Fixed 720 × 1000 canvas. All drawing writes onto `ctx`
   * (swappable) using the active palette `C`. That lets the same engine
   * be reused for: the main view, the thumbnails of a series
   * and the exportable layers per territory.
   * ------------------------------------------------------------------ */
  ...
  /* --- Pixelated CP437 typography (Telenova Compis) ------------------
   * Used for the poems and for a new family of objects made of
   * glyphs: shading strata (░▒▓█), box frames/diagrams and loose
   * signs (☼☺♥♪…). The monospace fallback keeps the characters if the
   * font fails to load (e.g. when opening the file via file://). */
   ...
   * Each palette defines the same seven semantic "slots". The drawing
   * code never uses literal colors: it asks for C.blue, C.red, etc., so
   * that switching palettes reorders the whole color world untouched.
   * ------------------------------------------------------------------ */
  const PALETTES = {
    riso:    { name: "classic riso",   paper: "#ffffff", ink: "#0b0b0b", blue: "#1038ff", red: "#ff1616", green: "#04be36", yellow: "#ffd400", white: "#ffffff" },
    tierra:  { name: "earth",          paper: "#f3ead6", ink: "#2a2017", blue: "#3a6b5f", red: "#b5471f", green: "#6f7a2c", yellow: "#e0a52b", white: "#fbf6ea" },
    mineral: { name: "cold mineral",   paper: "#eef1f4", ink: "#1b2430", blue: "#2f4b7c", red: "#a05195", green: "#4c8577", yellow: "#cab43d", white: "#ffffff" },
    abismo:  { name: "dark abyss",     paper: "#0a0e1a", ink: "#e8eefc", blue: "#4f8cff", red: "#ff5d73", green: "#39d98a", yellow: "#ffd166", white: "#ffffff" },
    neon:    { name: "night neon",     paper: "#0d0d12", ink: "#f5f5ff", blue: "#00e5ff", red: "#ff2bd6", green: "#7bff3d", yellow: "#fff14d", white: "#ffffff" },
    mono:    { name: "monochrome",     paper: "#ffffff", ink: "#111111", blue: "#444444", red: "#222222", green: "#666666", yellow: "#999999", white: "#ffffff" },
    aleatoria: { name: "random (scattered color)", random: true, paper: "#ffffff" }
  };

  /* --- Palette selection ---------------------------------------------
   * The selector includes an "auto" (surprise) mode: for each plate a
   * concrete palette is chosen at random, weighted and derived from the
   * seed (reproducible). Colorful riso is the majority; monochrome, rare;
   * the night-background palettes and the per-element "random" are included. */
  ...
  /* Per-element "random" palette: each shape takes a random color,
   * with plenty of color and little black, over some white void. */
  const RANDOM_ACCENTS = ["#1038ff", "#ff1616", "#04be36", "#ffd400"];

  function weightedRandomColor() {
    const r = rng();
    if (r < 0.42) return "#ffffff";   // void
    if (r < 0.52) return "#000000";   // occasional black
  ...
  /* The poetic lexicon lives in lexicon.js (loaded before this
   * file in index.html): edit the phrases there, by zone. */


  /* --- Engine state (swappable between drawing destinations) -------- */
  let ctx = mainCtx;          // active context being drawn on
  let C = PALETTES.riso;      // active palette
  let rng = Math.random;      // seeded pseudo-random generator
  let activeMode = "total";   // territory mode of the current render
  let textVisible = true;     // draws phrases
  let symbolsEnabled = true;  // draws graphic species (primitives obey)
  let occupied = [];
  let symbolBoxes = [];       // footprints (boxes) of species already drawn
  let shapeBox = null;        // box under construction for the current species
  let animTime = 0;           // temporal phase: 0 = rest; animation moves it
  let driftX = 0;             // animation sway: shifts ONLY the paint,
  let driftY = 0;             // never the footprint or placement decisions
  ...
  // Legible ink for text: in the random palette it avoids white; in
  // fixed palettes it uses their own ink (light on dark papers).
  function textInk() { return C.random ? "#0b0b0b" : C.ink; }

  /* --- Footprint of the current species ------------------------------
   * The primitives register the points they touch; `species()` opens and
   * closes the box. The footprint is recorded EVEN IF symbolsEnabled is
   * off, so that the exported text layer places the phrases in the
   * same spots as the full view. */
  ...
  /* Looks for a lightly occupied spot within a range. Overlapping ONE
   * existing footprint is allowed (the forest lives at the foot of the
   * mountain), but if the spot piles up more overlaps it keeps looking;
   * once attempts run out it keeps the least crowded candidate. */
  ...
  /* --- Drawing primitives -------------------------------------------
   * They obey `symbolsEnabled`. When it is off, the species functions
   * still run (and return their anchor point) but paint nothing: this
   * way the text layer can be placed next to the shapes without the
   * shapes appearing. */
  ...
  // Note: these primitives do NOT bail out when symbols are hidden:
  // px() no longer paints in that case, but the footprint must still be
  // registered so the text layer makes the same decisions as the view.
  ...
  function looseDots(x, y, radius, count, colors = [C.ink, C.blue, C.red, C.green]) {
    // Always consume the randomness (even when not painting): the text
    // layer must walk the same random sequence as the full view.
  ...
  /* --- Graphic species (return an anchor point for the text) ------- */
  function drawLenticularCloud(x, y, s, color) {
    // horizontal wind drift (zero at rest; only moves the paint)
  ...
  function drawSpiral(x, y, s, color) {
    // The footprint is fixed at rest: wind spin only moves the paint.
    markPoint(x - 13 * s, y - 13 * s); markPoint(x + 13 * s, y + 13 * s);
    const box = shapeBox; shapeBox = null;
    const rot = animTime * 0.5;   // slow wind spin
    let last = null;
  ...
  // Triangular peaks with hillside shading
  ...
  // Volcano: truncated cone with crater rim and rising dotted smoke
  function mountainVolcano(x, y, width, color) {
    ...
    line([[cx + cw, y - h], [cx + cw * .4, y - h + 6]], C.red);
    // The smoke footprint is fixed at rest; when animated, the wisps rise.
  ...
  // Fir: chevrons; the branches now grow DOWNWARD (wider at the base)
  ...
  // Pine: rows of horizontal needles, wider below
  ...
  // Bare tree: recursive Y-branching
  ...
  // Willow: short trunk and branches that fall in arcs toward the ground
  ...
  function waveZig(x, y, length, amp, color) {
    ...
    const off = animTime * 8;   // horizontal displacement of the zigzag
  ...
  function drawFish(x, y, s, color = C.blue) {
    driftX = Math.sin(animTime * .9 + y * .05) * 5 * s;   // slow back-and-forth swim
    const v = rint(0, 3);
  ...
  // Long-tentacled jellyfish: small dome and threads that ripple
  // (the animTime phase sways them when the animation is on)
  function jellyStreamer(x, y, s, color) {
    // The footprint is fixed at rest: tentacles ripple only in the paint.
  ...
  function drawJelly(x, y, s, color = C.blue) {
    driftY = Math.sin(animTime * 1.1 + x * .04) * 3 * s;  // vertical pulse of the bell
    const v = rint(0, 4);
  ...
  // Algae: grow upward from the base (y); still like the landscape
  ...
  function drawBird(x, y, s, color = C.ink) {
    driftX = Math.sin(animTime * .4 + y * .03) * 8;   // gliding drift
    const v = rint(0, 2);
    if (v === 0) {
  ...
  // Whale: curved back, V-shaped tail and occasional dotted spout
  ...
  // Seahorse: S-shaped body, snout and spiral tail
  ...
  // Turtle: plated shell, dotted head and flippers
  ...
  // Deer: back, legs, neck and branched antlers
  ...
  // Shading stratum: grid of ░▒▓█ (dither-like texture / layer)
  function drawShadeBlock(x, y, s, color = C.ink, zone = "tectonic") {
    ...
    // The glyphs are always drawn (even if not painted), so the text
    // layer walks the same random sequence as the view.
    const rowsTxt = [];
  ...
  // Frame / box diagram with a glyph inside
  ...
  // Loose CP437 sign (1–3 thematic glyphs), another species of the drawing
  ...
  // Heron / water bird (standing on the water)
  ...
  // Palm: curved trunk and fronds that arch and fall
  ...
  /* --- Text as a species of the drawing ----------------------------- */
  ...
  function textAt(phrase, anchor, yMin, yMax) {
    ...
    /* Two-phase search: first near the anchor and on clean paper;
     * then writing over a drawing is accepted (without erasing it).
     * Text NEVER overlaps other text: with no room, the phrase is dropped. */
    ...
    if (anchor && chance(.7)) {
      // The connector is drawn as text (not as a symbol) so the text
      // layer keeps its tether even when the species are hidden.
  ...
  function placeTexts(anchors, groups) {
    /* Quota per zone: each active section ALWAYS contributes its phrases,
     * instead of drawing from a common bag (where the large underground
     * lexicon could leave the sea or the abyss mute). */
    ...
    pool.sort(() => rng() - .5);   // interleaves the zones when placing
    pool.forEach(phrase => {
  ...
  function drawGlobalNoise() {
    ...
    // loose CP437 signs amid the noise
  ...
  /* --- Render a full scene onto the active `ctx` -------------------- */
  function drawScene(seedSource, mode, flags) {
    const source = (seedSource || "").trim() || "memory-without-center";
    activeMode = mode;
    ...
    // Places `count` instances seeking lightly occupied spots and recording
    // each one's footprint. Field-shapes (waves, shores, dunes, fog)
    // do NOT go through here: they are thin lines that must cross everything.
  ...
  /* --- Reusable entry point: composes a scene in any context (main
   * view, thumbnail, layer). Saves and restores the engine's global
   * state. ----------------------------------------------------------- */
  ...
  function regenerate(newSeed = false) {
    if (newSeed) {
      ui.seed.value = [
        pick(["memory", "cloud", "root", "fault", "sediment", "archive", "system", "shore", "algorithm"]),
        pick(["without-center", "that-drifts", "underground", "that-breathes", "scattered", "minimal", "pelagic", "mineral"]),
        rint(10, 999)
      ].join("-");
    }
    drawMain();
  ...
  function exportPNG() {
    // Clean render to its own scene + animated layer if it's active.
    // With animation on, the text comes from the frozen layer, just
    // like on screen.
    ...
    download(scaledExportCanvas(off, false, paperFor(currentPalette, ui.seed.value)), `strata-${safeSeed(ui.seed.value)}`);
  }

  /* --- Series: same ecology, mutating seed ------------------------- */
  let seriesItems = [];

  function buildSeries() {
    const base = (ui.seed.value || "").trim() || "memory-without-center";
    const count = clamp(parseInt(ui.seriesCount.value, 10) || 6, 2, 12);
    ui.seriesStrip.innerHTML = "";
    seriesItems = [];
    const restTime = animTime; animTime = 0;   // thumbnails at rest
    ...
    for (let i = 0; i < count; i++) {
      ...
      thumb.title = `${seed}  ·  click to load`;
  ...
  function exportSeries() {
    ...
    seriesItems.forEach((item, i) => {
      const filename = `series-${safeSeed((ui.seed.value || "").trim() || "world")}-${String(i + 1).padStart(2, "0")}`;
      download(scaledExportCanvas(item.canvas, false, paperFor(currentPalette, item.seed)), filename);
    });
  }

  /* --- Exportable layers per territory (transparent PNGs) ---------- */
  function exportLayers() {
    const base = (ui.seed.value || "").trim() || "memory-without-center";
    const mode = ui.mode.value;
    const territories = mode === "total" ? ["aire", "mar", "tectonica"] : [mode];
    const restTime = animTime; animTime = 0;   // layers at rest
    ...
    territories.forEach(terr => {
      ...
      download(scaledExportCanvas(off, true), `layer-${terr}-${safeSeed(base)}`);
    });

    // Text layer: the species are computed (for the anchors) but not
    // painted; only the phrases remain on a transparent background.
    ...
    download(scaledExportCanvas(offText, true), `layer-text-${safeSeed(base)}`);
    animTime = restTime;
  }

  /* --- Slow animation: currents, clouds and signs ------------------
   * Lives in a transparent overlay layer, independent of the static
   * canvas. It is seeded with the active seed to stay stable. */
  let animOn = false;
  let rafId = null;
  let lastFrame = 0;
  let particles = [];
  ...
  let special = [];           // particles specific to the territory
  let specialKind = null;     // "bubble" | "snow" | "sand"
  let specialCols = [];
  let textLayer = null;       // frozen text: drawn ONCE and the
  let textBoxes = [];         // animation only stamps it as a fixed image

  /* The text is not recomputed during the animation: it is rendered once
   * (at rest, without symbols, transparent background) to its own layer.
   * Each animated frame stamps that layer as-is: the phrases cannot
   * move or disappear even as the rest of the world flows. */
  function buildTextLayer() {
    ...
    animTime = restTime;
    textBoxes = occupied.slice();   // footprints of the phrases, to avoid them
  }
  ...
  function buildAnim() {
    ...
    // In the per-element palette white is omitted (invisible) and black + accents are used.
    const cols = p.random
    ...
    // Particles specific to the territory: bubbles that rise (water),
    // snow that falls (ice) and sand in a lateral wind (desert).
    ...
    // Fireflies: twinkle in the forest/swamp band.
  ...
  function stepAnim(ts) {
    ...
    if (ts - lastFrame < 66) return;   // ~15 fps: slow, light animation
    lastFrame = ts;
    const t = ts * 0.001;
    animTime = t;

    // Redraws the REAL scene with the temporal phase: printed waves flow
    // (water) and clouds/spirals drift (wind). The text is NOT
    // recomputed: the frozen layer is stamped, immune to the animation.
    composeScene(mainCtx, {
    ...
    // Light overlay layer: particles (signs) and fireflies.
    ...
    particles.forEach(pt => {
      ...
      if (overText(x, y)) return;               // particles avoid the phrases
    ...
    special.forEach((sp, i) => {
      ...
      if (sp.big) {                                    // large bubble/flake: ring
    ...
    fireflies.forEach(f => {
      ...
      if (overText(x, y)) return;               // fireflies avoid the phrases
  ...
  function syncAnim() {
    // Called after each redraw: re-seeds the layer if the animation is on.
    if (animOn) buildAnim();
  ...
  function setAnim(on) {
    animOn = on;
    ui.toggleAnim.textContent = `A animation: ${on ? "on" : "paused"}`;
    if (on) {
      buildAnim();
      lastFrame = 0;
      ...
      animTime = 0;            // waves and wind return to rest
      octx.clearRect(0, 0, SCENE_W, SCENE_H);
      drawMain();
    }
  }
  ...
  ui.toggleText.addEventListener("click", () => {
    ...
    ui.toggleText.textContent = `T text: ${textVisible ? "on" : "hidden"}`;
    drawMain();
  });
  ...
  // Populate the selector: first "auto" (surprise), then the fixed catalog.
  [["auto", { name: "auto: surprise (mostly riso)" }], ...Object.entries(PALETTES)]
    .forEach(([key, value]) => {
  ...
  // The Compis font may not be ready on the first draw: once loaded,
  // it redraws so poems and glyphs use the pixel typography.
