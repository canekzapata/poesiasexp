...
var ESCENA = [
  ...
  { id: "riovert",  animas: ["anima-tinta", "anima-mascara", "anima-sombra"] },
...
function vestir(ch) {
  var r = rand();
  if (r < .55) return ch;
  if (r < .70) return "<sub>" + ch + "</sub>";
  if (r < .82) return "<sup>" + ch + "</sup>";
  if (r < .93) return "<font>" + ch + "</font>";
  return "<h6>" + ch + "</h6>";
}
...
function parcela() {
  ...
  if (tipo === "jardin") {
    ...
    var flor = elegir(FLORAL.length ? FLORAL : PLANTARIO);
  ...
  if (tipo === "caserio") {
    ...
    if (quizas(.5)) s += manojo(elegir(FLORAL.length ? FLORAL : PLANTARIO), G.flor, entero(2, 4), .6);   /* su jardin */
    return s;
  }
...
function valle(densidad) {
  ...
  return partes.join(expandir("#aire##aire#"));
}
...
function construir() {
  ...
  ESCENA.forEach(function (capa) {
    ...
    if (capa.id === "riovert") {
      /* el rio: nace en el horizonte de las montañas (nunca en el cielo)
         y corre hasta abajo; angosto, nunca mas de ~5 caracteres */
      div.style.fontSize = azar(1.6, 2.6).toFixed(2) + "vmax";
      div.style.left = entero(4, 80) + "%";
      div.style.width = azar(3, 5).toFixed(1) + "em";
      div.style.top = (100 - horizonteBase) + "%";
      var direccion = quizas(.5) ? "baja" : "sube";
      div.style.setProperty("--degcapa", (direccion === "baja" ? 180 : 0) + "deg");
      /* cada rio tiene su propia especie de planta en las orillas */
      G.plantaRio = [elegir(PLANTARIO)];
      var lineas = [];
      var nl = entero(7, 13);
      for (var r = 0; r < nl; r++) lineas.push(expandir("#riolinea#"));
      ...
      html += fila(lineas.join("<br>"), elegir(["left", "center", "right"]), false);
    ...
    if (capa.id === "nubes") {
      ...
      var nn = entero(1, 2);
      for (var g = 0; g < nn; g++) html += fila(repetir(tramoSegun("frentetramo", dn), entero(1, 4), ""), elegir(ALINEACIONES));
    }
    ...
    if (capa.id === "vida") {
      ...
      /* a veces el valle vive sobre un fondo de otro color —
         pero como banda contenida, nunca media pantalla */
      if (quizas(.4)) {
        anima = elegir(["anima-tinta", "anima-sombra"]);
        div.className = "capa capa-" + capa.id + " " + anima + " fondo-valle";
        div.style.fontSize = azar(2, 3.2).toFixed(2) + "vmax";
        nv = 1;
      }
