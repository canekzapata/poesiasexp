/*
  motor.js — PAISAJES
  poesiasexp / canekzapata

  escenario en cinco capas superpuestas (la mas atras primero):
    1. cielo      — fondo total: astro, nubes, satelites, aves, estrellas
    2. montanas   — block elements con direccion: subida ◢ cumbre ▲ bajada ◣
    3. nubes      — nubes y pajaros por delante de las montañas
    4. vida       — humanos en actividades y naturaleza en caracteres
    5. agua       — el rio al frente: ondas, peces, barcas, cosas marinas

  motor en la linea de sutilezas: prng estilo fxhash, esquemas de color
  (momento del dia x modo armonico) sobre variables css, esquemas de
  animacion por capa, mini-tracery sobre LEXICO y contenedores de texto
  (span font h4 h5 h6 sub sup) cada uno con su pulso.
*/

/* ---------------- prng ---------------- */

var alfabeto = "123456789abcdefghijkmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ";
var semilla = (typeof fxhash !== "undefined") ? fxhash :
  "oo" + Array(49).fill(0).map(function () { return alfabeto[(Math.random() * alfabeto.length) | 0]; }).join("");

function b58dec(str) {
  var r = 0;
  for (var i = 0; i < str.length; i++) r = r * alfabeto.length + alfabeto.indexOf(str[i]) | 0;
  return r;
}
function sfc32(a, b, c, d) {
  return function () {
    a |= 0; b |= 0; c |= 0; d |= 0;
    var t = (a + b | 0) + d | 0;
    d = d + 1 | 0;
    a = b ^ b >>> 9;
    b = c + (c << 3) | 0;
    c = c << 21 | c >>> 11;
    c = c + t | 0;
    return (t >>> 0) / 4294967296;
  };
}
var trozos = semilla.slice(2).match(/.{12}/g).map(b58dec);
var rand = (typeof fxrand !== "undefined") ? fxrand : sfc32(trozos[0], trozos[1], trozos[2], trozos[3]);

function azar(min, max) { return min + rand() * (max - min); }
function entero(min, max) { return Math.floor(azar(min, max + 1)); }
function elegir(arr) { return arr[(rand() * arr.length) | 0]; }
function quizas(p) { return rand() < p; }

/* ---------------- colores ---------------- */

function hsl(h, s, l) {
  return "hsl(" + (((h % 360) + 360) % 360).toFixed(1) + "," + s.toFixed(1) + "%," + l.toFixed(1) + "%)";
}

/* momento del dia: sesga los tonos y decide el astro */
var MOMENTOS = {
  amanecer:  { tonos: [330, 30, 200], luz: [45, 90], sat: [40, 90], astro: "sol" },
  mediodia:  { tonos: [195, 215, 45, 130], luz: [40, 85], sat: [45, 100], astro: "sol" },
  atardecer: { tonos: [10, 35, 300, 275], luz: [25, 70], sat: [55, 100], astro: "sol" },
  noche:     { tonos: [230, 250, 270, 210], luz: [4, 38], sat: [30, 80], astro: "luna" }
};

/* modo armonico: como se reparten los tonos alrededor del momento */
var MODOS = {
  caos:        function (t) { return rand() * 360; },
  mono:        function (t) { return t + azar(-14, 14); },
  analogo:     function (t) { return t + azar(-45, 45); },
  complemento: function (t) { return quizas(.5) ? t + azar(-20, 20) : t + 180 + azar(-20, 20); },
  triada:      function (t) { return t + elegir([0, 120, 240]) + azar(-15, 15); }
};

var momento = elegir(Object.keys(MOMENTOS));
var modo = elegir(Object.keys(MODOS));
var M = MOMENTOS[momento];

function colorNuevo() {
  var base = elegir(M.tonos);
  return hsl(MODOS[modo](base), azar(M.sat[0], M.sat[1]), azar(M.luz[0], M.luz[1]));
}

var raiz = document.documentElement.style;
for (var ci = 0; ci < 32; ci++) raiz.setProperty("--c" + ci, colorNuevo());

raiz.setProperty("--deg", entero(1, 360) + "deg");
raiz.setProperty("--deg2", entero(1, 360) + "deg");
raiz.setProperty("--t1", azar(0.9, 2.4).toFixed(2) + "s");
raiz.setProperty("--t2", azar(4, 8).toFixed(2) + "s");
raiz.setProperty("--t3", azar(2.4, 5.4).toFixed(2) + "s");
raiz.setProperty("--t4", azar(1.2, 3).toFixed(2) + "s");
raiz.setProperty("--t5", azar(8, 17).toFixed(2) + "s");
raiz.setProperty("--shad", azar(0.4, 2.2).toFixed(2) + "px");

/* tiempos del carrusel de nubes: el de atras va mas lento, esta mas lejos */
raiz.setProperty("--t6", azar(28, 75).toFixed(1) + "s");
raiz.setProperty("--t7", azar(50, 120).toFixed(1) + "s");

/* el color del rio: su propia familia de agua dentro del sistema */
var TONOS_RIO = [185, 200, 215, 230];
function colorRio() {
  return hsl(MODOS[modo](elegir(TONOS_RIO)), azar(M.sat[0], M.sat[1]), azar(M.luz[0], M.luz[1]));
}
raiz.setProperty("--rio0", colorRio());
raiz.setProperty("--rio1", colorRio());
raiz.setProperty("--rio2", colorRio());

/* la tinta viva: para lo que va SOBRE un fondo (el sol en su placa,
   los peces en su rio, los solitarios) — siempre en la luz opuesta
   a la paleta del dia, para que nunca se fundan con el fondo */
var luzMedia = (M.luz[0] + M.luz[1]) / 2;
function colorVivo() {
  return hsl(MODOS[modo](elegir(M.tonos)), azar(55, 95), luzMedia > 50 ? azar(3, 18) : azar(84, 97));
}
raiz.setProperty("--tintaviva0", colorVivo());
raiz.setProperty("--tintaviva1", colorVivo());
raiz.setProperty("--tintaviva2", colorVivo());

/* los fondos de placa: banda media de luz, distinta de la tinta viva */
function colorPlaca() {
  return hsl(MODOS[modo](elegir(M.tonos)), azar(M.sat[0], M.sat[1]), azar(32, 58));
}
raiz.setProperty("--placa0", colorPlaca());
raiz.setProperty("--placa1", colorPlaca());
raiz.setProperty("--placa2", colorPlaca());

/* ---------------- mini tracery ---------------- */

var G = {};

function expandir(regla, prof) {
  prof = prof || 0;
  if (prof > 24) return "";
  return regla.replace(/#([a-zA-Z0-9_]+)#/g, function (_, sim) {
    var opciones = G[sim];
    if (!opciones) return "";
    return expandir(elegir(opciones), prof + 1);
  });
}

function repetir(sim, veces, sep) {
  var salida = [];
  for (var i = 0; i < veces; i++) salida.push(expandir("#" + sim + "#"));
  return salida.join(sep || "");
}

/* terminales: el lexico entra directo como simbolos */
G.montanaE   = LEXICO.montanas.egipcio;
G.montanaA   = LEXICO.montanas.anatolio;
G.montanaEm  = LEXICO.montanas.emoji;
G.triangulo  = LEXICO.montanas.triangulos;
G.subida     = LEXICO.laderas.subida;
G.bajada     = LEXICO.laderas.bajada;
G.cornisaI   = LEXICO.laderas.cornisa_izq;
G.cornisaD   = LEXICO.laderas.cornisa_der;
G.cumbre     = LEXICO.laderas.cumbres;
G.macizo     = LEXICO.laderas.macizo;
G.sextante   = LEXICO.laderas.sextantes;
G.bloque     = LEXICO.laderas.bloques;
G.sombra     = LEXICO.laderas.sombras;
G.filo       = LEXICO.laderas.filos;
G.rocaE      = LEXICO.rocas.egipcio;
G.rocaB      = LEXICO.rocas.bloques;
G.boveda     = LEXICO.cielo.boveda_egipcio;
G.solE       = LEXICO.cielo.sol_egipcio.concat(LEXICO.cielo.sol_anatolio);
G.solEm      = LEXICO.cielo.sol_emoji;
G.lunaE      = LEXICO.cielo.luna_egipcio.concat(LEXICO.cielo.luna_anatolio);
G.lunaEm     = LEXICO.cielo.luna_emoji;
G.estrellaE  = LEXICO.cielo.estrellas_egipcio;
G.estrella   = LEXICO.cielo.estrellas;
G.satelite   = LEXICO.cielo.satelites;
G.nubeEm     = LEXICO.nubes.emoji;
G.tormenta   = LEXICO.nubes.tormenta_anatolio.concat(LEXICO.nubes.lluvia_egipcio);
G.viento     = LEXICO.nubes.viento_egipcio;
G.aguaE      = LEXICO.agua.egipcio.concat(LEXICO.agua.anatolio);
G.aguaEm     = LEXICO.agua.emoji;
G.onda       = LEXICO.agua.ondas;
G.barca      = LEXICO.barcas.egipcio.concat(LEXICO.barcas.emoji);
G.pezE       = LEXICO.peces.egipcio;
G.pezEm      = LEXICO.peces.emoji;
G.aveE       = LEXICO.aves.egipcio.concat(LEXICO.aves.anatolio);
G.aveEm      = LEXICO.aves.emoji;
G.bestiaE    = LEXICO.animales.egipcio.concat(LEXICO.animales.anatolio, LEXICO.animales.linearb);
G.bicho      = LEXICO.animales.reptiles_egipcio.concat(LEXICO.animales.insectos_egipcio);
G.bestiaEm   = LEXICO.animales.emoji;
G.humano     = LEXICO.humanos.egipcio.concat(LEXICO.humanos.anatolio, LEXICO.humanos.linearb);
G.mujer      = LEXICO.humanos.egipcio_mujeres;
G.dios       = LEXICO.humanos.egipcio_dioses;
G.plantaE    = LEXICO.plantas.egipcio.concat(LEXICO.plantas.anatolio, LEXICO.plantas.linearb);
G.plantaEm   = LEXICO.plantas.emoji;
G.signoA     = LEXICO.signos.lineara;
G.signoB     = LEXICO.signos.linearb_silabas;
G.vasija     = LEXICO.signos.linearb_vasijas;
G.flor       = (LEXICO.plantas.flowers || []);
G.casa       = (LEXICO.moradas ? LEXICO.moradas.egipcio.concat(LEXICO.moradas.emoji) : []);

/* simbolos que el lexico invoca desde flowers */
G.animal       = G.bestiaE.concat(G.bestiaEm);
G.grasssource  = ["🌾", "🌱", "☘", "🍀", "𓇋", "𓆰", "𓇏", "🌿"];
G.rarergrasses = ["🌷", "🌻", "🪷", "𓆸", "𓇬", "🌺", "🍄", "𓇗"];

/* todo el lexico junto: la urna de donde salen los solitarios */
var TODOS = [].concat(
  G.humano, G.mujer, G.dios, G.bestiaE, G.bestiaEm, G.aveE, G.pezE,
  G.plantaE, G.flor, G.signoA, G.signoB, G.vasija, G.montanaE,
  G.aguaE, G.estrella, G.nubeEm, G.solEm, G.casa
);

/* ---- el aire: espacios vacios como material ----
   sin distancia las cosas no habitan, solo estan impresas */

G.aire = [
  "\u00A0\u00A0", "\u00A0\u00A0\u00A0", "\u00A0\u00A0\u00A0\u00A0",
  "\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0",
  "\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0",
  "\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0",
  "\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0",
  "\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0",
  "\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0",
  "#aire##aire#", "\u00A0"
];

/* ---- reglas: cielo (capa 1, la mas atras) ---- */

G.astro = (M.astro === "sol") ?
  ["<h4>#solE#</h4>", "<h4>#solEm#</h4>", "<h5>#solE#</h5>", "<font>#solEm#</font>"] :
  ["<h4>#lunaE#</h4>", "<h4>#lunaEm#</h4>", "<h5>#lunaE#</h5>", "<font>#lunaE#</font>"];

G.brillo = (M.astro === "sol") ?
  ["#estrella#", "<sup>#estrella#</sup>", "#aire#", "#aire#"] :
  ["#estrella#", "#estrellaE#", "<sup>#estrella#</sup>", "<sub>#estrellaE#</sub>", "<sup>#estrellaE#</sup>", "#aire#"];

G.orbita = ["#satelite#", "<sup>#satelite#</sup>", "<sub>#satelite#</sub>", "<font>#satelite#</font>", "#aire#", "#aire#", "#aire#"];

G.nube = [
  "#nubeEm#", "#nubeEm##aire##nubeEm#", "<sup>#nubeEm#</sup>", "<sub>#nubeEm#</sub>",
  "<span>#nubeEm#</span>", "<font>#nubeEm#</font>", "<h6>#nubeEm#</h6>",
  "#tormenta#", "<sub>#tormenta#</sub>", "<span>#boveda#</span>", "<sup>#viento#</sup>", "#aire#",
  "#nubeEm##nubeEm##nubeEm#",                      /* racimo */
  "#nubeEm#<sub>#tormenta#</sub>",                 /* nube que llueve */
  "<font>#nubeEm##nubeEm#</font>",
  "<sup>#nubeEm##nubeEm#</sup>",
  "<sub>#nubeEm#</sub>#nubeEm#<sup>#nubeEm#</sup>" /* frente nuboso */
];

G.avecielo = ["<sub>#aveE#</sub>", "<sup>#aveE#</sup>", "<sup>#aveEm#</sup>", "#aveE#", "#aire#", "#aire#", "#aire#"];

G.cielotramo = [
  "#aire##nube##aire##brillo##aire##orbita#",
  "#brillo##aire##aire##nube##aire##avecielo#",
  "#aire##aire##orbita##aire##nube##aire#",
  "#aire##brillo##aire##aire##brillo##aire##aire#",
  "#nube##aire##nube##aire##aire##brillo#",
  "#aire##aire##avecielo##aire##orbita##aire##aire#",
  "#aire##aire##aire##nube##aire##aire#",
  "#aire##nube##salto##aire##aire##brillo##aire#"
];

/* densidades: el mismo cielo, cargado o casi vacio */
G.cielotramo_denso = [
  "#nube##nube##brillo##nube#",
  "#nube##aire##nube##nube##avecielo##nube#",
  "#brillo##nube##orbita##nube##brillo#",
  "#nube##nube##nube##brillo##nube#",
  "#avecielo##nube##brillo##nube##nube#",
  "#nube##brillo##nube##salto##nube##nube##avecielo#"
];
G.cielotramo_ralo = [
  "#aire##aire##nube##aire##aire##aire#",
  "#aire##aire##aire##brillo##aire##aire#",
  "#aire##aire##aire##aire##orbita##aire##aire#",
  "#aire##aire##aire##aire##aire#",
  "#aire##avecielo##aire##aire##aire##aire#",
  "#aire##aire##aire##aire##brillo##aire##aire##aire#"
];

/* ---- reglas: nubes frente (capa 3, sobre las montañas) ---- */

G.nubefrente = [
  "#nubeEm#", "#nubeEm##nubeEm#", "<font>#nubeEm#</font>", "<sup>#nubeEm#</sup>",
  "<h6>#nubeEm#</h6>", "<sub>#nubeEm#</sub>", "#tormenta#", "",
  "<span class='placa'>#nubeEm#</span>"
];

G.frentetramo = [
  "#aire##nubefrente##aire##avecielo##aire#",
  "#avecielo##aire##aire##nubefrente#",
  "#aire##nubefrente##aire##aire#",
  "#aire##aire##avecielo##aire#",
  "#nubefrente##aire##nubefrente##aire##aire##avecielo#"
];

G.frentetramo_denso = [
  "#nubefrente##nubefrente##avecielo##nubefrente#",
  "#nubefrente##nubefrente##nubefrente#",
  "#avecielo##nubefrente##avecielo##nubefrente#",
  "#nubefrente##aire##nubefrente##nubefrente#"
];
G.frentetramo_ralo = [
  "#aire##aire##nubefrente##aire##aire#",
  "#aire##aire##aire##avecielo##aire#",
  "#aire##aire##aire##aire#",
  "#aire##aire##aire##nubefrente##aire##aire##aire#"
];

/* ---- reglas: vida (capa 4) ---- */

G.actividad = [
  "#humano#", "#humano##humano#", "#mujer#", "#humano#<sub>#vasija#</sub>",
  "<span>#humano#</span>", "<font>#humano#</font>", "<h5>#humano#</h5>",
  "<sub>#humano#</sub>", "#mujer##humano#", "<font>#dios#</font>", "#humano#<sup>#aveE#</sup>",
  "#humano##humano##humano#",                  /* procesion */
  "#humano##aire##humano#",                    /* dos que se encuentran */
  "#humano#<sub>#bestiaE#</sub>",              /* pastor */
  "#mujer##mujer#",
  "#mujer#<sub>#humano#</sub>",                /* madre e hijo */
  "#humano#<sup>#plantaE#</sup>",              /* quien carga la cosecha */
  "<font>#humano#</font>#aire#<font>#mujer#</font>",
  "<span class='placa'>#humano#</span>"
];

G.mata = [
  "#plantaE#", "#plantaE#", "<span>#plantaE#</span>", "<h5>#plantaE#</h5>",
  "#plantaEm#", "<font>#plantaE#</font>", "<sub>#plantaE#</sub>", "<sup>#plantaE#</sup>",
  "<span class='placa'>#plantaE#</span>", "<span class='placa'>#flor#</span>"
];

G.compania = [
  "#bestiaE#", "<font>#bestiaE#</font>", "#bestiaEm#", "<sub>#bicho#</sub>",
  "<sub>#rocaE#</sub>", "#rocaB#", "<sub>#signoB#</sub>", "", "",
  "#bestiaE##bestiaE#",                        /* pareja */
  "#bestiaE##bestiaE##bestiaE#",               /* manada */
  "#bestiaE##aire##bestiaE#",
  "<sub>#bestiaE##bestiaE#</sub>",             /* crias */
  "#aveE##aveE#",                              /* bandada a ras de suelo */
  "#bestiaE#<sub>#bicho#</sub>",
  "<span class='placa'>#bestiaE#</span>",
  "<span class='placa'>#bestiaEm#</span>"
];

G.vidatramo = [
  "#mata##aire##actividad##aire##mata#",
  "#actividad##aire##compania##aire##aire##mata#",
  "#aire##mata##mata##aire##compania##aire##actividad#",
  "#actividad##actividad##aire##aire##mata##aire#",
  "#mata#<h6>#mata#</h6>#aire##actividad##aire##compania#",
  "#aire##compania##aire##actividad##aire##aire##mata#",
  "#aire##aire##actividad##aire##aire#",
  "#mata##aire##actividad##salto##aire##aire##compania##aire##mata#"
];

G.vidatramo_denso = [
  "#mata##actividad##mata##compania##mata#",
  "#actividad##compania##mata##mata##actividad#",
  "#mata##mata##actividad##actividad##mata##compania#",
  "#compania##mata##actividad##mata##compania#",
  "#actividad##mata##actividad##salto##mata##compania##mata##actividad#"
];
G.vidatramo_ralo = [
  "#aire##aire##mata##aire##aire##aire#",
  "#aire##aire##aire##actividad##aire##aire#",
  "#aire##compania##aire##aire##aire#",
  "#aire##aire##aire##aire##mata##aire##aire#",
  "#aire##aire##aire##aire##aire##actividad##aire#"
];

/* ---- reglas: agua (capa 5, al frente) ---- */

G.corriente = [
  "#aguaE#", "#aguaE#", "#onda#", "<span>#aguaE#</span>", "<font>#onda#</font>",
  "<h5>#aguaE#</h5>", "#onda##onda#", "<h6>#aguaE#</h6>"
];

G.marino = [
  "<sub>#pezE#</sub>", "<sub>#pezE#</sub>", "<sup>#pezE#</sup>", "<sub>#pezEm#</sub>",
  "<sub><font>#pezE#</font></sub>", "<sup>#barca#</sup>", "<sup><span>#barca#</span></sup>",
  "<sub>#aguaEm#</sub>", "",
  "<sub>#pezE##pezE#</sub>",                   /* cardumen chico */
  "<sub>#pezE##pezE##pezE#</sub>",             /* cardumen */
  "<sub>#plantaE#</sub>",                      /* planta acuatica */
  "<sup>#plantaE#</sup>",                      /* junco en la orilla */
  "<sub>#pezE#</sub><sup>#aveE#</sup>",        /* el ave que pesca */
  "<span class='placa'>#pezE#</span>"
];

/* el salto de linea tambien es material: el agua es una estrofa,
   no una linea — unos cuantos caracteres de agua <br> otros cuantos */

G.salto = ["<br>"];

G.verso = [
  "#corriente##corriente##marino##corriente#",
  "#aire##corriente##marino##corriente#",
  "#corriente##corriente##corriente#",
  "#corriente#<span>#corriente#</span>#marino#",
  "#marino##corriente##corriente##aire#",
  "#aire##corriente##corriente#",        /* orilla: el agua tambien respira */
  "#corriente##aire##marino#"
];

G.estrofa = [
  "#verso##salto##verso#",
  "#verso##salto##aire##verso#",
  "#verso##salto##verso##salto##verso#",
  "#verso##salto##aire##verso##salto##verso#",
  "#aire##verso##salto##verso##salto##aire##verso##salto##verso#"
];

G.verso_denso = [
  "#corriente##corriente##corriente##marino##corriente##corriente#",
  "#corriente##marino##corriente##corriente##marino##corriente#",
  "#corriente##corriente##marino##corriente##marino##corriente##corriente#"
];
G.estrofa_densa = [
  "#verso_denso##salto##verso_denso#",
  "#verso_denso##salto##verso##salto##verso_denso#",
  "#verso_denso##salto##verso_denso##salto##verso_denso#"
];

G.verso_ralo = [
  "#aire##corriente##aire#",
  "#corriente##aire##marino#",
  "#aire##marino##aire##corriente#",
  "#aire##aire##corriente##aire#"
];
G.estrofa_rala = [
  "#verso_ralo##salto##aire##verso_ralo#",
  "#verso_ralo#",
  "#verso_ralo##salto##verso_ralo##salto##verso_ralo#"
];

/* ---- rio vertical: baja de las montañas o sube del mar ----
   angosto (hasta 20% del ancho), con peces y plantas adentro */

G.brisa = ["", "", "\u00A0", "\u00A0\u00A0"];

G.riolinea = [
  "#brisa##aguaE#",
  "#brisa##aguaE##aguaE#",
  "#brisa##onda##aguaE#",
  "#brisa#<sub>#pezE#</sub>#aguaE#",
  "#brisa##aguaE#<sub>#pezE#</sub>",
  "#brisa##aguaE#<sub>#plantaRio#</sub>",
  "#brisa#<sub>#plantaRio#</sub>#aguaE#",
  "#brisa##pezEm#",
  "#brisa##onda#",
  "#brisa##aguaE##onda#",
  "#brisa#<font>#aguaE#</font>",
  "", "",                        /* el rio tambien calla: huecos verticales */
  "#brisa#",
  "#brisa##brisa#"
];

/* ---- floresta: flowers regadas al azar al pie de las montañas ---- */

G.floresta = [
  "#flor#", "#flor#", "<sub>#flor#</sub>", "<sup>#flor#</sup>", "<font>#flor#</font>",
  "#rarergrasses#", "#grasssource#", "<sub>#rarergrasses#</sub>",
  "<span class='placa'>#flor#</span>", "<span class='placa'>#rarergrasses#</span>",
  "#aire#", "#aire#", "#aire#", "#aire#"
];

G.florestatramo = [
  "#floresta##floresta##aire##floresta#",
  "#aire##floresta##aire##floresta##floresta#",
  "#floresta##aire##aire##floresta#",
  "#aire##aire##floresta##aire#"
];

/* ---- montañas raster (capa 2): laderas que se acumulan con
        diferente inclinacion, celda por celda ----

   el perfil sube y baja por segmentos; cada segmento elige inclinacion:
     suave    ~18°  🭈🭆🭂 (una fila cada tres columnas)
     media    ~34°  🭇🭄   (una fila cada dos columnas)
     recta     45°  ◢     (una fila por columna)
     empinada ~63°  🭅+🭋  (dos filas por columna)
   y debajo del perfil todo se rellena de macizo █                       */

var INCL = LEXICO.laderas.inclinaciones;

function sierra(W, H) {
  var grid = [], relleno = [], x = 0, h = 0, y;
  for (y = 0; y < H; y++) grid.push(new Array(W).fill(" "));
  for (y = 0; y < W; y++) relleno.push(0);

  function poner(cx, fy, ch) {
    if (cx >= 0 && cx < W && fy >= 0 && fy < H) grid[fy][cx] = ch;
  }

  /* una unidad de subida con la inclinacion dada; devuelve false si no cabe */
  function subir(incl, tope) {
    if (incl === "suave" && x + 3 <= W && h < tope) {
      for (var a = 0; a < 3; a++) { poner(x, H - h - 1, INCL.subida_suave[a]); relleno[x] = h; x++; }
      h++; return true;
    }
    if (incl === "media" && x + 2 <= W && h < tope) {
      for (var b = 0; b < 2; b++) { poner(x, H - h - 1, INCL.subida_media[b]); relleno[x] = h; x++; }
      h++; return true;
    }
    if (incl === "empinada" && x < W && h + 2 <= tope) {
      poner(x, H - h - 1, INCL.subida_empinada[0]);
      poner(x, H - h - 2, INCL.subida_empinada[1]);
      relleno[x] = h; x++; h += 2; return true;
    }
    if (x < W && h < tope) {   /* recta */
      poner(x, H - h - 1, INCL.subida_recta[0]); relleno[x] = h; x++; h++; return true;
    }
    return false;
  }

  /* una unidad de bajada; devuelve false si no cabe */
  function bajar(incl, piso) {
    if (incl === "suave" && x + 3 <= W && h > piso) {
      for (var c = 0; c < 3; c++) { poner(x, H - h, INCL.bajada_suave[c]); relleno[x] = h - 1; x++; }
      h--; return true;
    }
    if (incl === "media" && x + 2 <= W && h > piso) {
      for (var d = 0; d < 2; d++) { poner(x, H - h, INCL.bajada_media[d]); relleno[x] = h - 1; x++; }
      h--; return true;
    }
    if (incl === "empinada" && x < W && h - 2 >= piso) {
      poner(x, H - h, INCL.bajada_empinada[0]);
      poner(x, H - h + 1, INCL.bajada_empinada[1]);
      relleno[x] = h - 2; x++; h -= 2; return true;
    }
    if (x < W && h > piso) {   /* recta */
      poner(x, H - h, INCL.bajada_recta[0]); relleno[x] = h - 1; x++; h--; return true;
    }
    return false;
  }

  function plano(largo) {
    while (largo-- > 0 && x < W) { relleno[x] = h; x++; }
  }

  var INCLINACIONES = ["suave", "media", "recta", "recta", "empinada", "empinada"];

  /* la cordillera es una sucesion de montes, lomas y huecos:
     no tiene que ocupar todo el ancho — a veces una montaña
     simplemente no sigue, o sigue despues de un vacio */
  while (x < W) {
    var accion = elegir(["monte", "monte", "monte", "monte", "hueco", "hueco", "lomas"]);

    if (accion === "hueco") {          /* suelo desnudo: nada */
      x += entero(2, 9);
      continue;
    }

    if (accion === "lomas") {          /* colinas bajas y suaves */
      var alto = entero(1, Math.max(1, (H / 3) | 0));
      while (h < alto) if (!subir(elegir(["suave", "media"]), alto)) break;
      plano(entero(2, 6));
      while (h > 0) if (!bajar(elegir(["suave", "media"]), 0)) break;
      continue;
    }

    /* monte: subir hasta un pico alto, cresta breve, bajar hasta el suelo,
       cambiando de inclinacion en el camino */
    var pico = entero(Math.ceil(H * .55), H - 1);
    while (h < pico) if (!subir(elegir(INCLINACIONES), pico)) break;
    if (quizas(.35)) plano(entero(1, 3));
    while (h > 0) if (!bajar(elegir(INCLINACIONES), 0)) break;
    if (quizas(.5)) x += quizas(.5) ? 1 : 0;
  }

  /* relleno bajo el perfil */
  for (var cx = 0; cx < W; cx++) {
    for (var fy = H - relleno[cx]; fy < H; fy++) {
      if (grid[fy][cx] === " ") grid[fy][cx] = quizas(.07) ? elegir(LEXICO.laderas.macizo) : "█";
    }
  }

  return grid.map(function (f) { return f.join(""); }).join("<br>");
}

G.roquerio = ["<sub>#rocaE#</sub>", "<sub>#bestiaE#</sub>", "<sub>#signoA#</sub>", "<sub>#plantaE#</sub>", "", "", "", ""];

function faldas() {
  /* base de la cordillera: macizo con rocas, bestias y signos incrustados */
  var s = "";
  var n = entero(8, 18);
  for (var i = 0; i < n; i++) {
    s += expandir(elegir(["#macizo#", "#macizo#", "#macizo#", "#sextante#", "#sombra#", "#bloque#"]));
    if (quizas(.22)) s += expandir("#roquerio#");
  }
  return s;
}

/* ---------------- capas del escenario ---------------- */

/* esquemas de animacion permitidos por capa: las de adelante van sin fondo
   para dejar ver a las de atras */
var ESCENA = [
  { id: "cielo",      animas: ["anima-gradiente", "anima-gradiente", "anima-mascara"] },
  { id: "nubesfondo", animas: ["anima-tinta", "anima-sombra"] },
  { id: "sierra",     animas: ["anima-tinta", "anima-sombra", "anima-mascara"] },
  { id: "montanas",   animas: ["anima-tinta", "anima-sombra", "anima-mascara"] },
  { id: "riovert",    animas: ["anima-vivo"] },
  { id: "nubes",      animas: ["anima-tinta", "anima-sombra"] },
  { id: "vida",       animas: ["anima-tinta", "anima-sombra", "anima-mascara"] },
  { id: "agua",       animas: ["anima-tinta", "anima-mascara", "anima-sombra"] },
  { id: "solitarios", animas: ["anima-vivo"] }
];

function fila(html, alinear, pad) {
  var p = (pad === false) ? "0" : "0 " + entero(0, 18) + "%";
  return "<div class='fila' style='text-align:" + alinear + ";padding:" + p + "'>" + html + "</div>";
}

var ALINEACIONES = ["left", "center", "right", "center"];

/* cada capa poblada elige su densidad: cargada, media o casi vacia */
var DENSIDADES = ["denso", "medio", "medio", "ralo", "ralo"];
function tramoSegun(base, densidad) {
  return densidad === "medio" ? base : base + "_" + densidad;
}

/* ---- el valle es un mapa: parcelas con especie dominante ----
   la naturaleza de un lugar es ~60% una planta, con otras raras;
   la gente esta con gente, con animales, en casas, en jardines */

function vestir(ch) {
  ch = expandir(ch);   /* si el glifo es una regla (#...#), que florezca */
  var r = rand();
  if (r < .50) return ch;
  if (r < .64) return "<sub>" + ch + "</sub>";
  if (r < .75) return "<sup>" + ch + "</sup>";
  if (r < .84) return "<font>" + ch + "</font>";
  if (r < .96) return "<span class='placa'>" + ch + "</span>";   /* los elegidos llevan recuadro */
  return "<h6>" + ch + "</h6>";
}

function manojo(dominante, otras, n, fidelidad) {
  /* n individuos donde la especie dominante aparece ~fidelidad */
  var s = "";
  for (var i = 0; i < n; i++) {
    s += vestir(quizas(fidelidad) ? dominante : elegir(otras));
    if (quizas(.3)) s += expandir("#aire#");
  }
  return s;
}

var PLANTARIO = LEXICO.plantas.egipcio.concat(LEXICO.plantas.linearb, LEXICO.plantas.emoji, LEXICO.plantas.anatolio);
var GENTE = LEXICO.humanos.egipcio.concat(LEXICO.humanos.egipcio_mujeres, LEXICO.humanos.anatolio, LEXICO.humanos.linearb);
var FLORAL = G.flor.filter(function (f) { return f !== "<br>"; });

function parcela() {
  var tipo = elegir(["jardin", "jardin", "bosque", "pastizal", "caserio", "reunion", "claro", "claro"]);
  var fidelidad = azar(.5, .75);
  var s = "";

  if (tipo === "claro") return expandir("#aire##aire#");

  if (tipo === "jardin") {
    /* una flor dominante entre flores raras */
    var flor = expandir(elegir(FLORAL.length ? FLORAL : PLANTARIO));
    s = manojo(flor, G.flor.concat(G.rarergrasses), entero(4, 9), fidelidad);
    if (quizas(.5)) s = vestir(elegir(GENTE)) + expandir("#aire#") + s;   /* la jardinera */
    return s;
  }

  if (tipo === "bosque") {
    var arbol = elegir(PLANTARIO);
    return manojo(arbol, PLANTARIO.concat(G.animal), entero(4, 8), fidelidad);
  }

  if (tipo === "pastizal") {
    var res = elegir(G.animal);            /* el rebaño es de una sola especie */
    s = manojo(res, G.grasssource, entero(3, 7), fidelidad);
    if (quizas(.6)) s += expandir("#aire#") + vestir(elegir(GENTE));      /* el pastor */
    return s;
  }

  if (tipo === "caserio") {
    var casa = elegir(G.casa);             /* las casas del caserio se parecen */
    var ncasas = entero(1, 3);
    for (var c = 0; c < ncasas; c++) {
      s += vestir(casa);
      if (quizas(.7)) s += "<sub>" + elegir(GENTE) + "</sub>";            /* quien la habita */
      if (quizas(.35)) s += "<sub>" + elegir(G.vasija) + "</sub>";
      s += expandir("#aire#");
    }
    if (quizas(.5)) s += manojo(expandir(elegir(FLORAL.length ? FLORAL : PLANTARIO)), G.flor, entero(2, 4), .6);   /* su jardin */
    return s;
  }

  /* reunion: gente con gente, haciendo cosas */
  var npers = entero(2, 5);
  for (var q = 0; q < npers; q++) {
    s += vestir(elegir(GENTE));
    if (quizas(.3)) s += "<sub>" + elegir(G.vasija) + "</sub>";
  }
  if (quizas(.4)) s += expandir("#aire#") + vestir(elegir(G.animal));     /* y sus animales */
  return s;
}

function valle(densidad) {
  var n = densidad === "denso" ? entero(3, 5) : densidad === "medio" ? entero(2, 3) : entero(1, 2);
  var partes = [];
  for (var i = 0; i < n; i++) partes.push(parcela());
  return partes.join(expandir("#aire##aire##aire#"));
}

function construir() {
  var escenario = document.getElementById("escenario");
  escenario.innerHTML = "";
  var rasgos = {};
  var horizonteBase = 32;   /* donde las montañas tocan el suelo (% desde abajo) */

  ESCENA.forEach(function (capa) {
    /* el rio vertical no siempre corre */
    if (capa.id === "riovert" && quizas(.45)) { rasgos.riovert = "seco"; return; }

    var div = document.createElement("div");
    var anima = elegir(capa.animas);
    div.className = "capa capa-" + capa.id + " " + anima;
    div.style.setProperty("--degcapa", entero(1, 360) + "deg");
    var html = "";

    if (capa.id === "cielo") {
      div.style.fontSize = azar(2, 4).toFixed(2) + "vmax";
      var dc = elegir(DENSIDADES);
      rasgos["densidad cielo"] = dc;
      /* el astro una sola vez, arriba, a un lado */
      html += fila(expandir("#aire##astro##aire#"), elegir(["left", "right", "center"]));
      var nf = entero(3, 5);
      for (var f = 0; f < nf; f++) html += fila(repetir(tramoSegun("cielotramo", dc), entero(2, 5), ""), elegir(ALINEACIONES));
    }

    if (capa.id === "sierra") {
      /* cordillera lejana: mas columnas, letra chica, medio difusa.
         tamaño en px enteros para que la malla no deje costuras */
      var Ws = entero(100, 150), Hs = entero(6, 10);
      div.style.fontSize = 2 * Math.floor(escenario.clientWidth / Ws) + "px";
      div.style.bottom = entero(44, 58) + "%";
      div.style.opacity = azar(.45, .85).toFixed(2);
      html += fila(sierra(Ws, Hs), "left", false);
    }

    if (capa.id === "montanas") {
      /* cordillera cercana: alta y anclada arriba, para dejar
         el valle y el agua a la vista */
      var Wm = entero(56, 92), Hm = entero(9, 15);
      div.style.fontSize = 2 * Math.floor(escenario.clientWidth / Wm) + "px";
      horizonteBase = entero(26, 38);
      div.style.bottom = horizonteBase + "%";
      html += fila(sierra(Wm, Hm), "left", false);
      /* a veces las montañas marcan un horizonte que sigue donde ellas no */
      if (quizas(.55)) {
        var trazo = elegir(["▔", "▁", "─", "🮂", "🮁"]);
        html += fila(trazo.repeat(Wm), "left", false);
      }
      if (quizas(.3)) html += fila(faldas(), "center");
      /* al pie de las montañas, flowers regadas al azar */
      if (quizas(.7)) html += fila(repetir("florestatramo", entero(2, 4), ""), elegir(ALINEACIONES));
    }

    if (capa.id === "riovert") {
      /* el agua dulce nace en el horizonte de las montañas (nunca en
         el cielo). rio: angosto y corre hasta abajo. lago: mas ancho
         y su lecho se detiene en un punto. adentro siempre agua y
         peces; afuera, en las orillas, ciertas plantas de agua */
      div.style.fontSize = azar(1.6, 2.6).toFixed(2) + "vmax";
      div.style.top = (100 - horizonteBase) + "%";
      G.plantaRio = [elegir(PLANTARIO)];   /* la especie propia de esta agua */

      function orillaLinea() {
        var q = rand();
        if (q < .28) return expandir("#plantaRio#");
        if (q < .40) return expandir("#grasssource#");
        if (q < .48) return "<sub>" + expandir("#plantaRio#") + "</sub>";
        return "";
      }

      var lineas = [], oI = [], oD = [], r, nl, anchoLecho;

      if (quizas(.4)) {
        /* lago */
        div.style.left = entero(4, 66) + "%";
        div.style.bottom = entero(5, Math.max(6, horizonteBase - 14)) + "%";
        div.style.setProperty("--degcapa", entero(70, 110) + "deg");
        anchoLecho = azar(6, 14).toFixed(1);
        nl = entero(3, 7);
        for (r = 0; r < nl; r++) lineas.push(expandir("#riolinea#") + expandir("#riolinea#"));
        rasgos.riovert = "lago";
      } else {
        /* rio */
        div.style.left = entero(4, 78) + "%";
        var direccion = quizas(.5) ? "baja" : "sube";
        div.style.setProperty("--degcapa", (direccion === "baja" ? 180 : 0) + "deg");
        anchoLecho = azar(3, 5).toFixed(1);
        nl = entero(7, 13);
        for (r = 0; r < nl; r++) lineas.push(expandir("#riolinea#"));
        var manantial = expandir("#brisa#") + "𓈗";   /* la fuente, agua triple */
        if (direccion === "baja") lineas.unshift(manantial); else lineas.push(manantial);
        rasgos.riovert = direccion;
      }

      for (r = 0; r < lineas.length; r++) { oI.push(orillaLinea()); oD.push(orillaLinea()); }

      html += "<div class='rioflex'>" +
        "<div class='orilla'>" + oI.join("<br>") + "</div>" +
        "<div class='lecho' style='width:" + anchoLecho + "em'>" + lineas.join("<br>") + "</div>" +
        "<div class='orilla'>" + oD.join("<br>") + "</div></div>";
    }

    if (capa.id === "nubesfondo") {
      /* nubes que pasan por detras de las montañas, en carrusel lento */
      div.style.fontSize = azar(2, 4.2).toFixed(2) + "vmax";
      div.style.top = entero(6, 30) + "%";
      var db = elegir(DENSIDADES);
      var nb = entero(1, 2);
      for (var w = 0; w < nb; w++) {
        var filaB = fila(repetir(tramoSegun("frentetramo", db), entero(1, 4), ""), elegir(ALINEACIONES));
        if (quizas(.5)) filaB = filaB.replace("style='", "style='animation-direction:reverse;");
        html += filaB;
      }
    }

    if (capa.id === "nubes") {
      div.style.fontSize = azar(2.4, 5).toFixed(2) + "vmax";
      div.style.top = entero(14, 38) + "%";
      var dn = elegir(DENSIDADES);
      var nn = entero(1, 2);
      for (var g = 0; g < nn; g++) {
        var filaN = fila(repetir(tramoSegun("frentetramo", dn), entero(1, 4), ""), elegir(ALINEACIONES));
        if (quizas(.5)) filaN = filaN.replace("style='", "style='animation-direction:reverse;");
        html += filaN;
      }
    }

    if (capa.id === "vida") {
      div.style.fontSize = azar(2.2, 4.4).toFixed(2) + "vmax";
      div.style.bottom = entero(7, 17) + "%";
      var dv = elegir(DENSIDADES);
      rasgos["densidad vida"] = dv;
      var nv = entero(1, 2);
      /* la poblacion no puede compartir sistema de color con las montañas */
      var vedados = [rasgos.montanas, rasgos.sierra];
      var propios = capa.animas.filter(function (a) { return vedados.indexOf(a) < 0; });
      anima = elegir(propios.length ? propios : capa.animas);
      div.className = "capa capa-" + capa.id + " " + anima;
      /* el valle casi siempre es mapa (parcelas con especie dominante),
         a veces pura deriva como antes */
      var esMapa = quizas(.7);
      rasgos.valle = esMapa ? "parcelas" : "deriva";
      for (var h = 0; h < nv; h++) {
        html += fila(esMapa ? valle(dv) : repetir(tramoSegun("vidatramo", dv), entero(2, 4), ""), elegir(ALINEACIONES));
      }
    }

    if (capa.id === "agua") {
      div.style.fontSize = azar(2, 4).toFixed(2) + "vmax";
      var da = elegir(DENSIDADES);
      rasgos["densidad agua"] = da;
      var simboloEstrofa = da === "medio" ? "#estrofa#" : (da === "denso" ? "#estrofa_densa#" : "#estrofa_rala#");
      var ne = entero(1, 2);
      for (var k = 0; k < ne; k++) html += fila(expandir(simboloEstrofa), elegir(ALINEACIONES));
    }

    if (capa.id === "solitarios") {
      /* los solitarios: caracteres al azar con cualidades especiales
         (sup, sub, placa, borde, sombra, volteo) — como en sutilezas,
         y siempre salen, en tinta viva distinta de cualquier fondo */
      var ns = entero(3, 7);
      for (var so = 0; so < ns; so++) {
        var ch = expandir(elegir(TODOS));
        var forma = elegir(["placa", "placa", "borde", "sombra", "sub", "sup", "volteado"]);
        var cuerpo = ch;
        if (forma === "sub") cuerpo = "<sub>" + ch + "</sub>";
        if (forma === "sup") cuerpo = "<sup>" + ch + "</sup>";
        var clase = "solitario" +
          (forma === "placa" ? " placa" :
           forma === "borde" ? " q-borde" :
           forma === "sombra" ? " q-sombra" :
           forma === "volteado" ? " q-volteado" : "");
        html += "<span class='" + clase + "' style='left:" + entero(2, 90) + "%;top:" +
          entero(3, 86) + "%;font-size:" + azar(1.6, 4.6).toFixed(2) + "vmax'>" + cuerpo + "</span>";
      }
    }

    div.innerHTML = html;
    escenario.appendChild(div);
    rasgos[capa.id] = anima;
  });

  return rasgos;
}

var rasgos = construir();

/* ---------------- rasgos ---------------- */

window.$fxhashFeatures = {
  momento: momento,
  paleta: modo,
  astro: M.astro,
  "esquema cielo": rasgos.cielo,
  "esquema sierra": rasgos.sierra,
  "esquema montanas": rasgos.montanas,
  "esquema agua": rasgos.agua,
  "rio vertical": rasgos.riovert,
  "densidad cielo": rasgos["densidad cielo"],
  "densidad vida": rasgos["densidad vida"],
  "densidad agua": rasgos["densidad agua"]
};

console.log("paisajes ::", semilla);
console.log("momento:", momento, "| paleta:", modo, "| astro:", M.astro);
console.log(rasgos);

if (typeof fxpreview === "function") setTimeout(fxpreview, 800);

/* clic: otro paisaje */
document.addEventListener("click", function () { location.reload(); });
