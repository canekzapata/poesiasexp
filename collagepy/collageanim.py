...
def generar_frames_animacion(imagenes_base, num_frames=15, tamaño_canvas=(1024, 1024), num_elementos=7, num_elementos_estaticos=0):
    ...
    for img in imagenes_animadas:
        ...
        usar_combinado = random.random() < 0.2
    ...
    # Crear canvas para cada frame (inicialmente transparente)
    canvas_acumulativo = Image.new('RGBA', tamaño_canvas, (0, 0, 0, 0))
    
    # Agregar elementos estáticos al canvas (solo una vez, ya que no cambian)
    for elem in elementos_estaticos:
        canvas_acumulativo.alpha_composite(elem['imagen'], elem['posicion'])
    
    # Generar cada frame
    for frame_num in range(num_frames):
        # Crear un nuevo canvas para este frame (pero mantenemos los elementos estáticos)
        frame_actual = canvas_acumulativo.copy()
        
        # Actualizar cada elemento animado
        for elem in elementos_animados:
...
def main():
    try:
        ...
        if opcion == "1":
            ...
            try:
                num_frames = int(input("Número de frames (10-120, por defecto 55): ").strip() or "55")
                num_frames = max(10, min(120, num_frames))  # Limitar entre 10 y 120
                ...
                num_elementos = int(input("Número de elementos animados (1-20, por defecto 5): ").strip() or "5")
                num_elementos = max(1, min(20, num_elementos))
                
                num_elementos_estaticos = int(input("Número de elementos estáticos (0-14, por defecto 2): ").strip() or "2")
