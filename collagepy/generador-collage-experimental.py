...
def duplicar(imagen, tamaño_collage):
    ...
    num_duplicados = random.randint(3, 10)
    ...
    for i in range(num_duplicados):
        if patron == 'linea':
            ...
            radio = random.randint(50, max(ancho_collage, alto_collage) // 2)
...
def dibujar_formas(tamaño_canvas, num_elementos=None):
    if num_elementos is None:
        num_elementos = random.randint(3, 5)
    
    canvas = Image.new('RGBA', tamaño_canvas, (0, 0, 0, 0))
    draw = ImageDraw.Draw(canvas)
    
    for _ in range(num_elementos):
        forma = random.choice(['linea', 'rectangulo', 'circulo', 'rayones', 'linea_compleja'])
        color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255), random.randint(128, 255))
        
        if forma == 'linea':
            inicio = (random.randint(0, tamaño_canvas[0]), random.randint(0, tamaño_canvas[1]))
            fin = (random.randint(0, tamaño_canvas[0]), random.randint(0, tamaño_canvas[1]))
            draw.line([inicio, fin], fill=color, width=random.randint(4, 5))
        
        elif forma == 'rectangulo':
            x0 = random.randint(0, tamaño_canvas[0] - 1)
            y0 = random.randint(0, tamaño_canvas[1] - 1)
            x1 = random.randint(x0 + 1, tamaño_canvas[0])
            y1 = random.randint(y0 + 1, tamaño_canvas[1])
            draw.rectangle([x0, y0, x1, y1], outline=color, width=random.randint(4, 5))
        
        elif forma == 'circulo':
            x = random.randint(0, tamaño_canvas[0])
            y = random.randint(0, tamaño_canvas[1])
            radio = random.randint(10, min(tamaño_canvas) // 4)
            draw.ellipse([x-radio, y-radio, x+radio, y+radio], outline=color, width=random.randint(4, 5))
            ...
            num_rayones = random.randint(10, 30)
            
            for _ in range(num_rayones):
                dx = random.randint(-ancho//2, ancho//2)
                dy = random.randint(-alto//2, alto//2)
                x1 = x + int(dx * np.cos(angulo) - dy * np.sin(angulo))
                y1 = y + int(dx * np.sin(angulo) + dy * np.cos(angulo))
                x2 = x + int(-dx * np.cos(angulo) - dy * np.sin(angulo))
                y2 = y + int(-dx * np.sin(angulo) + dy * np.cos(angulo))
                draw.line([x1, y1, x2, y2], fill=color, width=random.randint(1, 3))
        
        elif forma == 'linea_compleja':
            num_puntos = random.randint(3, 22)
            puntos = []
            x, y = random.randint(0, tamaño_canvas[0]), random.randint(0, tamaño_canvas[1])
            puntos.append((x, y))
            ...
            for _ in range(num_puntos - 1):
                ...
                if tipo == 'recto':
                    ...
                    cx = random.randint(0, tamaño_canvas[0])
                    cy = random.randint(0, tamaño_canvas[1])
                    x = random.randint(0, tamaño_canvas[0])
                    y = random.randint(0, tamaño_canvas[1])
                    puntos.extend([(cx, cy), (x, y)])
            
            draw.line(puntos, fill=color, width=random.randint(1, 5), joint='curve')
    
    return canvas

def aplicar_formas_en_capas(collage, tamaño_collage):
    # Capa de formas debajo de las imágenes
    formas_debajo = dibujar_formas(tamaño_collage, num_elementos=random.randint(2, 6))
    
    # Combinar formas de abajo con el collage
    collage_con_formas = Image.alpha_composite(formas_debajo, collage)
    
    # Capa de formas encima de las imágenes
    formas_encima = dibujar_formas(tamaño_collage, num_elementos=random.randint(2, 6))
    
    # Combinar formas de arriba con el collage
    collage_final = Image.alpha_composite(collage_con_formas, formas_encima)
    
    return collage_final
...
def aplicar_efectos_multiples(imagen, tamaño_collage):
    try:
        ...
        # Aplicar dibujar_formas a la imagen con un 30% de probabilidad
        if random.random() < 0.3:
            formas = dibujar_formas(imagen.size)
            imagen = Image.alpha_composite(imagen, formas)
        ...
        factor_escala = random.uniform(0.2, 0.8)
...
# uso usar salida, escoge el tamaño y numero de objts. recuerda que el numero de img no está acá
def generar_collage(imagenes, num_elementos=145):
    tamaño_collage = (1200, 720)
    ...
    # Aplicar formas en capas
    collage_final = aplicar_formas_en_capas(collage, tamaño_collage)
