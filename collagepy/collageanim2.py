...
def cambiar_hue(imagen, cambio_hue):
    ...
    # Modificar el canal H (Hue)
    hsv[:, :, 0] = (hsv[:, :, 0] + int(cambio_hue * 180)) % 180
    
    # Convertir de nuevo a RGB
    rgb = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)
    
    # Recomponer la imagen con el canal alfa original
    resultado = np.dstack((rgb, alpha))
...
def mover_imagen(imagen_origen, posicion_actual, tipo_movimiento, frame, total_frames, canvas_size, parametros_movimiento=None):
    ...
    velocidad = parametros_movimiento.get('velocidad', random.randint(5, 20))  # Píxeles por frame
...
def generar_frames_animacion(imagenes_base, num_frames=15, tamaño_canvas=(1024, 1024), num_elementos=7, num_elementos_estaticos=0):
    """
    Genera una serie de frames para una animación GIF.
    ...
    # 1. No acumular (20%)
    # 2. Acumular pero borrar cada ~10 frames (20%)
    # 3. Acumular completamente (60%)
    ...
    if probabilidad < 0.2:
        ...
        frecuencia_borrado = random.randint(8, 12)  # Aproximadamente 10 frames
    ...
    for img in imagenes_animadas:
        ...
        # Determinar si este elemento usará movimientos combinados (20% de probabilidad)
        usar_combinado = random.random() < 0.4
        
        if usar_combinado:
            # Seleccionar 2 o 3 movimientos aleatorios para combinar
        ...
        # Determinar si este elemento usará cambio de hue (30% de probabilidad)
        aplicar_hue = random.random() < 0.4
        ...
        elementos_animados.append({
            ...
            'factor_escala': 1.0,
            'cambio_hue': 0.0,
            'velocidad_hue': random.uniform(0.01, 0.05) if aplicar_hue else 0,
            'aplicar_hue': aplicar_hue,
            'velocidad_escala': random.uniform(0.01, 0.05),
            'direccion_escala': 1,  # 1 para crecer, -1 para disminuir
            'angulo_rotacion': 0,
            'velocidad_rotacion': random.uniform(-5, 5)  # Grados por frame
        })
    
    # Crear canvas base con elementos estáticos (nunca se borran)
    canvas_estatico = Image.new('RGBA', tamaño_canvas, (0, 0, 0, 0))
    for elem in elementos_estaticos:
        canvas_estatico.alpha_composite(elem['imagen'], elem['posicion'])
    
    # Canvas acumulativo (se actualiza según el modo seleccionado)
    ...
    for frame_num in range(num_frames):
        ...
        if modo_acumulacion == "no_acumular":
            # No mantener el frame anterior - comenzar desde el canvas estático
            frame_actual = canvas_estatico.copy()
        elif modo_acumulacion == "borrar_periodico" and frame_num % frecuencia_borrado == 0 and frame_num > 0:
            # Borrar periódicamente - volver al canvas estático cada X frames
            canvas_acumulativo = canvas_estatico.copy()
            frame_actual = canvas_acumulativo
        else:
            # Mantener todos los frames - usar el canvas acumulativo
        ...
        for i, elem in enumerate(elementos_animados):
            # Imagen base para las transformaciones
            imagen_actual = elem['imagen']
            
            # 1. Cambiar el hue (solo si se debe aplicar)
            if elem['aplicar_hue']:
            ...
            if elem['factor_escala'] > 1.5:
                elem['direccion_escala'] = -1
            elif elem['factor_escala'] < 0.5:
                elem['direccion_escala'] = 1
            ...
            # 5. Pegar la imagen en el canvas
            frame_actual.alpha_composite(imagen_actual, nueva_pos)
        
        # Actualizar el canvas acumulativo si corresponde
        if modo_acumulacion != "no_acumular":
...
def main():
    try:
        ...
        # Preguntar al usuario si quiere generar un collage estático o una animación
        print("\n¿Qué deseas generar?")
        ...
        if opcion == "1":
            ...
            # Configurar parámetros de la animación
            print("\nConfiguración de la animación:")
            try:
                num_frames = int(input("Número de frames (10-120, por defecto 55): ").strip() or "15")
                num_frames = max(10, min(120, num_frames))  # Limitar entre 10 y 50
                
                duracion_frame = int(input("Duración de cada frame en ms (50-200, por defecto 55): ").strip() or "55")
