...
def mover_imagen(imagen_origen, posicion_actual, tipo_movimiento, frame, total_frames, canvas_size, parametros_movimiento=None):
    ...
    velocidad = parametros_movimiento.get('velocidad', random.randint(5, 40))  # Píxeles por frame
...
def generar_frames_animacion(imagenes_base, num_frames=15, tamaño_canvas=(1024, 1024), num_elementos=7, num_elementos_estaticos=0):
    ...
    for img in imagenes_animadas:
        ...
        elementos_animados.append({
            ...
            'velocidad_escala': random.uniform(0.01, 0.091),
            'direccion_escala': 1,
            'angulo_rotacion': 0,
            'velocidad_rotacion': random.uniform(-5, 5),
            'historial_posiciones': [],
            # NUEVO: Parámetros para el recorte abstracto
    ...
    for frame_num in range(num_frames):
        ...
        for i, elem in enumerate(elementos_animados):
            ...
            if elem['aplicar_recorte_abstracto']:
                ...
                if frame_num == elem['frame_inicio_recorte']:
                    ...
                    imagen_recortada = recortar_aleatorio(imagen_original, debug=False)
                ...
                if frame_num >= elem['frame_inicio_recorte'] and frame_num < frame_fin_recorte and elem['recorte_aplicado']:
                    # Usar la imagen recortada para este frame
                    imagen_actual = elem['imagen_recortada']
                else:
                    # Volver a la imagen normal fuera del período de recorte
        ...
        for idx in elementos_para_hue_global:
            ...
            if info_hue and frame_num >= info_hue['inicio'] and frame_num < info_hue['fin']:
                # Estamos dentro del período de cambio de hue para este elemento
                
                if len(elementos_animados[idx]['historial_posiciones']) > 0:
                    # Crear un canvas temporal para el elemento específico
                    canvas_elemento = Image.new('RGBA', tamaño_canvas, (0, 0, 0, 0))
                    
                    # Pegar todas las posiciones anteriores del elemento
                    for img_hist, pos_hist in elementos_animados[idx]['historial_posiciones']:
                        canvas_elemento.alpha_composite(img_hist, pos_hist)
                    
                    # Aplicar cambio de hue al canvas del elemento
                    hue_valor = info_hue['valor_hue']
                    canvas_elemento_hue = cambiar_hue(canvas_elemento, hue_valor)
                    
                    # Combinar con el frame actual
                    frame_actual = Image.alpha_composite(frame_actual, canvas_elemento_hue)
                    
                    # Si es el primer frame del cambio, registrar en el historial
                    if frame_num == info_hue['inicio']:
                        historial_global.append((idx, frame_actual.copy()))
