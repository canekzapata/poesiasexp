...
(function (root) {
  ...
  function addStandingWave(builder, spec, rng) {
    var Lx = spec.width;
    var Ly = spec.depth * rng.float(1.6, 2.2);        // buena huella de "tela"
    var amplitude = spec.height * rng.float(0.24, 0.34);
    var elevation = spec.height * rng.float(0.42, 0.6);
    var m = rng.pick([1, 2, 2, 3]);
    var n = rng.pick([1, 1, 2, 2, 3]);
    var nx = clamp(Math.round(Lx / 0.95), 12, 34);
    var ny = clamp(Math.round(Ly / 1.05), 9, 20);
    ...
    for (var iu = 0; iu <= nx; iu += 1) {
      ...
      for (var iv = 0; iv <= ny; iv += 1) {
        ...
        builder.add(body, face,
          spec.originX + (u - 0.5) * Lx,
          spec.baseY + (v - 0.5) * Ly,
          elevation + amplitude * w,
          "wave:" + iu + ":" + iv);
      }
    }
    // Dos mástiles delgados en las líneas nodales sostienen la tela tensa.
    for (var mside = -1; mside <= 1; mside += 2) {
      var mastX = spec.originX + mside * Lx * 0.5;
      for (var mz = 0; mz <= elevation; mz += 0.82) {
        builder.add(body, "frente", mastX, spec.baseY, mz, "wave-mast:" + mside + ":" + mz.toFixed(1));
        builder.add(body, "lateral", mastX + 0.7, spec.baseY + 0.7, mz, "wave-mast-side:" + mside + ":" + mz.toFixed(1));
      }
    }
  }
    ...
    if (assembly === "ESFERA MONUMENTAL") {
      ...
      addStandingWave(builder,
        sculpturalSpec("membrana", rng.float(-1.5, 1.5), 0,
          rng.float(26, 34), rng.float(24, 32), rng.float(7, 11), madness, side),
  ...
  function addGrammarAnnex(builder, gene, bounds, traits, rng) {
    ...
    if (gene === "PUENTE INCOMPLETO") {
      ...
      var moonCenterX = (moonSide > 0 ? bounds.maxX : bounds.minX) + moonSide * width * rng.float(0.18, 0.34);
      var moonBaseZ = bounds.minZ + height * rng.float(0.16, 0.52);
      addInvertedCrescentCrown(builder, moonCenterX, width * rng.float(0.5, 0.78), moonBaseZ, traits, rng);
    } else {
