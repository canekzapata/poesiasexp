...
(function (root) {
  ...
  var ARCH_STRUCTURES = ["MASA ARQUEADA", "ARCADAS", "MÉNSULAS", "BÓVEDAS", "ROTONDA", "COSTILLAS", "LOSAS", "ZIGURATS", "COLUMNAS", "PÓRTICOS", "CELOSÍAS", "PANTALLAS", "CINTAS"];
  var ARCH_VOIDS = ["ARCO", "TÚNEL", "PATIO", "GRIETA", "PORTAL", "NÚCLEO AUSENTE"];
  var ARCH_CROWNS = ["ANILLO SUPERIOR", "VOLADIZO", "CÚPULA", "BLOQUE DESPLAZADO", "TERRAZAS", "ALETAS", "CORONA"];
  ...
  var MORPHOLOGY_WEIGHTS = [
    ...
    { nombre: "COSTILLAS", peso: 16 }, { nombre: "ANILLO", peso: 15 },
    { nombre: "ESTRATOS", peso: 14 }, { nombre: "ZIGURAT", peso: 13 },
  ...
  var ARCH_STRUCTURE_WEIGHTS = [
    { nombre: "MASA ARQUEADA", peso: 24 }, { nombre: "ARCADAS", peso: 18 },
    { nombre: "MÉNSULAS", peso: 15 }, { nombre: "BÓVEDAS", peso: 14 },
    { nombre: "ROTONDA", peso: 13 }, { nombre: "COSTILLAS", peso: 10 },
    { nombre: "LOSAS", peso: 9 }, { nombre: "ZIGURATS", peso: 9 },
  ...
  var ARCH_CROWN_WEIGHTS = [
    { nombre: "ANILLO SUPERIOR", peso: 20 }, { nombre: "CÚPULA", peso: 14 },
    { nombre: "TERRAZAS", peso: 12 }, { nombre: "VOLADIZO", peso: 12 },
    { nombre: "CORONA", peso: 10 }, { nombre: "BLOQUE DESPLAZADO", peso: 8 },
    { nombre: "ALETAS", peso: 6 }
  ];
  ...
  function decide(seed, forceSpecies) {
    ...
    var species = forceSpecies || (speciesRoll < 0.78
      ? "gramatica" : speciesRoll < 0.86 ? "arco" : speciesRoll < 0.92
        ? "espera" : speciesRoll < 0.97 ? "entrelazada" : "noeuclidiana");
    ...
    if (archStructure === "MASA ARQUEADA" && rng.chance(0.58)) archCrown = "ANILLO SUPERIOR";
    else if (archVoid === "ARCO" && rng.chance(0.28)) archCrown = "ANILLO SUPERIOR";
  ...
  function addGrammarCrown(builder, gene, bounds, traits, rng) {
    ...
    if (gene === "ANILLO SUPERIOR") {
      ...
      var supportRatio = rng.float(0.58, 0.72);
      var supportTop = centerZ - radiusZ * Math.sqrt(1 - supportRatio * supportRatio);
      for (var ringSide = -1; ringSide <= 1; ringSide += 2) {
        var ringSupportX = centerX + ringSide * radiusX * supportRatio;
        for (var ringSupportZ = top; ringSupportZ <= supportTop + 0.3; ringSupportZ += 0.72) {
          builder.add("cubierta", ringSide < 0 ? "frente" : "lateral",
            ringSupportX, 0, ringSupportZ,
            "upper-ring-support:" + ringSide + ":" + ringSupportZ.toFixed(1));
          builder.add("cubierta", "lateral", ringSupportX, 0.82, ringSupportZ,
            "upper-ring-support-depth:" + ringSide + ":" + ringSupportZ.toFixed(1));
        }
      }
