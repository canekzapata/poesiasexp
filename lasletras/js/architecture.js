...
(function (root) {
  ...
  var SCULPTURAL_ASSEMBLIES = [
    "ESFERA SOBRE HÉLICE", "HÉLICE SOBRE ESFERA", "OBELISCO ROTO",
    "PIRÁMIDES INTERPENETRADAS", "ESFERA ATRAVESADA", "CUBOS EN ESPIRAL",
  ...
  var ARCH_CROWNS = ["NINGUNA", "ANILLO SUPERIOR", "VOLADIZO", "CÚPULA", "BLOQUE DESPLAZADO", "TERRAZAS", "ALETAS", "CORONA"];
  ...
  var SCULPTURAL_ASSEMBLY_WEIGHTS = [
    ...
    { nombre: "ESFERA ATRAVESADA", peso: 12 },
  ...
  function addSphere(builder, spec, rng) {
    ...
    var latitudes = rng.int(13, 19);
    var longitudes = rng.int(34, 48);
    var aperture = rng.chance(0.28 + spec.madness * 0.18);
    ...
    if (!spec.noBase && rng.chance(0.58)) {
      var stemTop = Math.max(0.6, centerZ - radiusZ);
      addEntangledPier(builder, spec.originX, spec.baseY, stemTop,
        Math.max(1.2, spec.width * 0.13), "sphere-pedestal:" + spec.body);
    }
  ...
  function addSculpturalAxis(builder, spec, rng) {
    var length = spec.length;
    var half = spec.thickness * 0.5;
    var layers = rng.int(2, 4);
    for (var x = -length / 2; x <= length / 2; x += 0.76) {
      for (var layer = 0; layer < layers; layer += 1) {
        builder.add(spec.body, layer ? "lateral" : "frente",
          spec.centerX + x, spec.centerY + layer * 0.72,
          spec.centerZ - half, "sculptural-axis:" + x.toFixed(1) + ":" + layer);
      }
      builder.add(spec.body, "techo", spec.centerX + x,
        spec.centerY + half, spec.centerZ + half,
        "sculptural-axis-top:" + x.toFixed(1));
    }
  }
  ...
  function makeSculpturalSurface(traits, interpretation, rng) {
    ...
    if (assembly === "ESFERA SOBRE HÉLICE") {
    ...
    } else if (assembly === "ESFERA ATRAVESADA") {
      addSphere(offsetBuilder(builder, rng.float(3.5, 6.5)),
        sculpturalSpec("esfera", 0, 0, rng.float(16, 21), rng.float(16, 21), rng.float(5.5, 8), madness, side),
        rng.fork("esfera-atravesada"));
      addSculpturalAxis(builder, {
        body: "eje", centerX: rng.float(-0.8, 0.8), centerY: 0,
        centerZ: rng.float(12, 17), length: rng.float(26, 34), thickness: rng.float(1.2, 2.4)
      }, rng.fork("eje"));
      modules = ["ESFERA", "EJE"];
      relation = "ATRAVESAMIENTO";
