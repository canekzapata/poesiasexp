...
(function (root) {
  ...
  function decide(seed, forceSpecies) {
    ...
    var intrusion = rng.fork("intrusion-v13").chance(0.30);
  ...
  function addGrammarAnnex(builder, gene, bounds, traits, rng) {
    ...
    if (gene === "PUENTE INCOMPLETO") {
      ...
      var moonCenterX = (moonSide > 0 ? bounds.maxX : bounds.minX) + moonSide * width * rng.float(0.16, 0.32);
  ...
  function applyIntrusions(surface, traits, rng) {
    ...
    rng.shuffle(pool);
    var count = clamp(1 + Math.floor(visibles.length / 240), 1, 5);
