...
(function (root) {
  ...
  function addParabolicAntennaCrown(builder, centerX, width, top, traits, rng) {
    ...
    var vertexZ = top + width * rng.float(0.1, 0.2);
    ...
    addCrownMast(builder, {
      key: "parabolic-mast",
      baseX: centerX + side * width * rng.float(-0.12, 0.08),
      topX: vertexX,
      baseZ: top, topZ: vertexZ,
      width: width * rng.float(0.035, 0.06),
      bend: -side * width * rng.float(0.02, 0.055)
    });

    var focusDistance = dishDepth * rng.float(1.35, 1.85);
    var focusX = vertexX - axisX * focusDistance;
    var focusZ = vertexZ - axisZ * focusDistance;
    var boomSteps = 12;
    for (var boom = 0; boom <= boomSteps; boom += 1) {
      var boomT = boom / boomSteps;
      builder.add("cubierta", boom % 3 ? "lateral" : "techo",
        lerp(vertexX, focusX, boomT), 0.2, lerp(vertexZ, focusZ, boomT),
        "parabolic-feed-boom:" + boom);
    }
    for (var feed = 0; feed < 12; feed += 1) {
      var feedTheta = feed / 12 * TAU;
      builder.add("cubierta", feed % 3 ? "frente" : "techo",
        focusX + Math.cos(feedTheta) * dishRadius * 0.07,
        Math.sin(feedTheta) * dishRadius * 0.05,
        focusZ + Math.sin(feedTheta) * dishRadius * 0.07,
        "parabolic-feed:" + feed);
    }
    return side < 0 ? "ORIENTADA A LA IZQUIERDA" : "ORIENTADA A LA DERECHA";
  }
