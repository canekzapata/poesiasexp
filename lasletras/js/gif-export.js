...
(function (root) {
  ...
  function drawText(ctx, item, glyph, color, alpha, scale, dx, dy) {
    if (!glyph) return;
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.fillStyle = color;
    ctx.font = (item.sfont * scale) + 'px "Architecture Mono", "Unicode Backup", monospace';
  ...
  function drawColophon(ctx, spec, scale) {
    var rightX = 946 * scale;
    var y = 954 * scale;
    ctx.save();
    ...
    spec.lines.forEach(function (line) {
      ...
      ctx.font = (line.s * scale) + 'px "Architecture Mono", monospace';
      ctx.fillText(line.t, rightX, y);
      y -= (line.s + 7) * scale;
    });
    ctx.globalAlpha = 0.5;
    ...
    ctx.beginPath();
    ctx.moveTo(rightX - 176 * scale, y + 5 * scale);
    ctx.lineTo(rightX, y + 5 * scale);
    ctx.stroke();
    ctx.restore();
  }

  function renderFrame(ctx, width, height, piece, state, faceColor, colophon, finalFrame) {
    ...
    if (piece.traits.carbon) {
      piece.surface.forEach(function (item, index) {
        ...
        drawText(ctx, item, item.baseGlyph, piece.colors.carbon, 0.16, scale,
          piece.traits.direction * 3.2, 2.4);
      });
    }

    piece.surface.forEach(function (item, index) {
      if (!item.glyph || !state[index]) return;
      drawText(ctx, item, state[index], faceColor(piece, item), item.opacity, scale);
    });

    if (finalFrame) drawColophon(ctx, colophon, scale);
  }
