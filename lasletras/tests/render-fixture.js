...
function render(seed, output) {
  ...
  var glyphs = surface.filter(function (item) { return item.glyph; }).map(function (item) {
  ...
  var svg = '<?xml version="1.0" encoding="UTF-8"?>\n' +
    ...
    '<g fill="' + piece.colors.frente + '" text-anchor="end"><text x="946" y="930" font-size="11">LA LETRA SOSTIENE EL TECHO · v0.6</text>' +
  ...
  return { seed: seed, output: output, piece: piece };
}
...
seeds.forEach(function (seed, index) {
  ...
  console.log(JSON.stringify({ seed: seed, file: result.output, species: result.piece.meta.speciesKey,
