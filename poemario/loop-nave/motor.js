...
function sembrar() {
  rnd = mulberry32(xmur3(SEMILLA + ":" + viaje.ciclos + ":" + viaje.crashes)());
}
...
const MAX_VIAJES = 24;   // cuántas semillas se recuerdan antes de reciclar la más vieja

/* los hitos: el arco del hallazgo ya no avanza por leer mucho, sino por
   hacer las cosas. son la columna vertebral del viaje y persisten entre
   ciclos DE LA MISMA SEMILLA: lo que una semilla descubrió, lo descubrió. */
function hitosVacios() {
  return {
    arco: 0,          // fase máxima del hallazgo GANADA por hechos (índice en CORPUS.hallazgo)
    despego: false,   // alguna vez cruzó la torre
    sonda: false,     // halló la sonda (sólo en vuelo)
    mensaje: false,   // recuperó y descifró el mensaje del núcleo (sólo si halló la sonda)
    muestra: false,   // tomó/estudió la muestra
    contacto: false,  // la otra voz usó los nombres de la tripulación
    omega: false,     // llegó a la fase Ω
  };
}

/* un VIAJE es todo lo que pertenece a una semilla: su arco, sus vueltas, su
   desgaste. cada semilla es un viaje distinto —empieza limpio y legible— y
   se recuerda aparte (la pregunta del lector: sí, recordamos por semilla). */
function viajeVacio() {
  return {
    hitos: hitosVacios(), // el arco del hallazgo, gatillado por hechos
    ciclos: 0,            // vueltas al inicio en ESTA semilla
    crashes: 0,           // crashes en ESTA semilla
    deterioro: 0,         // rareza acumulada en ESTA semilla (arranca en 0: legible)
    cicatrices: [],       // frases que dejaron los crashes de este viaje
    contaminadas: {},     // palabras secuestradas en este viaje
    recuperadas: [],      // frases rescatadas de cajas negras ajenas
    encuentros: [],       // qué se avistó
    notasAbiertas: {},    // cuántas veces se abrió cada nota (las notas muerden)
    palabrasTocadas: 0,   // cuántas puertas tocó el lector
    palabrasHeridas: {},  // qué palabras-puerta tocó y cuántas veces (el diccionario herido)
    contagio: [],         // el árbol de contagio: [{de, a}] — la contaminación que se propaga
    contadorContagio: 0,  // pasos desde el último contagio
    bitacoraMision: [],   // lo que el sistema conserva para imprimir. o pierde.
  };
}

function normalizarViaje(v) {
  const nv = Object.assign(viajeVacio(), v || {});
  nv.hitos = Object.assign(hitosVacios(), (v && v.hitos) || {});
  return nv;
}

/* la memoria GLOBAL guarda sólo lo que cruza los viajes: la versión y el
   museo de crashes (la arqueología entre lecturas). los viajes viven aparte,
   uno por semilla, y no se contaminan entre sí: por eso una semilla nueva
   despierta la nave entera y legible. */
function memoriaVacia() {
  return {
    v: 4,
    viajes: {},                     // { [semilla]: viaje }
    museo: { crashes: 0, laminas: [] },  // crashes de todas las lecturas (el museo)
  };
}
...
function cargarMemoria() {
  try {
    ...
    const raw = JSON.parse(cruda);
    if (raw && raw.v >= 4) {
      const m = Object.assign(memoriaVacia(), raw);
      m.viajes = m.viajes || {};
      m.museo = Object.assign({ crashes: 0, laminas: [] }, m.museo || {});
      for (const k in m.viajes) m.viajes[k] = normalizarViaje(m.viajes[k]);
      return m;
    }
    // migración desde v<4 (memoria plana global): lo acumulado pasa al museo,
    // y los viajes arrancan de cero. así el lector recupera un inicio legible
    // sin perder la arqueología de sus crashes anteriores.
    const m = memoriaVacia();
    m.museo.crashes = (raw && raw.crashesTotales) || 0;
    m.museo.laminas = ((raw && raw.cicatrices) || []).slice(-14);
    return m;
  } catch (e) {
...
/* el viaje activo: el de la semilla actual. una semilla = un viaje.
   los loops de la MISMA semilla conservan el viaje (el arco persiste);
   R o otra ?seed= abre un viaje nuevo, limpio, con su arco en cero. */
let viaje;
function usarViaje() {
  if (!memoria.viajes[SEMILLA]) {
    // reciclar la semilla más vieja si ya recordamos demasiadas
    const claves = Object.keys(memoria.viajes);
    if (claves.length >= MAX_VIAJES) delete memoria.viajes[claves[0]];
    memoria.viajes[SEMILLA] = viajeVacio();
  }
  viaje = memoria.viajes[SEMILLA];
  return viaje;
}
usarViaje();
...
const estado = {
  ...
  deriva: 0,
  ciclos: viaje.ciclos,
  crashes: viaje.crashes,
  pasos: 0,
  escena: "inicio",
  ruta: [],
  cicatrices: viaje.cicatrices.slice(),
  archivo: [],          // frases que la nave se dice y podría repetir
  voz: "nave",
  tomada: false,
  crash: false,
  motivoCrash: null,
  despegada: false,     // el pacto: en tierra hasta que el lector pida despegar
};

function estadoInicialDeCiclo() {
  const d = viaje.deterioro;
  estado.energia  = clamp(100 - d * 2 - azarN(6), 40, 100);
  estado.casco    = clamp(100 - viaje.crashes * 3 - azarN(8), 30, 100);
  estado.memoria  = clamp(100 - viaje.ciclos * 4 - viaje.crashes * 2, 15, 100);
  estado.senal    = clamp(100 - azarN(10) - d, 25, 100);
  estado.coherencia = clamp(100 - d * 3 - azarN(6), 20, 100);
  estado.contaminacion = clamp(Object.keys(viaje.contaminadas).length * 7 + d, 0, 70);
  estado.deriva = clamp(d * 2, 0, 60);
  estado.ciclos = viaje.ciclos;
  estado.crashes = viaje.crashes;
  estado.pasos = 0;
  estado.ruta = [];
  estado.archivo = [];
  estado.cicatrices = viaje.cicatrices.slice();
  estado.voz = "nave";
  estado.tomada = false;
  estado.crash = false;
  estado.motivoCrash = null;
  estado.despegada = false;
}
...
function archivar(frase) {
  ...
  if (prob(0.2)) {
    viaje.recuperadas.push(frase);
    if (viaje.recuperadas.length > 24) viaje.recuperadas.shift();
  }
}

function registrarCicatriz(texto) {
  viaje.cicatrices.push(texto);
  if (viaje.cicatrices.length > 18) viaje.cicatrices.shift();
  estado.cicatrices = viaje.cicatrices.slice();
  guardarMemoria();
}

function registrarEncuentro(texto) {
  viaje.encuentros.push(texto);
  if (viaje.encuentros.length > 20) viaje.encuentros.shift();
}

/* toda lectura deja huella: clics, notas, puertas. las palabras-puerta que el
   lector abre se guardan con su cuenta de dolor (el diccionario herido). */
function registrarLectura(evento, palabra) {
  if (evento === "puerta") {
    viaje.palabrasTocadas += 1;
    if (palabra) {
      const clave = String(palabra).trim().toLowerCase().slice(0, 24);
      if (!viaje.palabrasHeridas) viaje.palabrasHeridas = {};
      viaje.palabrasHeridas[clave] = (viaje.palabrasHeridas[clave] || 0) + 1;
    }
  }
  guardarMemoria();
}
...
function contaminarPalabra() {
  const candidatas = Object.keys(CORPUS.contaminables)
    .filter(p => !(p in viaje.contaminadas));
  if (!candidatas.length) return;
  const p = azar(candidatas);
  viaje.contaminadas[p] = azar(CORPUS.contaminables[p]);
  guardarMemoria();
}

/* la contaminación no se queda quieta: cada tantas lecturas, una palabra ya
   secuestrada infecta a una vecina —se registra el árbol de contagio (quién
   infectó a quién)—. cuanto más contaminada la nave, más rápido se propaga.
   se llama en cada escena; sólo prende si ya hay al menos una contaminada. */
function propagarContaminacion() {
  const madres = Object.keys(viaje.contaminadas);
  if (!madres.length) return;                       // nada que propagar todavía
  viaje.contadorContagio = (viaje.contadorContagio || 0) + 1;
  // el umbral se acorta con la contaminación: la infección se acelera
  const umbral = Math.max(3, 8 - Math.floor(estado.contaminacion / 15));
  if (viaje.contadorContagio < umbral) return;
  viaje.contadorContagio = 0;
  const candidatas = Object.keys(CORPUS.contaminables)
    .filter(p => !(p in viaje.contaminadas));
  if (!candidatas.length) return;                   // ya cayeron todas
  const madre = azar(madres);
  const hija = azar(candidatas);
  viaje.contaminadas[hija] = azar(CORPUS.contaminables[hija]);
  if (!viaje.contagio) viaje.contagio = [];
  viaje.contagio.push({ de: madre, a: hija });
  if (viaje.contagio.length > 40) viaje.contagio.shift();
  estado.contaminacion = clamp(estado.contaminacion + 4, 0, 100);
  registrarBitacora("contagio", "«" + madre + "» infectó a «" + hija + "»");
  guardarMemoria();
}

function aplicarContaminacion(texto) {
  const claves = Object.keys(viaje.contaminadas);
  if (!claves.length) return texto;
  for (const p of claves) {
    const re = new RegExp("\\b" + p + "\\b", "gi");
    texto = texto.replace(re,
      '<span class="contaminada">' + viaje.contaminadas[p] + "</span>");
  }
  return texto;
}
...
/* la intensidad del deterioro del TEXTO. la clave de la legibilidad: mientras
   la nave esté entera, el texto se lee limpio. el deterioro sólo muerde de
   verdad cuando la coherencia cae y el viaje ya se gastó de tanto dar vueltas.
   devuelve 0..1; por debajo de ~0.12 el texto queda intacto. una semilla nueva
   arranca con coherencia alta y deterioro 0, así que arranca legible. */
function intensidadDeterioro() {
  const UMBRAL_COH = 58;   // arriba de esta coherencia, el texto queda limpio
  const porCoherencia = Math.max(0, (UMBRAL_COH - estado.coherencia) / UMBRAL_COH);
  const porViaje = Math.max(0, viaje.deterioro - 2) * 0.02;   // sólo tras varias vueltas
  const porCrash = estado.crash ? 0.28 : 0;                   // un crash sí desordena el texto
  return clamp(porCoherencia * 0.72 + porViaje + porCrash, 0, 0.85);
}

/* con deterioro ALTO la palabra se rompe en estática: algunas letras se
   vuelven bloques, ondas o marcas. los "caracteres raros", con propósito,
   sólo cuando de verdad se está cayendo la lectura. */
function glifarPalabra(w, intensidad) {
  const chars = w.split("");
  const cuantas = clamp(Math.round(chars.length * (intensidad - 0.35)), 1, chars.length - 1);
  for (let k = 0; k < cuantas; k++) {
    const i = azarN(chars.length);
    const pool = rnd() < 0.5 ? CORPUS.glifos.bloques
      : (rnd() < 0.5 ? CORPUS.glifos.ondas : CORPUS.glifos.marcas);
    chars[i] = azar(pool);
  }
  return chars.join("");
}
...
function degradarTexto(texto, intensidad) {
  if (intensidad <= 0.12) return texto;   // casi todo el rango bajo queda intacto: se lee
  return texto.split(" ").map(w => {
    if (w.length < 3) return w;
    const r = rnd();
    // efectos leves primero; el escalón fuerte (caracteres raros) sólo con deterioro alto
    if (r < intensidad * 0.12) return desvocalizar(w);
    if (r < intensidad * 0.17) return w + " " + w;                       // stein
    if (r < intensidad * 0.22) return intercambiarLetras(w);
    if (intensidad > 0.45 && r < intensidad * 0.30) return glifarPalabra(w, intensidad);
    return w;
  }).join(" ");
}

function cortarSenal(texto) {
  if (estado.senal >= 38) return texto;   // sólo con señal baja de verdad
  const k = (38 - estado.senal) / 38;   // 0..1
  return texto.split(" ").map(w =>
    prob(k * 0.22) ? "···" : w).join(" ");
}
...
function puertaAleatoria(palabra) {
  ...
  if (r < 0.38) {
    ...
    spec = { acc: "insertar", arg: azar(["eco", "archivo", "manual", "senal", "poemaConcreto", "error404", "enciclopedia", "letania", "interrogatorio", "xeno", "cientifico", "telemetria", "espectro", "exobiologia", "autopsia", "cartaAstro", "tripulacion", "novela", "dialogo", "hallazgo", "verso", "haiku", "salmo", "ecologia", "cosmologia", "fisiologia", "subtrama"]) };
  } else if (r < 0.88) {
    spec = { acc: "escena", arg: azar(poolNavegable()), fx: "deriva:3" };
  } else if (r < 0.96) {
    spec = { acc: "recordar" };
  } else {
    spec = { acc: "crash" };
...
function seguirHipervinculo(a) {
  registrarLectura("puerta", a.textContent);
...
function abrirNota(texto, desde) {
  ...
  viaje.notasAbiertas[k] = (viaje.notasAbiertas[k] || 0) + 1;
  const veces = viaje.notasAbiertas[k];
  let mostrado = texto;
  let clase = "";
  if (veces === 3) {
...
function textoGlosa(palabra) {
  ...
  g = g.replace("{palabra}", palabra || azarCaos(CORPUS.palabras))
       .replace("{n}", String(1 + (viaje.palabrasTocadas % 89)));
  return g;
}
...
/* =====================================================================
   9b · las formas científicas — el discurso de la misión.
   astrofísica, exobiología, exoquímica: menos metáfora, más instrumento.
===================================================================== */

/* informe científico: plantilla con ranuras rellenadas desde el corpus duro */
function informeCientifico() {
  let t = azar(CORPUS.informesCientificos);
  t = t.replace(/\{astro\}/g, () => azar(CORPUS.astrofisica))
       .replace(/\{espectro\}/g, () => azar(CORPUS.espectros))
       .replace(/\{telemetria\}/g, () => azar(CORPUS.telemetria))
       .replace(/\{exobiologia\}/g, () => azar(CORPUS.exobiologia))
       .replace(/\{exoquimica\}/g, () => azar(CORPUS.exoquimica))
       .replace(/\{autopsia\}/g, () => azar(CORPUS.autopsiaSonda))
       .replace(/\{cuerpo\}/g, () => azar(CORPUS.cuerposDuros))
       .replace(/\{instrumento\}/g, () => azar(CORPUS.tecnica.instrumentos))
       .replace(/\{n\}/g, () => pad3(azarN(400) + 1));
  const cab = t.split(":")[0];
  archivar(cab);
  registrarBitacora("informe", cab);
  return b("cientifico", "<p>" + anotar(procesar(t), 0.4) + "</p>");
}

/* telemetría: lecturas crudas, monoespaciadas, con firma de instrumento */
function telemetriaTexto() {
  const n = 3 + azarN(3);
  const filas = muestra(CORPUS.telemetria, n);
  const cab = "TELEMETRÍA / " + azar(CORPUS.tecnica.instrumentos).toUpperCase() +
    " / T+" + pad3(azarN(999));
  return b("telemetria", '<div class="rotulo">' + cab + "</div><pre>" +
    filas.map(f => procesarPlano("  " + f)).join("\n") + "</pre>");
}

/* espectrograma: una firma atmosférica leída de lejos, con barras ascii */
function espectrograma() {
  const s = azar(CORPUS.espectros);
  registrarEncuentro("espectro: " + s.slice(0, 40));
  registrarBitacora("espectro", s);
  // una banda de absorción dibujada en caracteres de bloque
  const anchos = "▁▂▃▄▅▆▇█▇▆▅▄▃▂▁";
  let banda = "";
  for (let i = 0; i < 40; i++) {
    banda += prob(0.28) ? azar(CORPUS.glifos.bloques) : "·";
  }
  const linea = azarN(3) === 0 ? " ".repeat(azarN(30)) + anchos : banda;
  return b("espectro", '<div class="rotulo">ESPECTRO DE ABSORCIÓN / λ 400–2500 nm</div>' +
    "<pre>" + linea + "</pre><p>" + anotar(procesar(s), 0.35) + "</p>");
}

/* disección exobiológica: anatomía y fisiología de lo hallado */
function diseccionExobiologica() {
  const n = 3 + azarN(2);
  const rasgos = muestra(CORPUS.exobiologia, n);
  const cab = "FICHA EXOBIOLÓGICA / ESPÉCIMEN " +
    azar(CORPUS.glifos.runas) + pad3(azarN(999));
  const html = '<div class="rotulo">' + cab + "</div>" +
    rasgos.map(r => "<p>· " + procesar(r) + "</p>").join("") +
    '<p class="veredicto">clasificación vivo/no-vivo: ' +
    azar(["pendiente", "en disputa", "convención terrestre no aplicable",
      "sí, en el sentido técnico y aterrador", "metaboliza, luego incomoda"]) + "</p>";
  registrarBitacora("exobiología", rasgos[0]);
  return b("cientifico", html);
}

/* autopsia de sonda: forense de una máquina hallada. también tiene cuerpo. */
function autopsiaDeS0nda() {
  const n = 3 + azarN(2);
  const hallazgos = muestra(CORPUS.autopsiaSonda, n);
  const html = '<div class="rotulo">FORENSE DE SONDA / UNIDAD ' +
    azar(CORPUS.tecnica.siglas) + "-" + pad3(azarN(999)) + "</div>" +
    hallazgos.map(h => "<p>· " + procesar(h) + "</p>").join("") +
    '<p class="veredicto">causa de muerte probable: el tiempo. se recupera el mensaje; se pierde el cuerpo.</p>';
  registrarBitacora("forense", hallazgos[0]);
  return b("cientifico", html);
}

/* =====================================================================
   9c · las formas narrativas — a la pieza le faltaba historia.
   una tripulación, un arco de hallazgo que avanza, una novela de a bordo.
===================================================================== */

/* nota de tripulación: quién va (o iba) a bordo */
function notaTripulacion() {
  const t = azar(CORPUS.tripulacion);
  archivar(t.split(",")[0]);
  registrarBitacora("tripulación", t.split(",")[0]);
  return b("tripulacion", '<div class="rotulo">TRIPULACIÓN / ROL</div><p>' +
    anotar(procesar(t), 0.3) + "</p>");
}

/* la novela de a bordo: prosa narrativa, gente conviviendo con la ciencia */
function novelaDeABordo() {
  const p = azar(CORPUS.novelaDeABordo);
  archivar(p.split(",")[0] + ",");
  registrarBitacora("novela", p.slice(0, 50));
  return b("novela", "<p>" + anotar(procesar(p), 0.35) + "</p>");
}

/* diálogo de a bordo: la voz humana corta contra el fondo científico */
function dialogoDeABordo() {
  const n = 1 + azarN(2);
  const html = muestra(CORPUS.dialogos, n).map(d =>
    "<p>" + procesar(d) + "</p>").join("");
  return b("dialogo", html);
}

/* --------------------------------------------------------------------
   el arco del hallazgo, gatillado por hechos.
   índices en CORPUS.hallazgo (FASE 0 … FASE Ω). la historia NO avanza
   por leer mucho: avanza por HACER — despegar, hallar la sonda, leer su
   mensaje, tomar la muestra, recibir a la otra voz. la coherencia es esa:
   la nave no puede contar que halló la sonda antes de haberla hallado.
-------------------------------------------------------------------- */
const ARCO = {
  ANOMALIA: 0,   // se capta la anomalía (se puede desde la torre)
  TRIANGULA: 1,  // se repite y se triangula la fuente (torre / órbita)
  ESPECTRO: 2,   // el espectro del tránsito (requiere vuelo)
  DESCARTE: 3,   // se descartan las hipótesis abióticas (vuelo)
  SONDA: 4,      // la nave halla la sonda (vuelo + llegar a ella)
  MENSAJE: 5,    // el mensaje del núcleo: no aterricen (requiere sonda)
  MUESTRA: 6,    // se toma la muestra (requiere mensaje)
  RESPONDE: 7,   // la muestra responde (requiere muestra)
  CONTACTO: 8,   // la otra voz usa los nombres (requiere responde/contaminación)
  FUSION: 9,     // no distingue estudiar de ser el hallazgo
  OMEGA: 10,     // la caja negra resume; la misión se repite
};

/* marca un hito y sube el techo del arco. no salta fases: sube como mucho
   a la fase pedida, y sólo si sus precondiciones (verificadas por quien
   llama) ya se cumplieron. devuelve true si algo cambió (recién ganado). */
function marcarHito(nombre, faseMin, evento) {
  const total = CORPUS.hallazgo.length;
  let cambio = false;
  if (nombre && !viaje.hitos[nombre]) { viaje.hitos[nombre] = true; cambio = true; }
  const objetivo = clamp(faseMin, 0, total - 1);
  if (objetivo > viaje.hitos.arco) { viaje.hitos.arco = objetivo; cambio = true; }
  if (cambio) {
    const etiqueta = CORPUS.hallazgo[viaje.hitos.arco].split("·")[0].trim();
    registrarBitacora("arco", "hito ganado: " + (evento || nombre || "avance") + " → " + etiqueta);
    guardarMemoria();
  }
  return cambio;
}

/* el objetivo vivo de la misión: qué falta por hacer. da rumbo al lector
   sin puntajes ni logros — es la misión hablando, no un tablero. */
function objetivoDeArco() {
  const h = viaje.hitos;
  if (!h.despego)  return "para alcanzar la fuente de la anomalía de 1420 MHz hay que despegar: la sonda no viene, se va a buscar.";
  if (!h.sonda)    return "en vuelo, rumbo a la fuente: algo hecho —no natural— emite a 3.2 parsecs. localizar la sonda.";
  if (!h.mensaje)  return "hallada la sonda: recuperar de su núcleo el mensaje, y descifrarlo.";
  if (!h.muestra)  return "descifrado el mensaje (no aterricen): decidir qué se hace con la muestra.";
  if (!h.contacto) return "tomada la muestra: sostener la cuarentena, si es que la cuarentena era de las paredes.";
  if (!h.omega)    return "la otra voz usa nuestros nombres: averiguar de quién es el plural que ya se escribe.";
  return "la misión se repite hasta que la palabra vida deje de sonar terrestre.";
}

/* la bitácora del hallazgo: los hitos del arco como una lista de a bordo.
   estructura de gamificación, pero en voz de la nave —no es un puntaje: es
   lo que la nave ha hecho y lo que le falta—. legible siempre (✓ / ·). */
function bitacoraDeHitos() {
  const h = viaje.hitos;
  const filas = [
    ["anomalía captada (1420 MHz)", true],
    ["fuente triangulada (3.2 pc)", h.arco >= ARCO.TRIANGULA],
    ["despegue", h.despego],
    ["espectro del tránsito", h.arco >= ARCO.ESPECTRO],
    ["sonda hallada", h.sonda],
    ["mensaje descifrado (no aterricen)", h.mensaje],
    ["muestra tomada", h.muestra],
    ["la muestra responde", h.arco >= ARCO.RESPONDE],
    ["contacto (usa nuestros nombres)", h.contacto],
    ["fase Ω", h.omega],
  ];
  const cuerpo = filas.map(([t, ok]) => (ok ? "  ✓ " : "  · ") + t).join("\n");
  return b("hitos", '<div class="rotulo">BITÁCORA DEL HALLAZGO</div><pre>' + cuerpo + "</pre>");
}

/* la gravedad narrativa: hacia dónde tira el azar cuando no hay destino
   fijado. no obliga (el vacío sigue siendo vacío), pero inclina la deriva
   hacia el próximo hito, para que el viaje avance y no se pierda. */
function poolHaciaHito() {
  const h = viaje.hitos;
  if (!estado.despegada) return null;       // en tierra manda el pacto, no el arco
  if (!h.sonda)    return ["senal", "campoMagnetico", "transitoEspectral", "sondaMuerta"];
  if (!h.mensaje)  return ["autopsiaDeSonda", "cajaNegraAjena", "sondaMuerta"];
  if (!h.muestra)  return ["hallazgoExobiologico", "muestraEnCuarentena", "campoDePolvo"];
  if (!h.contacto) return ["muestraEnCuarentena", "hallazgoExobiologico", "intrusion"];
  return null;
}

/* el arco de hallazgo: la historia servida por fases, hasta el techo GANADO.
   nunca revela una fase que el lector no se haya ganado con hechos: para
   leer "la nave halla la sonda" hay que haber despegado y hallado la sonda. */
function faseDeHallazgo() {
  const total = CORPUS.hallazgo.length;
  const techo = clamp(viaje.hitos.arco, 0, total - 1);
  let i = techo;   // por defecto, la frontera: lo más nuevo que se ha ganado
  // dentro de lo ya vivido, la lectura da textura: a veces relee una fase anterior
  if (techo > 0 && prob(0.4)) i = azarN(techo + 1);
  // la memoria baja desordena el arco (pero sólo dentro de lo vivido)
  if (estado.memoria < 35 && prob(0.5)) i = azarN(techo + 1);
  const f = CORPUS.hallazgo[i];
  archivar(f.split("·")[1] ? f.split("·")[1].trim().slice(0, 40) : f.slice(0, 40));
  registrarBitacora("hallazgo", f.split("·")[0].trim());
  const frontera = (i === techo);   // la fase recién ganada se marca distinta
  return b("hallazgoarco" + (frontera ? " arco-frontera" : ""),
    "<p>" + anotar(procesar(f), 0.3) + "</p>");
}

/* =====================================================================
   9d · v7 — la vuelta biológica, astrofísica y poética.
   la ciencia también se dice en verso; la biología también es ecología.
===================================================================== */

/* verso científico: un poema que muerde un dato */
function versoCientifico() {
  const v = azar(CORPUS.versoCientifico);
  registrarBitacora("verso", v.split("\n")[0]);
  return b("verso", "<pre>" + procesarPlano(v) + "</pre>");
}

/* haiku orbital: tres versos, la ciencia comprimida */
function haikuOrbital() {
  const h = azar(CORPUS.haikuOrbital);
  return b("haiku", "<pre>" + procesarPlano(h) + "</pre>");
}

/* salmo de datos: la liturgia de la instrumentación */
function salmoDeDatos() {
  const s = azar(CORPUS.salmoDeDatos);
  return b("salmo", s.split("\n").map(l => "<p>" + procesar(l) + "</p>").join(""));
}

/* informe ecológico: la vida como sistema, no como espécimen suelto */
function informeEcologico() {
  const fuente = azar([CORPUS.ecologia, CORPUS.evolucion, CORPUS.microbioma]);
  const n = 2 + azarN(2);
  const rasgos = muestra(fuente, n);
  const cab = azar(["ECOLOGÍA DE CAMPO", "REGISTRO EVOLUTIVO", "CENSO MICROBIANO", "INFORME DE ECOSISTEMA"]);
  registrarBitacora("ecología", rasgos[0]);
  return b("cientifico", '<div class="rotulo">' + cab + "</div>" +
    rasgos.map(r => "<p>· " + procesar(r) + "</p>").join(""));
}

/* cosmología: el marco grande. tiempo profundo, materia oscura, entropía */
function notaCosmologica() {
  const fuente = azar([CORPUS.cosmologia, CORPUS.tiempoProfundo, CORPUS.dinamicaOrbital]);
  return b("cientifico", '<div class="rotulo">MARCO / ESCALA MAYOR</div><p>' +
    anotar(procesar(azar(fuente)), 0.35) + "</p>");
}

/* fisiología de la tripulación: el cuerpo humano lejos de la gravedad */
function fisiologiaTripulacion() {
  const n = 2 + azarN(2);
  return b("cientifico", '<div class="rotulo">FISIOLOGÍA / TRIPULACIÓN</div>' +
    muestra(CORPUS.fisiologiaHumana, n).map(r => "<p>· " + procesar(r) + "</p>").join(""));
}

/* subtrama: una historia lateral de la tripulación que avanza */
function subtrama() {
  const s = azar(CORPUS.subtramas);
  archivar(s.split(",")[0] + ",");
  registrarBitacora("subtrama", s.slice(0, 50));
  return b("novela", "<p>" + anotar(procesar(s), 0.3) + "</p>");
}

/* carta astronómica: un cuerpo caracterizado como geología, no como símbolo */
function cartaAstronomica() {
  const cuerpo = azar(CORPUS.cuerposDuros);
  const astro = azar(CORPUS.astrofisica);
  registrarEncuentro("cuerpo: " + cuerpo.slice(0, 40));
  return b("cientifico", '<div class="rotulo">CARTA / CATÁLOGO ' +
    azar(CORPUS.tecnica.siglas) + " " + pad3(azarN(9999)) + "</div>" +
    "<p><b>primario:</b> " + procesar(astro) + "</p>" +
    "<p><b>cuerpo:</b> " + procesar(cuerpo) + "</p>");
}

/* =====================================================================
   9e · v8 — el pacto de despegue y la lectura del cielo.
   la nave todavía en tierra: plataforma, cuenta regresiva, ascenso.
   y el cielo leído dos veces: efemérides frías, carta astral tibia.
===================================================================== */

/* bitácora de plataforma: la nave atada a la torre, todavía */
function plataformaTexto() {
  const n = 2 + azarN(2);
  const lineas = muestra(CORPUS.plataforma, n).map(l => {
    archivar(l);
    return "<p>" + anotar(procesar(l), 0.25) + "</p>";
  }).join("");
  registrarBitacora("plataforma", "la nave espera en la torre.");
  return b("plataforma", '<div class="rotulo">PLATAFORMA / T−' +
    pad3(azarN(72) + 1) + " H</div>" + lineas);
}

/* la encuesta go/no-go: cada sistema dice su palabra */
function checklistTexto() {
  const n = 5 + azarN(4);
  const filas = muestra(CORPUS.checklistLanzamiento, n);
  registrarBitacora("go/no-go", filas[0]);
  return b("checklist", '<div class="rotulo">ENCUESTA DE LANZAMIENTO / GO — NO-GO</div><pre>' +
    filas.map(f => procesarPlano(f)).join("\n") + "</pre>");
}

/* clima espacial: el humor del sol, medido en protones */
function climaEspacialTexto() {
  const n = 3 + azarN(2);
  const filas = muestra(CORPUS.climaEspacial, n);
  registrarBitacora("clima", filas[0]);
  return b("telemetria", '<div class="rotulo">CLIMA ESPACIAL / BOLETÍN DE LA ESTRELLA DE CASA</div><pre>' +
    filas.map(f => procesarPlano("  " + f)).join("\n") + "</pre>");
}

/* carta astral: el cielo leído con fe. cada signo trae su astronomía adentro */
function cartaAstralTexto() {
  const signo = azar(CORPUS.zodiaco);
  const asp = azar(CORPUS.aspectos);
  const nombre = signo.split("·")[0].trim();
  archivar("carta astral bajo " + nombre.toLowerCase());
  registrarBitacora("carta astral", nombre);
  return b("cartaastral", '<div class="rotulo">CARTA ASTRAL / LEVANTADA A MANO</div>' +
    "<p><b>signo regente:</b> " + procesar(signo) + "</p>" +
    "<p><b>aspecto:</b> " + procesar(asp) + "</p>" +
    '<p class="veredicto-astral">los astros inclinan; los motores obligan.</p>');
}

/* horóscopo de a bordo: el pronóstico que consuela midiendo */
function horoscopoTexto() {
  const n = 2 + azarN(2);
  return b("horoscopo", '<div class="rotulo">HORÓSCOPO DE A BORDO</div>' +
    muestra(CORPUS.horoscopoNave, n).map(h => "<p>" + procesar(h) + "</p>").join(""));
}

/* efemérides: el futuro del cielo con decimales */
function efemeridesTexto() {
  const n = 4 + azarN(3);
  const filas = muestra(CORPUS.efemerides, n);
  return b("efemerides", '<div class="rotulo">EFEMÉRIDES / CICLO ' +
    pad3(estado.ciclos) + "</div><pre>" +
    filas.map(f => procesarPlano(f)).join("\n") + "</pre>");
}

/* la cuenta regresiva: la única letanía que termina en fuego */
function cuentaRegresivaTexto() {
  return b("cuenta", "<pre>" +
    CORPUS.cuentaRegresiva.map(l => procesarPlano(l)).join("\n") + "</pre>");
}

/* el ascenso: la física ordenada de subir */
function ascensoTexto() {
  const n = 3 + azarN(3);
  return b("ascenso", '<div class="rotulo">ASCENSO / PERFIL DE VUELO</div>' +
    muestra(CORPUS.ascenso, n).map(l => "<p>" + procesar(l) + "</p>").join(""));
}
...
function generarTexto(tipo) {
  switch (tipo) {
    ...
    case "cientifico": return informeCientifico();
    case "telemetria": return telemetriaTexto();
    case "espectro": return espectrograma();
    case "exobiologia": return diseccionExobiologica();
    case "autopsia": return autopsiaDeS0nda();
    case "cartaAstro": return cartaAstronomica();
    case "tripulacion": return notaTripulacion();
    case "novela": return novelaDeABordo();
    case "dialogo": return dialogoDeABordo();
    case "hallazgo": return faseDeHallazgo();
    case "verso": return versoCientifico();
    case "haiku": return haikuOrbital();
    case "salmo": return salmoDeDatos();
    case "ecologia": return informeEcologico();
    case "cosmologia": return notaCosmologica();
    case "fisiologia": return fisiologiaTripulacion();
    case "subtrama": return subtrama();
    case "plataforma": return plataformaTexto();
    case "checklist": return checklistTexto();
    case "clima": return climaEspacialTexto();
    case "cartaAstral": return cartaAstralTexto();
    case "horoscopo": return horoscopoTexto();
    case "efemerides": return efemeridesTexto();
    case "cuenta": return cuentaRegresivaTexto();
    case "ascenso": return ascensoTexto();
...
function generarMision() {
  if (viaje.ciclos > 5 || viaje.deterioro > 12) return azar(CORPUS.misionesImposibles);
  if (viaje.ciclos > 2) return azar(CORPUS.misionesRaras);
  if (viaje.crashes > 0) return azar(CORPUS.misionesHeridas);
...
const NAVEGABLES = [
  "orbitaBaja", "primerSilencio", "cumulo", "campoMagnetico",
  ...
  "transitoEspectral", "hallazgoExobiologico", "autopsiaDeSonda",
  "campoDePolvo", "muestraEnCuarentena", "guardiaDeABordo",
  /* el lago de medusas no se lista: se llega por rumor */
];

/* el pacto: antes del despegue, la deriva no cruza el vacío.
   la nave está en tierra y sólo circula por lo que hay en la torre. */
const NAVEGABLES_TIERRA = [
  "revisionSistemas", "lecturaDelCielo", "cartaAstral", "bodega",
  "guardiaDeABordo", "archivoTerrestre", "cartaACentral", "suenoNave",
];

/* qué pool usa el azar según el pacto: en tierra, la torre; en vuelo, el vacío */
function poolNavegable() {
  return estado.despegada ? NAVEGABLES : NAVEGABLES_TIERRA;
}

const ESCENAS = {

  /* --- 1 · inicio: la plataforma. la nave todavía en tierra. -------
     el pacto: puede revisarse todo — sistemas, cielo, carga —
     pero despegar hay que pedirlo. nadie despega a la nave por ella. */
  inicio() {
    const mision = generarMision();
    archivar(mision);
    registrarBitacora("misión", mision);
    const cab = "PLATAFORMA / CICLO " + pad3(estado.ciclos);
    const bloques = [];
    let cuerpo =
      "<p>la nave despierta<br>no en el " +
      hiper("espacio", { acc: "nota", arg: "2" }) +
      "<br>sino atada a la " +
      hiper("torre", { acc: "insertar", arg: "plataforma" }) +
      ", en la página que aún no es " +
      hiper("cielo", { acc: "insertar", arg: "efemerides" }) + "</p>" +
      ...
      "\ncoherencia: " + (estado.coherencia > 70 ? "administrativa" : palabraNivel(estado.coherencia)) +
      "\nestado: EN TIERRA · pernos armados · ventana abierta</pre>" +
      ...
      llamadaNota() + "</p>" +
      "<p class='objetivo-arco'><span class='rotulo-arco'>rumbo del hallazgo · fase " +
      pad3(viaje.hitos.arco) + "/" + pad3(CORPUS.hallazgo.length - 1) + "</span><br>" +
      procesar(objetivoDeArco()) + "</p>" +
      "<p class='detecciones'>el pacto es simple: la nave se deja revisar entera —los " +
      hiper("sistemas", { acc: "escena", arg: "revisionSistemas" }) +
      ", el " + hiper("cielo", { acc: "escena", arg: "lecturaDelCielo" }) +
      ", la " + hiper("carga", { acc: "escena", arg: "bodega" }) +
      "— y no se mueve. para irse hay que decírselo. la nave no despega sola:" +
      " esperar es también una forma de misión.</p>";
    bloques.push(b("apertura", cuerpo));
    bloques.push(prob(0.5) ? plataformaTexto() : checklistTexto());
    ...
    // la apertura, despejada: dos verbos. despegar, o entender primero qué es
    // esto. todo lo demás (revisar sistemas / cielo / carga, los nodos ganados,
    // el crash) vive dentro de "analizar la misión", para no llenar de botones.
    const opciones = [
      op("[despegar]", ["pedirDespegue"], {}),
      op("[analizar la misión — ¿qué es esto?]", ["analizarMision"], {}),
    ];
    // atajo sólo para quien ya voló en esta semilla: soltar amarras sin repasar
    if (viaje.hitos.despego) {
      opciones.push(op("[al vacío: retomar el viaje]", poolNavegable(), {}));
    }
    return { titulo: cab, bloques, opciones };
  },

  /* --- 1a0 · analizar la misión: qué es esto + el rumbo + lo revisable ---
     el menú despejado manda aquí lo que antes llenaba la plataforma. */
  analizarMision() {
    const nucleo = CORPUS.sobreLaObra.slice(0, 4)
      .map(p => "<p>" + procesar(p) + "</p>").join("");
    const bloques = [
      b("apertura", '<div class="rotulo">ANÁLISIS DE MISIÓN / QUÉ ES ESTO</div>' + nucleo),
      b("apertura", "<p class='objetivo-arco'><span class='rotulo-arco'>rumbo del hallazgo · fase " +
        pad3(viaje.hitos.arco) + "/" + pad3(CORPUS.hallazgo.length - 1) + "</span><br>" +
        procesar(objetivoDeArco()) + "</p>"),
      bitacoraDeHitos(),
    ];
    const opciones = [
      op("[revisar los sistemas]", ["revisionSistemas"], { senal: +3, energia: -1 }),
      op("[leer el cielo]", ["lecturaDelCielo"], { memoria: +2 }),
      op("[revisar la carga y la tripulación]", ["bodega", "guardiaDeABordo"], { memoria: +2 }),
      op("[despegar]", ["pedirDespegue"], {}),
      op("[ver la galería de viajes]", ["galeriaDeViajes"], {}),
      op("[volver a la plataforma]", ["inicio"], {}),
    ];
    // los cuartos que sólo se abren con umbrales: la gamificación de fondo.
    // aparecen aquí, en el hub, cuando el viaje (o el museo) los desbloquea.
    if (viaje.ciclos >= 3) {
      opciones.push(op("[entrar al jardín que desobedece]", ["jardin"], { coherencia: +4 }));
    }
    if (viaje.crashes >= 4 || memoria.museo.crashes >= 4) {
      opciones.push(op("[visitar el museo de crashes]", ["museoDeCrashes"], {}));
    }
    if (viaje.ciclos >= 6) {
      opciones.push(op("[bajar al astillero de las misiones muertas]", ["astillero"], { memoria: +4 }));
    }
    if ((viaje.palabrasTocadas || 0) >= 60) {
      opciones.push(op("[abrir el diccionario herido]", ["diccionarioHerido"], {}));
    }
    if ((memoria.museo.crashes || 0) >= 10) {
      opciones.push(op("[leer el archivo del fondo: motor.js]", ["cuartaPared"], { coherencia: -6 }));
    }
    opciones.push(op("[crashear antes de iniciar]", [], {}, { especial: "crash" }));
    return { titulo: "ANÁLISIS DE MISIÓN", bloques, menu: true, opciones };
  },

  /* --- 1b · revisión de sistemas: se puede hacer todo sin despegar -- */
  revisionSistemas() {
    const bloques = [
      b("apertura", "<p>" + procesar(
        "la nave se deja auscultar en la torre. cada subsistema reporta, uno por uno, " +
        "con la calma de lo que todavía no se juega nada.") + "</p>"),
      checklistTexto(),
      prob(0.5) ? informe() : climaEspacialTexto(),
    ];
    return {
      titulo: "REVISIÓN DE SISTEMAS / EN TIERRA",
      bloques,
      menu: true,
      opciones: [
        op("[revisar la propulsión]", ["revisionSistemas"], { energia: -2 }),
        op("[calibrar los instrumentos contra el cielo]", ["lecturaDelCielo", "cartaAstral"], { senal: +4, memoria: +3 }),
        op("[bajar a la bodega]", ["bodega"], {}),
        op("[volver a la plataforma]", ["inicio"], {}),
        op("[pedir despegue]", ["pedirDespegue"], {}),
      ],
    };
  },

  /* --- 1c · lectura del cielo: el cielo medido y el cielo creído ---- */
  lecturaDelCielo() {
    // leer el cielo desde la torre repite la anomalía y triangula la fuente:
    // el lector ya sabe adónde ir. pero para ir hay que despegar.
    marcarHito(null, ARCO.TRIANGULA, "triangulación de la fuente desde la torre");
    const bloques = [
      b("apertura", "<p>" + procesar(
        "el mismo cielo se lee dos veces desde la torre: una con el instrumento frío —efemérides, " +
        "clima de la estrella de casa— y otra con la carta levantada a mano, que no mide sino consuela. " +
        "la nave archiva ambas. no cree en los astros; cree en la costumbre de consultarlos.") + "</p>"),
      prob(0.5) ? efemeridesTexto() : climaEspacialTexto(),
      prob(0.55) ? cartaAstralTexto() : horoscopoTexto(),
    ];
    return {
      ...
      menu: true,
      opciones: [
        op("[consultar las efemérides]", ["lecturaDelCielo"], { memoria: +3 }),
        op("[levantar la carta astral de la misión]", ["cartaAstral"], { coherencia: -2, memoria: +2 }),
        op("[pedir el boletín de clima espacial]", ["revisionSistemas"], { senal: +3 }),
        op("[volver a la plataforma]", ["inicio"], {}),
        op("[pedir despegue: la ventana está abierta]", ["pedirDespegue"], {}),
      ],
    };
  },

  /* --- 1d · carta astral: el cielo leído con fe, con astronomía dentro */
  cartaAstral() {
    const bloques = [
      cartaAstralTexto(),
      prob(0.5) ? horoscopoTexto() : b("apertura", "<p>" + procesar(
        "el zodiaco es una franja del cielo por donde caminan los planetas: doce malentendidos " +
        "bien organizados. la precesión ya corrió los signos de sus constelaciones, pero la carta " +
        "los usa igual, porque una carta astral no describe el cielo: describe el miedo con el que se lo mira.") + "</p>"),
    ];
    return {
      titulo: "CARTA ASTRAL / DE LA MISIÓN",
      bloques,
      opciones: [
        op("[creerle a la carta]", ["lecturaDelCielo", "suenoNave"], { coherencia: -4, memoria: +4 }),
        op("[contrastarla con las efemérides]", ["revisionSistemas", "lecturaDelCielo"], { senal: +3, memoria: +3 }),
        op("[guardarla en la caja negra, por si acaso]", ["bodega"], { memoria: +2 }),
        op("[pedir despegue de una vez]", ["pedirDespegue"], {}),
      ],
    };
  },

  /* --- 1e · bodega: la carga, la caja sin manifiesto, los durmientes - */
  bodega() {
    const o = azar(CORPUS.objetos);
    registrarEncuentro("carga: " + o.slice(0, 40));
    const bloques = [
      b("apertura", "<p>" + procesar(
        "en la bodega, antes de subir, la nave repasa lo que lleva: " + o + ". " +
        "y, al fondo, la caja que no está en el manifiesto, que pesa lo que pesa una caja vacía " +
        "y sin embargo promete algo adentro. Tarabá le da una vuelta y no la abre.") + "</p>"),
      prob(0.5) ? notaTripulacion() : novelaDeABordo(),
    ];
    return {
      titulo: "BODEGA / PRE-VUELO",
      bloques,
      opciones: [
        op("[inventariar la carga]", ["revisionSistemas", "bodega"], { memoria: +3 }),
        op("[revisar la caja sin manifiesto]", ["cajaNegraAjena", "muestraEnCuarentena"], { contaminacion: +5 }, { traicion: 0.2 }),
        op("[bajar a hidroponía con Ihara]", ["guardiaDeABordo", "archivoTerrestre"], { coherencia: +3, memoria: +3 }),
        op("[volver a la plataforma]", ["inicio"], {}),
        op("[pedir despegue]", ["pedirDespegue"], {}),
      ],
    };
  ...
  /* --- 1f · pedir despegue: el permiso. la última cortesía en tierra. */
  pedirDespegue() {
    const bloques = [
      b("apertura", "<p>" + procesar(
        "alguien de este lado lo pidió. la nave lo registra con una gravedad que no volverá a permitirse. " +
        "el despegue no es un botón: es un permiso mutuo. la torre entrega el brazo umbilical; " +
        "los tanques presurizan; el guiado pasa a interno y por un segundo la nave se piensa sola.") + "</p>"),
      checklistTexto(),
      cuentaRegresivaTexto(),
    ];
    registrarBitacora("despegue", "el lector pidió despegar. la cuenta empezó a bajar.");
    return {
      titulo: "PERMISO DE DESPEGUE / GO",
      bloques,
      opciones: [
        op("[confirmar: encender motores]", ["ascenso"], { energia: -6 }, { especial: "despegar" }),
        op("[esperar la siguiente ventana]", ["lecturaDelCielo", "inicio"], { coherencia: +4 }),
        op("[abortar en la torre]", ["revisionSistemas"], { energia: +2 }, { traicion: 0.15 }),
      ],
    };
  },

  /* --- 2 · ascenso: la física ordenada de subir. aquí la nave despega */
  ascenso() {
    estado.despegada = true;
    // el hito del despegue: a partir de aquí la sonda deja de ser una idea.
    // el arco puede alcanzar la triangulación de la fuente; el resto se gana en vuelo.
    marcarHito("despego", ARCO.TRIANGULA, "la nave dejó la torre");
    const bloques = [
      b("apertura", "<p>" + procesar(
        "T−0. los pernos explosivos dicen su única palabra. el motor no arde: conjuga. " +
        "la nave sube sobre una columna de física ordenada. abajo queda todo lo que la palabra abajo puede cargar.") + "</p>"),
      ascensoTexto(),
    ];
    if (prob(0.6)) bloques.push(prob(0.5) ? bitacora() : informe());
    registrarBitacora("ascenso", "la nave dejó la torre. el cielo dejó de ser pronóstico.");
    return {
      titulo: "ASCENSO / MAX-Q Y DESPUÉS",
      bloques,
      opciones: [
        op("[alcanzar órbita baja]", ["orbitaBaja"], { energia: -5 }),
        op("[hacer la ronda con la tripulación]", ["guardiaDeABordo"], { coherencia: +3, memoria: +3 }),
        op("[mirar atrás: la Tierra empieza a caber en la ventana]", ["archivoTerrestre"], { memoria: +4, deriva: +6 }, { traicion: 0.2 }),
        op("[soltar amarras hacia el espacio profundo]", ["cumulo", "campoMagnetico"], { casco: -4, senal: -6 }),
      ],
    };
  },

  /* --- 2b · despegue (alias histórico: se conserva como sinónimo) --- */
  despegue() { return ESCENAS.ascenso(); },
  ...
  orbitaBaja() {
    const bloques = [
      ...
      prob(0.5) ? novelaDeABordo() : informe(),
    ];
    if (prob(0.6)) bloques.push(prob(0.5) ? notaTripulacion() : ecoTerrestre());
    return {
      titulo: "ÓRBITA BAJA",
      bloques,
      opciones: [
        op("[soltar amarras: espacio profundo]", ["cumulo", "primerSilencio", "campoDePolvo"], { energia: -6, senal: -6 }),
        op("[una vuelta más]", ["orbitaBaja"], { deriva: +5, memoria: -3 }),
        op("[barrer el cielo con los instrumentos]", ["transitoEspectral", "cumulo"], { energia: -4, memoria: +3 }),
  ...
  cumulo() {
    ...
    return {
      titulo: "CÚMULO",
      bloques: [
        b("apertura", "<p>" + procesar(azar(CORPUS.astrofisica) +
          ". los sensores anotan: " + azar(["alegría", "duelo", "peligro", "sed"]) + ".") + "</p>"),
        prob(0.5) ? cartaAstronomica() : constelacionVerbal(),
      ],
      opciones: [
        op("[entrar al cúmulo]", ["estrellaExcesiva", "anomaliaTemporal"], { casco: -5, energia: -4 }),
        op("[caracterizar el primario]", ["transitoEspectral", "planeta"], { energia: -5, memoria: +3 }),
  ...
  senal() {
    return {
      ...
      bloques: [
        senalRecibida(),
        prob(0.35) ? telemetriaTexto() : (prob(0.5) ? archivoEncontrado() : vozNoHumana()),
      ],
      menu: true,
      opciones: [
        op("[decodificar señal]", ["sondaMuerta", "planeta", "intrusion"], { contaminacion: +7, memoria: +2 }),
        op("[triangular la fuente]", ["transitoEspectral", "cumulo"], { senal: +4, energia: -4 }),
  ...
  sondaMuerta() {
    ...
    // el hito central: hallar la sonda. sólo ocurre en vuelo —el pool de tierra
    // no la contiene—, así que despegar era, literalmente, la condición para hallarla.
    if (estado.despegada) marcarHito("sonda", ARCO.SONDA, "la nave halló la sonda");
    return {
      titulo: "SONDA",
      bloques: [
        b("apertura", "<p>" + procesar("a la deriva: " + s + ".") + "</p>"),
        prob(0.5) ? autopsiaDeS0nda() : (prob(0.5) ? tablaRota() : archivoEncontrado()),
      ],
      opciones: [
        op("[practicarle la autopsia]", ["autopsiaDeSonda"], { memoria: +5, energia: -4 }),
  ...
  /* --- 9b · caja negra ajena ------------------------------------------- */
  cajaNegraAjena() {
    // dos cajas, según el viaje. si ya se halló la sonda, ésta es su núcleo y
    // trae el mensaje (avanza el arco). si no, es la caja sin manifiesto de
    // Tarabá: promete y, al abrirse, se desvanece (la regla de Montalbetti).
    const esSonda = viaje.hitos.sonda;
    let apertura, cola;
    if (esSonda) {
      marcarHito("mensaje", ARCO.MENSAJE, "mensaje del núcleo: no aterricen");
      apertura = "la sonda hallada todavía guarda su núcleo. cerrado, promete algo adentro. se abre. " +
        "adentro, intacto, un mensaje —y la aleación no cabe en nuestra tabla—:";
      cola = prob(0.5) ? faseDeHallazgo() : b("apertura", "<p>" + procesar(
        "el mensaje, decodificado, dice: no aterricen. la superficie lee. y viene con nuestra letra, " +
        "dos ciclos más gastada.") + "</p>");
    } else {
      apertura = "es la caja sin manifiesto que Tarabá no abrió. cerrada, promete cualquier cosa, " +
        "incluso la Tierra. se abre. adentro, como estaba anunciado:";
      cola = prob(0.4) ? notaEditor() : textoSilencio(
        "(la promesa, al abrirse, se desvanece: queda el objeto a secas. abrirla era elegir)");
    }
    return {
      titulo: esSonda ? "EL NÚCLEO DE LA SONDA" : "LA CAJA SIN MANIFIESTO",
      bloques: [
        b("apertura", "<p>" + procesar(apertura) + "</p>"),
        cajaNegraTexto(),
        cola,
      ],
      opciones: [
        op("[quedarse la frase]", esSonda ? ["muestraEnCuarentena", "hallazgoExobiologico"] : poolNavegable(), { memoria: +6, contaminacion: +6 }),
        op("[devolverla]", ["primerSilencio"], { coherencia: +5 }),
        op("[cerrar la caja de nuevo]", esSonda ? ["sondaMuerta"] : poolNavegable(), {}, { traicion: 0.2 }),
      ],
    };
  },
  ...
  sondaEco() {
    ...
    // la sonda que responde con nuestra voz también es una forma del contacto.
    if (estado.despegada && (viaje.hitos.muestra || estado.contaminacion > 60))
      marcarHito("contacto", ARCO.CONTACTO, "otra sonda respondió con nuestra voz");
    return {
  ...
  planeta() {
    ...
    return {
      ...
      bloques: [
        ...
        prob(0.55) ? cartaAstronomica() : (prob(0.5) ? prosaGalactica() : enciclopediaFalsa()),
      ],
      menu: true,
      opciones: [
        op("[aterrizar]", ["crashInminente"], { casco: -10 }),
        op("[analizar su atmósfera]", ["transitoEspectral"], { energia: -6, memoria: +4 }),
  ...
  lluviaMicrometeoritos() {
    ...
    return {
      ...
      opciones: [
        ...
        op("[analizar la composición de los impactos]", ["campoDePolvo", "averia"], { memoria: +3, casco: -3 }),
  ...
  intrusion() {
    estado.voz = "otra";
    // la otra voz que usa los nombres: contacto. exige el hilo del hallazgo
    // (muestra) o un derrumbe de contaminación en pleno vacío.
    if (estado.despegada && (viaje.hitos.muestra || estado.contaminacion > 60))
      marcarHito("contacto", ARCO.CONTACTO, "la otra voz usó los nombres de la tripulación");
    return {
      titulo: "INTRUSIÓN",
      bloques: [
        vozIntrusa(),
  ...
  tomaDeControl() {
    ...
    // la toma es contacto; y si ya hubo contacto, estudiar y ser se confunden (fusión).
    if (estado.despegada && (viaje.hitos.muestra || estado.contaminacion > 55))
      marcarHito("contacto", ARCO.CONTACTO, "la otra voz tomó el timón");
    if (viaje.hitos.contacto) marcarHito(null, ARCO.FUSION, "estudiar el hallazgo y ser el hallazgo se confunden");
    return {
  ...
  escenaOlvidada() {
    const fuente = estado.archivo.length ? estado.archivo :
      (viaje.recuperadas.length ? viaje.recuperadas : CORPUS.bitacoras);
  ...
  crash() {
    ...
    viaje.crashes += 1;
    viaje.deterioro += 1;
    // el museo cruza los viajes: cada crash, de cualquier semilla, queda
    // en la arqueología común (el museo de crashes lo exhibe entre lecturas).
    memoria.museo.crashes = (memoria.museo.crashes || 0) + 1;
    const motivo = estado.motivoCrash || azar(CORPUS.crashes);
    estado.motivoCrash = null;
    const numero = pad3(viaje.crashes);
    memoria.museo.laminas.push("CRASH " + numero + " · semilla " + SEMILLA + ": " + motivo);
    if (memoria.museo.laminas.length > 20) memoria.museo.laminas.shift();
    ...
    const bloques = [
      b("crash crt-" + (viaje.crashes % 6), "<pre>" +
    ...
    // a los diez crashes de por vida, la falla abre una grieta estructural:
    // por ahí se ve el archivo que escribe la obra. la cuarta pared.
    if ((memoria.museo.crashes || 0) >= 10) {
      bloques.push(b("glitch", "<pre>" + glitchVerbal(
        "la grieta llega hasta el archivo que te escribe") + "</pre>"));
      opciones.push(op("[seguir la grieta hasta motor.js]", ["cuartaPared"], { coherencia: -8 }));
    }
    return { titulo: "", bloques, opciones };
  },

  /* --- 28 · caja negra --------------------------------------------------------------------------------------- */
  cajaNegra() {
    // tras el contacto, la caja negra resume el hallazgo: la fase Ω.
    if (viaje.hitos.contacto) marcarHito("omega", ARCO.OMEGA, "la caja negra resume el hallazgo");
    return {
      titulo: "CAJA NEGRA",
  ...
  /* --- 29 · loop ------------------------------------------------------------------------------------------------ */
  loop() {
    // Ω: sólo tras el contacto, la caja negra puede resumir el hallazgo,
    // y la misión se reinicia con el plural adentro.
    if (viaje.hitos.contacto) marcarHito("omega", ARCO.OMEGA, "la misión se repite con el plural adentro");
    const bloques = [
      b("apertura", "<pre>" +
        ...
        "\n\nse conserva: " + (Object.keys(viaje.contaminadas).length ?
          "las palabras contaminadas (" + Object.keys(viaje.contaminadas).join(", ") + ")" :
          "casi todo, que es una forma de no conservar nada") +
        "\nse pierde: la certeza de que esto no había pasado ya" +
        "</pre>"),
      manualTexto(),
    ];
    if (viaje.recuperadas.length) {
      bloques.push(b("cicatrices", '<div class="rotulo">frases recuperadas</div><p>' +
        procesarPlano(azar(viaje.recuperadas)) + "</p>"));
    }
    return {
      titulo: "",
      bloques,
  ...
  finalFalso() {
    ...
    return {
      ...
      opciones: [
        ...
        op("[rechazarlo]", poolNavegable(), { coherencia: -4 }),
  ...
  /* --- 38 · tránsito espectral: leer una atmósfera de lejos ----------------------------------------------------------- */
  transitoEspectral() {
    // el espectro del tránsito y el descarte de hipótesis: sólo tiene sentido
    // ya en vuelo, camino a la fuente. es el paso previo a hallar la sonda.
    if (estado.despegada) marcarHito(null, ARCO.DESCARTE, "espectro del tránsito y descarte de hipótesis abióticas");
    return {
      titulo: "TRÁNSITO / ESPECTROSCOPÍA",
      bloques: [
        b("apertura", "<p>" + procesar(
          "el cuerpo cruza frente a su estrella. durante 4.1 horas, la luz que lo bordea trae, escrita en líneas de absorción, la lista de lo que respira. o de lo que se descompone. el instrumento no distingue esperanza de química.") + "</p>"),
        espectrograma(),
        prob(0.5) ? faseDeHallazgo() : (prob(0.5) ? telemetriaTexto() : informeCientifico()),
      ],
      opciones: [
        op("[confirmar el desequilibrio químico]", ["hallazgoExobiologico", "planeta"], { memoria: +5, energia: -5 }),
        op("[descartar hipótesis abióticas]", ["cartaACentral", "averia"], { coherencia: +4, energia: -4 }),
        op("[no creerle al instrumento]", ["anomaliaTemporal", "errorNavegacion"], { coherencia: -5 }, { traicion: 0.2 }),
        op("[transmitir el hallazgo a la Tierra]", ["senal", "silencio"], { senal: -8 }),
      ],
    };
  },

  /* --- 39 · hallazgo exobiológico: el cuerpo importa, no sólo la lengua ----------------------------------------------- */
  hallazgoExobiologico() {
    registrarEncuentro("hallazgo exobiológico");
    // el cuerpo que metaboliza y responde: fase del arco si ya hay muestra.
    if (viaje.hitos.muestra) marcarHito(null, ARCO.RESPONDE, "la muestra responde");
    return {
      titulo: "HALLAZGO EXOBIOLÓGICO",
      bloques: [
        b("apertura", "<p>" + procesar(
          "no es una señal: es un cuerpo. metaboliza, se repara, responde a estímulos. " +
          "el protocolo pide clasificarlo antes de nombrarlo. la nave, que ya lo nombró, procede a la ficha.") + "</p>"),
        faseDeHallazgo(),
        diseccionExobiologica(),
        prob(0.5) ? dialogoDeABordo() : null,
      ].filter(Boolean),
      menu: true,
      opciones: [
        op("[tomar una muestra]", ["muestraEnCuarentena"], { contaminacion: +8, memoria: +4 }),
        op("[estudiar su fisiología sin tocarlo]", ["transitoEspectral", "planetaTexto"], { energia: -6, memoria: +5 }),
        op("[intentar comunicación química]", ["sondaEco", "intrusion"], { contaminacion: +10 }),
        op("[declararlo no-vivo por prudencia]", ["cartaACentral", "eclipse"], { coherencia: +5, memoria: -4 }),
      ],
    };
  },

  /* --- 40 · autopsia de sonda: forense de una máquina hallada -------------------------------------------------------- */
  autopsiaDeSonda() {
    registrarEncuentro("autopsia de sonda");
    // hallar la sonda para abrirla: el forense también cuenta como hallazgo.
    if (estado.despegada) marcarHito("sonda", ARCO.SONDA, "forense de la sonda hallada");
    return {
      titulo: "FORENSE DE SONDA",
      bloques: [
        b("apertura", "<p>" + procesar(
          "la sonda hallada no responde. se abre. una máquina también tiene fisiología: " +
          "un metabolismo de isótopos que decae, una circulación de fluido que se congela, una memoria que sobrevive al cuerpo.") + "</p>"),
        autopsiaDeS0nda(),
        prob(0.4) ? cajaNegraTexto() : b("apertura", "<p>" + procesarPlano(
          "le están practicando la autopsia a un espejo con número de serie distinto.") + "</p>"),
      ],
      opciones: [
        op("[recuperar el mensaje del núcleo]", ["cajaNegraAjena", "senal"], { memoria: +6, contaminacion: +5 }),
        op("[canibalizar sus piezas]", ["averia", "orbitaBaja"], { casco: +10, energia: -6 }),
        op("[enterrarla en el cementerio orbital]", ["cementerioOrbital"], { coherencia: +4 }),
        op("[reconocerse en ella]", ["escenaOlvidada", "suenoNave"], { memoria: -5, deriva: +4 }),
      ],
    };
  },

  /* --- 41 · campo de polvo / condritos: cosmoquímica del origen ------------------------------------------------------ */
  campoDePolvo() {
    estado.casco = clamp(estado.casco - probabilidad(2, 6), 0, 100);
    return {
      titulo: "CAMPO DE POLVO / CONDRITOS",
      bloques: [
        b("apertura", "<p>" + procesar(
          "un cinturón de escombros. condritos carbonáceos con aminoácidos e inclusiones más viejas que el sol. " +
          "el material del que estamos hechos, antes de estar hechos, golpeando el casco a 12 km/s.") + "</p>"),
        prob(0.5) ? telemetriaTexto() : informeCientifico(),
      ],
      opciones: [
        op("[capturar un condrito]", ["muestraEnCuarentena", "averia"], { casco: -4, memoria: +4 }),
        op("[atravesar a toda velocidad]", ["lluviaMicrometeoritos", "errorNavegacion"], { casco: -8, energia: -5 }),
        op("[analizar su cosmoquímica]", ["transitoEspectral", "cartaACentral"], { energia: -5, memoria: +4 }),
        op("[rodear el campo]", ["derivaLarga", "cumulo"], { deriva: +6 }),
      ],
    };
  },

  /* --- 42 · muestra en cuarentena: bioseguridad y la palabra vivo --------------------------------------------------- */
  muestraEnCuarentena() {
    estado.contaminacion = clamp(estado.contaminacion + 5, 0, 100);
    // tomar la muestra es fase del arco sólo después de descifrar el mensaje;
    // antes, es un frasco frío cualquiera (la caja de la bodega, un condrito).
    if (viaje.hitos.mensaje) marcarHito("muestra", ARCO.MUESTRA, "muestra en cuarentena nivel 4");
    return {
      titulo: "MUESTRA EN CUARENTENA / NIVEL 4",
      bloques: [
        b("apertura", "<p>" + procesar(
          "la muestra está sellada a 4 K tras tres barreras. el biosensor marca actividad enzimática. " +
          "viva, quizá, en el sentido técnico y aterrador de la palabra viva. la esclusa parpadea en ámbar.") + "</p>"),
        prob(0.5) ? diseccionExobiologica() : b("cientifico",
          '<div class="rotulo">PROTOCOLO DE BIOSEGURIDAD</div><p>' +
          procesar("1. no exponer a solvente. 2. no exponer a la tripulación. 3. no exponer a la esperanza. " +
            "una quiralidad invertida no puede infectarnos — pero puede enseñarnos, que es peor.") + "</p>"),
      ],
      opciones: [
        op("[secuenciar su información]", ["hallazgoExobiologico", "planetaTexto"], { memoria: +6, contaminacion: +6 }),
        op("[abrir la cámara (contra el protocolo)]", ["intrusion", "crashInminente"], { contaminacion: +18 }, { traicion: 0.3 }),
        op("[incinerar la muestra]", ["primerSilencio", "averia"], { contaminacion: -12, memoria: -6 }),
        op("[mantener la cuarentena y seguir]", ["orbitaBaja", "derivaLarga"], { coherencia: +5 }),
      ],
    };
  },

  /* --- 43 · guardia: la vida a bordo entre medición y medición --------------------------------------------------------- */
  guardiaDeABordo() {
    return {
      titulo: "GUARDIA / TERCERA VIGILIA",
      bloques: [
        novelaDeABordo(),
        prob(0.6) ? dialogoDeABordo() : notaTripulacion(),
      ],
      opciones: [
        op("[relevar la guardia]", ["orbitaBaja", "averia", "transitoEspectral"], { energia: -3 }),
        op("[seguir la historia de la tripulación]", ["guardiaDeABordo"], { memoria: +4, energia: -2 }),
        op("[bajar a hidroponía con Ihara]", ["archivoTerrestre", "hallazgoExobiologico"], { coherencia: +4, memoria: +3 }),
        op("[revisar la caja sin manifiesto]", ["cajaNegraAjena", "muestraEnCuarentena"], { contaminacion: +6 }, { traicion: 0.2 }),
      ],
    };
  },

  /* --- 44 · escenas que la memoria desbloquea ------------------------------------------------------------------------- */
  jardin() {
  ...
  museoDeCrashes() {
    // la arqueología de la falla. la sala de ESTE viaje (cicatrices) y la
    // colección permanente que cruza todas las semillas (el museo global).
    const placas = estado.cicatrices.slice(-5).map(c =>
      "<p>" + procesarPlano(c) + "</p>").join("") || "<p>(sala de este viaje: aún vacía)</p>";
    const permanente = (memoria.museo.laminas || []).slice(-6)
      .map(c => "<p>" + procesarPlano(c) + "</p>").join("");
    const bloques = [
      b("apertura", "<p>" + procesar(
        "un equipo de arqueólogos del futuro — unos de silicio, otros de carbono — " +
        "montó esta sala con las ruinas de tus lecturas. en la vitrina de este viaje, las placas dicen:") + "</p>"),
      b("cicatrices", placas),
    ];
    if (permanente) {
      bloques.push(b("cicatrices", '<div class="rotulo">colección permanente · ' +
        (memoria.museo.crashes || 0) + " crashes de todas las semillas</div>" + permanente));
    }
    return {
  ...
  /* --- 45 · la galería de viajes: cada semilla, su avance -----------------
     cierra la idea de recordar por seed: la nave colecciona sus derivas. */
  galeriaDeViajes() {
    const seeds = Object.keys(memoria.viajes);
    const filas = seeds.map(s => {
      const v = memoria.viajes[s];
      const fase = pad3(v.hitos.arco) + "/" + pad3(CORPUS.hallazgo.length - 1);
      const marca = v.hitos.omega ? "Ω" : (v.hitos.sonda ? "◇" : (v.hitos.despego ? "↑" : "·"));
      const aqui = (s === SEMILLA) ? "  ← estás aquí" : "";
      return " " + marca + "  " + ("semilla " + s).padEnd(20) + " fase " + fase +
        " · " + pad3(v.ciclos) + " vueltas · " + pad3(v.crashes) + " crashes" + aqui;
    }).join("\n") || "  (aún no hay más viaje que éste)";
    return {
      titulo: "GALERÍA DE VIAJES",
      bloques: [
        b("apertura", "<p>" + procesar(
          "cada semilla es un viaje que la nave recuerda aparte. la galería los alinea: " +
          "lo que cada uno avanzó queda, aunque el lector no vuelva. la nave colecciona sus propias derivas.") + "</p>"),
        b("hitos", '<div class="rotulo">VIAJES RECORDADOS · ' + seeds.length + "</div><pre>" +
          procesarPlano(filas) + "</pre>"),
        b("apertura", "<p class='detecciones'>Ω llegó al final · ◇ halló la sonda · ↑ despegó · · sigue en tierra</p>"),
      ],
      opciones: [
        op("[volver a la plataforma]", ["inicio"], {}),
        op("[analizar esta misión]", ["analizarMision"], {}),
        op("[zarpar en otra semilla]", [], {}, { especial: "nuevaSemilla" }),
      ],
    };
  },

  /* --- 46 · el astillero de las misiones muertas (ruta secreta: ≥6 vueltas) */
  astillero() {
    const muertas = muestra(CORPUS.astillero, 3).map(l => "<p>" + procesar(l) + "</p>").join("");
    const declaradas = (viaje.bitacoraMision || []).filter(e => e.t === "misión").slice(-4)
      .map(e => "<p>" + procesarPlano("· " + e.x) + "</p>").join("");
    return {
      titulo: "EL ASTILLERO DE LAS MISIONES MUERTAS",
      bloques: [
        b("apertura", muertas),
        declaradas
          ? b("cicatrices", '<div class="rotulo">tus objetivos declarados, oxidándose</div>' + declaradas)
          : archivoEncontrado(),
      ],
      opciones: [
        op("[descolgar un objetivo muerto]", ["averia", "cartaACentral"], { memoria: +5 }),
        op("[canibalizar un casco viejo]", ["orbitaBaja", "derivaLarga"], { casco: +8, energia: -5 }),
        op("[prometer de nuevo]", ["inicio", "pedirDespegue"], { coherencia: -4 }, { traicion: 0.2 }),
        op("[volver]", poolNavegable(), {}),
      ],
    };
  },

  /* --- 47 · la nave que escribe esta obra (ruta secreta: ≥10 crashes museo)
     la cuarta pared con fisura estructural. la nave se descubre escrita. */
  cuartaPared() {
    estado.coherencia = clamp(estado.coherencia - 6, 0, 100);
    const lineas = muestra(CORPUS.cuartaPared, 3).map(l => "<p>" + anotar(procesar(l), 0.4) + "</p>").join("");
    return {
      titulo: "LA NAVE QUE ESCRIBE ESTA OBRA",
      bloques: [
        b("apertura", lineas),
        b("informe", "<pre>" + procesarPlano(
          "archivo: motor.js\nfunción en curso: renderEscena(\"cuartaPared\")\ndecisión en curso: la que crees tomar\nautor del casco: desconocido / tú / la otra voz") + "</pre>"),
      ],
      opciones: [
        op("[cerrar el archivo y fingir que no]", poolNavegable(), { coherencia: +5 }),
        op("[saludar al que lee]", ["sondaEco", "intrusion"], { contaminacion: +8 }),
        op("[leer la última línea del archivo]", ["finalFalso"], { memoria: -4 }),
        op("[tocar la palabra que te nombra]", [], {}, { especial: "crash" }),
      ],
    };
  },

  /* --- 48 · el diccionario herido (ruta secreta: ≥60 puertas tocadas)
     todas las palabras que el lector abrió, ordenadas por dolor. */
  diccionarioHerido() {
    const entradas = Object.entries(viaje.palabrasHeridas || {})
      .sort((a, b) => b[1] - a[1]).slice(0, 14);
    const cuerpo = entradas.map(([w, n]) => {
      const herida = n > 2 ? desvocalizar(w) : w;   // de tanto abrirla, se abrió del todo
      return "  " + herida.padEnd(20) + " ×" + n;
    }).join("\n") || "  (aún no has abierto ninguna puerta)";
    return {
      titulo: "EL DICCIONARIO HERIDO",
      bloques: [
        b("apertura", "<p>" + procesar(azar(CORPUS.diccionarioMarco)) + "</p>"),
        b("hitos", '<div class="rotulo">PUERTAS ABIERTAS · ' +
          Object.keys(viaje.palabrasHeridas || {}).length + "</div><pre>" + procesarPlano(cuerpo) + "</pre>"),
      ],
      opciones: [
        op("[releer la más herida]", ["escenaOlvidada", "planetaTexto"], { memoria: -4, contaminacion: +4 }),
        op("[cerrar el diccionario]", ["inicio"], { coherencia: +3 }),
        op("[volver al vacío]", poolNavegable(), {}),
      ],
    };
  },
};
...
function aplicarConsecuencias(opcion) {
  ...
  if (opcion.especial === "borrarAlgo") {
    estado.archivo = [];
    if (viaje.recuperadas.length) viaje.recuperadas.shift();
  ...
  if (opcion.especial === "plantar" && viaje.cicatrices.length) {
    // una cicatriz plantada deja de doler: se va del registro
    viaje.cicatrices.shift();
    estado.cicatrices = viaje.cicatrices.slice();
    guardarMemoria();
  }
  ...
  if (opcion.especial === "despegar") {
    // el pacto se cumple: a partir de aquí, el vacío se abre
    estado.despegada = true;
  }
}
...
function determinarSiguienteEscena(opcion) {
  ...
  // la deriva sin destino fijo tira hacia el próximo hito: da rumbo al viaje
  // sin obligar (la mitad de las veces sigue siendo vacío puro).
  const derivar = () => {
    const hito = poolHaciaHito();
    if (hito && hito.length && prob(0.5)) return azar(hito);
    return azar(poolNavegable());
  };
  // la opción puede traicionar: promete un destino y entrega otro.
  // pero el pacto manda: en tierra, la traición no cruza el vacío.
  if (opcion.traicion && prob(opcion.traicion)) return derivar();
  if (opcion.destinos && opcion.destinos.length) return azar(opcion.destinos);
  return derivar();
}

function elegir(opcion) {
  // zarpar en otra semilla: un viaje nuevo, limpio, recordado aparte
  if (opcion.especial === "nuevaSemilla") { nuevaSemilla(); return; }
...
function reiniciarCiclo() {
  registrarBitacora("loop", "ciclo " + pad3(viaje.ciclos) + " cerrado. la nave vuelve con cicatriz.");
  viaje.ciclos += 1;
  viaje.deterioro += 1;
...
function renderEscena(nombre) {
  ...
  desgastePaso();
  propagarContaminacion();   // el daño está vivo: la contaminación se propaga sola
  notasEscena = [];

  const def = generarEscena(nombre);
  const bloques = def.bloques.slice();

  // la ciencia y la historia deben estar en todas partes: casi toda
  // escena (salvo las del sistema) suma un registro científico o narrativo
  const sistemaScn = ["crash", "cajaNegra", "loop", "finalBlanco", "silencio", "finalFalso"];
  if (!sistemaScn.includes(nombre) && !def.menu && prob(0.72)) {
    // en tierra el cielo se lee de cerca (efemérides, clima, carta astral);
    // en vuelo, la ciencia dura y la cosmología. la narrativa y la poesía, siempre.
    const cientificas = estado.despegada
      ? ["cientifico", "telemetria", "espectro", "cartaAstro",
         "exobiologia", "ecologia", "cosmologia", "fisiologia"]
      : ["clima", "efemerides", "cartaAstral", "horoscopo", "checklist",
         "telemetria", "cartaAstro", "cientifico"];
    const narrativas = ["novela", "dialogo", "hallazgo", "tripulacion", "subtrama"];
    const poeticas = ["verso", "haiku", "salmo"];
    // ciencia lejos de la Tierra, narrativa cerca, y una veta poética que las cruza
    const r = rnd();
    const pool = r < 0.18 ? poeticas : (r < 0.62 ? cientificas : narrativas);
    bloques.push(generarTexto(azar(pool)));
  }
...
function renderOpciones(opciones) {
  ...
  // las opciones son el mando: se mantienen legibles casi siempre. sólo con
  // la coherencia muy caída pierden el orden (y, más abajo aún, la forma).
  if (estado.coherencia < 30) orden = barajar(orden);
  const nav = $opciones();
  nav.innerHTML = "";
  orden.forEach((idx, i) => {
    ...
    if (estado.coherencia < 22 && prob(0.15)) etiqueta = glitchVerbal(etiqueta);
...
function actualizarPanel() {
  ...
  let linea = partes.map(([k, v]) => {
    let clave = k;
    if (estado.memoria < 30 && prob(0.25)) clave = desvocalizar(k);
...
function programarCaos() {
  ...
  latidos = [];
  const nivel = viaje.deterioro + estado.contaminacion / 20;
...
function construirMapa() {
  ...
  // pero si aún no despega, la nave sigue en la torre, pegada a la Tierra.
  const tx = 2, ty = H - 3;
  const dist = estado.despegada
    ? clamp(Math.round(4 + estado.pasos * 1.1 + estado.ciclos * 4), 4, W - 5)
    : 1;
  const nx = clamp(tx + dist, 3, W - 3);
  const ny = estado.despegada
    ? clamp(2 + Math.round(estado.deriva / 9) + (estado.pasos % 4) - 1, 1, H - 5)
    : ty - 1;
  ...
  const enc = (viaje.encuentros || []).join(" ");
  ...
  // la sonda, una vez hallada, queda marcada en el vacío: el hito en el mapa
  if (viaje.hitos.sonda) g[Math.round(H * 0.34)][Math.round(W * 0.72)] = "◇";
  if (viaje.crashes > 0) {
    for (let i = 0; i < Math.min(viaje.crashes, 5); i++) {
  ...
  const leyenda =
    " ⊕ tierra   ⌖ la nave   ◇ sonda   × crash   ∿ heliopausa\n" +
    (estado.despegada
      ? " distancia: " + Math.round(dist * 7 + estado.deriva) + " unidades de relato"
      : " la nave sigue en la torre. distancia: 0. la ventana, abierta.") +
    "\n hallazgo: fase " + pad3(viaje.hitos.arco) + "/" + pad3(CORPUS.hallazgo.length - 1) +
    " · " + (viaje.hitos.sonda ? "sonda hallada ◇" :
      (estado.despegada ? "sonda: por hallar, en vuelo" : "sonda: por hallar; hay que despegar")) +
...
function imprimirBitacora() {
  ...
  let html = "<h1>BITÁCORA DE MISIÓN</h1>" +
    ...
    " · " + (viaje.crashes || 0) + " crashes de por vida</p>";
  if (!b.length) {
