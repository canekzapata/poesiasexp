...
/* los hitos: el arco del hallazgo ya no avanza por leer mucho, sino por
   ...
   ciclos: lo que la nave descubrió, lo descubrió de verdad. */
...
function memoriaVacia() {
  return {
    ...
    hitos: hitosVacios(), // el arco del hallazgo, gatillado por hechos reales
  };
}
...
function cargarMemoria() {
  try {
    ...
    // los hitos de una memoria vieja pueden venir incompletos: se normalizan
    m.hitos = Object.assign(hitosVacios(), m.hitos || {});
    return m;
  } catch (e) {
...
function marcarHito(nombre, faseMin, evento) {
  ...
  let cambio = false;
  if (nombre && !memoria.hitos[nombre]) { memoria.hitos[nombre] = true; cambio = true; }
  const objetivo = clamp(faseMin, 0, total - 1);
  if (objetivo > memoria.hitos.arco) { memoria.hitos.arco = objetivo; cambio = true; }
  if (cambio) {
    const etiqueta = CORPUS.hallazgo[memoria.hitos.arco].split("·")[0].trim();
...
function faseDeHallazgo() {
  const total = CORPUS.hallazgo.length;
  const techo = clamp(memoria.hitos.arco, 0, total - 1);
...
const ESCENAS = {
  ...
  inicio() {
    ...
    let cuerpo =
      ...
      pad3(memoria.hitos.arco) + "/" + pad3(CORPUS.hallazgo.length - 1) + "</span><br>" +
  ...
  cajaNegraAjena() {
    ...
    const esSonda = memoria.hitos.sonda;
    let apertura, cola;
    if (esSonda) {
  ...
  sondaEco() {
    ...
    if (estado.despegada && (memoria.hitos.muestra || estado.contaminacion > 60))
  ...
  tomaDeControl() {
    ...
    if (estado.despegada && (memoria.hitos.muestra || estado.contaminacion > 55))
      marcarHito("contacto", ARCO.CONTACTO, "la otra voz tomó el timón");
    if (memoria.hitos.contacto) marcarHito(null, ARCO.FUSION, "estudiar el hallazgo y ser el hallazgo se confunden");
    return {
  ...
  cajaNegra() {
    ...
    if (memoria.hitos.contacto) marcarHito("omega", ARCO.OMEGA, "la caja negra resume el hallazgo");
    return {
      titulo: "CAJA NEGRA",
  ...
  loop() {
    ...
    if (memoria.hitos.contacto) marcarHito("omega", ARCO.OMEGA, "la misión se repite con el plural adentro");
    const bloques = [
      b("apertura", "<pre>" +
  ...
  hallazgoExobiologico() {
    ...
    if (memoria.hitos.muestra) marcarHito(null, ARCO.RESPONDE, "la muestra responde");
    return {
  ...
  muestraEnCuarentena() {
    ...
    if (memoria.hitos.mensaje) marcarHito("muestra", ARCO.MUESTRA, "muestra en cuarentena nivel 4");
    return {
...
function construirMapa() {
  ...
  if (memoria.hitos.sonda) g[Math.round(H * 0.34)][Math.round(W * 0.72)] = "◇";
  ...
  const leyenda =
    ...
    "\n hallazgo: fase " + pad3(memoria.hitos.arco) + "/" + pad3(CORPUS.hallazgo.length - 1) +
    " · " + (memoria.hitos.sonda ? "sonda hallada ◇" :
