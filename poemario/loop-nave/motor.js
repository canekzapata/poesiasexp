...
function puertaAleatoria(palabra) {
  ...
  if (r < 0.38) {
    ...
    spec = { acc: "insertar", arg: azar(["eco", "archivo", "manual", "senal", "poemaConcreto", "error404", "enciclopedia"]) };
  } else if (r < 0.88) {
...
const ESCENAS = {
  ...
  senal() {
    return {
      ...
      bloques: [
        senalRecibida(),
        prob(0.5) ? bitacora() : archivoEncontrado(),
      ],
      menu: true,
      opciones: [
  ...
  /* --- 34 · escenas que la memoria desbloquea ------------------------------------------------------------------------- */
  jardin() {
