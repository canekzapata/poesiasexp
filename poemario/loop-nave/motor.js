...
function puertaAleatoria(palabra) {
  ...
  if (r < 0.38) {
    ...
    spec = { acc: "insertar", arg: azar(["eco", "archivo", "manual", "senal", "poemaConcreto", "error404", "enciclopedia", "letania", "interrogatorio", "xeno", "cientifico", "telemetria", "espectro", "exobiologia", "autopsia", "cartaAstro", "tripulacion", "novela", "dialogo", "hallazgo"]) };
  } else if (r < 0.88) {
...
function renderEscena(nombre) {
  ...
  if (!sistemaScn.includes(nombre) && !def.menu && prob(0.72)) {
    const cientificas = ["cientifico", "telemetria", "espectro", "cartaAstro", "exobiologia"];
    const narrativas = ["novela", "dialogo", "hallazgo", "tripulacion"];
    // el pool se inclina a lo científico lejos de la Tierra, a lo narrativo cerca
    const pool = prob(0.55) ? cientificas : narrativas;
