...
/* el arco de hallazgo: avanza con los pasos y ciclos del lector.
   una misma historia servida por fases, no una escena suelta. */
function faseDeHallazgo() {
  const total = CORPUS.hallazgo.length;
  // la fase depende de cuánto se ha leído en total: la historia progresa
  const avance = memoria.palabrasTocadas + estado.pasos + memoria.ciclos * 4;
  let i = Math.min(total - 1, Math.floor(avance / 6));
  // la memoria baja desordena la historia: el arco también se olvida
  if (estado.memoria < 35 && prob(0.5)) i = azarN(total);
  ...
  return b("hallazgoarco", "<p>" + anotar(procesar(f), 0.3) + "</p>");
}
...
function determinarSiguienteEscena(opcion) {
  ...
  if (opcion.traicion && prob(opcion.traicion)) return azar(poolNavegable());
