...
La obra cuenta, de manera no lineal, la historia de una nave que despierta
**en tierra**, atada a su torre de lanzamiento. El lector puede revisarla
entera sin despegar —los sistemas, el cielo, la carga— pero **despegar hay
que pedirlo**: la nave no se mueve sola. Sólo tras pedir el despegue, confirmar
la cuenta regresiva y encender motores se abre el espacio profundo, donde la
nave se interna, encuentra fenómenos, recibe señales, se deteriora, puede
crashear, puede ser tomada, puede recordar mal y puede volver al inicio con
pérdida o exceso de memoria. El lector decide, pero el azar, el daño y la
memoria de la nave también deciden.
...
**La vuelta v7** ensancha las cuatro dimensiones a la vez: **biológica**
(ecología, evolución, microbioma, fisiología humana en microgravedad — la
vida como sistema, no como espécimen suelto), **astrofísica** (cosmología,
tiempo profundo, dinámica orbital — el marco grande: materia oscura,
entropía, asistencias gravitatorias), **narrativa** (subtramas de la
tripulación que avanzan — la hija de Ihara que escribe a través del retraso
de la señal, la caja sin manifiesto de Tarabá, el durmiente once mil uno) y
**poética** (formas líricas que muerden el dato: verso científico, haiku
orbital, salmo de datos). Un ~13% de las escenas abre ahora una veta
poética; la ciencia y la narrativa siguen presentes en la mayoría.

**El pacto de despegue** (etapa 8): la nave ya no arranca en el vacío. Ahora
despierta **en tierra**, atada a la torre de lanzamiento, y el arco tiene un
umbral real: *puede revisarse todo sin despegar* —los sistemas (encuesta
go/no-go), el cielo (efemérides, clima espacial, carta astral), la carga y la
tripulación— pero **despegar hay que pedirlo**. La nave no despega sola: hay
que decírselo, confirmar la cuenta regresiva y encender motores; sólo entonces
se abre el espacio profundo (`estado.despegada`). En tierra, hasta el azar y la
traición se quedan en la torre (`NAVEGABLES_TIERRA`); el mapa marca *distancia:
0, la ventana abierta*. Esperar es también una forma de misión.

**La capa astrológica** (etapa 8): el mismo cielo se lee dos veces. Junto a la
astrofísica dura corre ahora una veta astronómico-astrológica —el `zodiaco`
(doce signos, cada uno con su astronomía real adentro: las Pléyades de Tauro,
Antares de Escorpio a punto de supernova, Sagitario apuntando al agujero negro
central), los `aspectos`, el `horoscopoNave`, las `efemerides`, el
`climaEspacial`, la `precesión` que corrió los signos de sus constelaciones—.
La astronomía mide dónde está el cuerpo; la astrología, mientras, dice qué
hacer con el miedo de que esté ahí. La nave no cree en los astros: cree en la
costumbre de consultarlos, y archiva las dos lecturas. Además, las secciones
más antiguas del corpus (misiones, bitácoras, sueños, notas, cartas, manuales,
crashes, enciclopedia, interrogatorio, letanía) recibieron injertos de discurso
científico-astrológico, para que la pieza pese menos hacia lo metaliterario y
más hacia el viaje: la ventana de lanzamiento, el max-Q, el perihelio, la
cuenta regresiva, el clima de la estrella de casa.

**La coherencia del arco** (etapa 9): hasta aquí el hallazgo avanzaba por
*leer mucho*; ahora avanza por *hacer*. El arco (`CORPUS.hallazgo`, FASE 0 → Ω)
está gatillado por **hitos** persistentes (`memoria.hitos`): la nave no puede
contar que halló la sonda antes de haberla hallado. Despegar deja de ser
opcional para la trama —*para alcanzar la fuente de la anomalía hay que
despegar: la sonda no viene, se va a buscar*—, y sólo en vuelo (`NAVEGABLES`,
nunca el pool de tierra) se la encuentra. La cadena es una dependencia real:
**triangular** la fuente (desde la torre) → **despegar** → **espectro del
tránsito** (en vuelo) → **hallar la sonda** → **descifrar su mensaje** (no
aterricen) → **tomar la muestra** → **la muestra responde** → **el contacto**
(la otra voz usa los nombres) → **la fusión** → **Ω**. `faseDeHallazgo()` sólo
sirve fases ya *ganadas* (jamás revela una que el lector no se haya ganado con
hechos); `objetivoDeArco()` dice, en la plataforma y en el mapa, qué falta —un
rumbo vivo, no un tablero: no hay puntajes ni logros—; y una **gravedad
narrativa** (`poolHaciaHito()`) inclina la deriva hacia el próximo hito sin
obligar. La misma escena `cajaNegraAjena` significa dos cosas según el viaje:
el **núcleo de la sonda** (con mensaje) si ya se halló, o la **caja sin
manifiesto** de Tarabá (promesa vacía, la regla de Montalbetti) si aún no.

**La biblioteca antigua** (etapa 9): casi todos los estratos del corpus
recibieron injertos de literatura antigua y de corpus de viaje interestelar,
transformados al registro de la nave —nunca citados ni copiados—: la *Eneida*
(el exilio de una costa en llamas hacia un suelo prometido que retrocede, el
piloto Palinuro que se duerme al timón, las dos puertas del sueño, la rama
dorada), la *Odisea* (el regreso, las sirenas que cantan lo que uno más quiere
oír, la caja de los vientos, la tela que se teje de día y se desteje de noche,
el perro que espera en la puerta, «nadie»), la *Ilíada* (el escudo con un
cosmos grabado, el catálogo de las naves que no responden), *Píndaro* (el agua
como lo mejor, las criaturas de un día), las *Églogas* (el niño de la edad de
oro, el pastor despojado), el *Libro de los Muertos* egipcio (el peso del
corazón contra la pluma, la barca solar de las doce horas, la confesión
negativa), y el corpus interestelar —*Dead astronauts*, *Ovni 78*, las
*Ciudades invisibles*— (el astronauta muerto a la deriva, el objeto de las
04:12, cada planeta que es la Tierra vista con otro miedo). El método es el ya
establecido en las bitácoras —ese Li Bai que alza la cabeza y mira la Tierra
brillante—: operar el motivo, no citarlo.

**La legibilidad y el viaje por semilla** (etapa 10): tres cambios al servicio
de que la obra se lea. (1) **La memoria es ahora por semilla**: cada `?seed=`
es un *viaje* propio (`memoria.viajes[semilla]`) con su arco, sus vueltas y su
desgaste; una semilla nueva **despierta la nave entera y legible**, y la misma
semilla conserva su avance al hacer loop. Lo único global es el **museo** de
crashes (`memoria.museo`), la arqueología que cruza todas las lecturas. La
memoria vieja (v<4) **migra**: lo acumulado pasa al museo y el viaje arranca
limpio. (2) **El deterioro del texto está umbralizado**: `intensidadDeterioro()`
devuelve ~0 mientras la nave esté entera (coherencia alta, viaje sin gastar), y
sólo muerde al caer la coherencia; los **caracteres raros** (bloques, ondas,
marcas: la *estática*) aparecen **con propósito, sólo con deterioro alto**
(`glifarPalabra()`), y las opciones-mando se mantienen legibles casi siempre.
La ilegibilidad dejó de ser el estado por defecto: es un estado *ganado* de la
nave. (3) **La plataforma se despejó**: abre con dos verbos —`[despegar]` y
`[analizar la misión — ¿qué es esto?]`—; esta última es una escena
(`analizarMision`) que explica la obra en su propia voz (`CORPUS.sobreLaObra`),
muestra el rumbo del arco y una **bitácora del hallazgo** (los hitos como lista
✓/·: estructura de gamificación sin marcador), y pliega dentro lo que antes
llenaba de botones la apertura (revisar sistemas / cielo / carga, los nodos
ganados, el crash).

**Más gamificación y trama** (etapa 11): cuatro cuartos nuevos, y el daño
cobra vida. (1) La **galería de viajes** (`galeriaDeViajes`) alinea todas las
semillas jugadas con su avance —fase del arco, vueltas, crashes, y una marca
por hito (Ω llegó al final · ◇ halló la sonda · ↑ despegó · · sigue en
tierra)—: cierra la idea de recordar por semilla y deja *zarpar en otra* desde
ahí. (2) La **contaminación se propaga** (`propagarContaminacion`, un árbol de
contagio `viaje.contagio`): cada tantas lecturas —el umbral se acorta cuanto
más contaminada está la nave— una palabra ya secuestrada infecta a una vecina,
y se registra quién infectó a quién. El daño dejó de ser estático: se
reproduce. (3) Tres **rutas secretas por umbral**, que reaparecen en «analizar
la misión» al ganarse: el **astillero de las misiones muertas** (≥6 vueltas del
viaje; ahí se oxidan los objetivos declarados de todos los ciclos), el
**diccionario herido** (≥60 puertas tocadas; todas las palabras que el lector
abrió, ordenadas por dolor, con las más releídas ya desvocalizadas de tanto
abrirlas) y **la nave que escribe esta obra** (≥10 crashes de museo; la cuarta
pared con fisura estructural: la nave abre `motor.js` y se descubre escrita —el
mayor gesto de trama, que ata el «una nave hecha de escritura» del manifiesto
con la sonda-que-es-nosotros—; también se llega siguiendo la grieta desde un
crash). El diccionario usa una huella nueva, `viaje.palabrasHeridas`, que
guarda cada palabra-puerta abierta y su cuenta de dolor.
...
| Eneida / Odisea / Ilíada | el viaje como exilio y regreso: Palinuro, las sirenas, la caja de los vientos, el escudo grabado, el catálogo de las naves que callan — operados, no citados |
| Píndaro / Églogas | el agua como lo mejor, las criaturas de un día; el niño de la edad de oro, el pastor despojado |
| Libro de los Muertos | el peso del corazón contra la pluma, la barca de las doce horas, la confesión negativa |
| Dead astronauts / Ovni 78 / Ciudades invisibles | el astronauta muerto a la deriva, el objeto, cada ciudad que es la misma con otro miedo |
...
1. **azar** — `azar`, `probabilidad`, `prob`, `barajar` (sembrados) y
   ...
   semilla narrativa).
2. **memoria** — `localStorage`, **por semilla**: `memoria.viajes[semilla]`
   guarda el viaje (arco/hitos, ciclos, crashes, deterioro, cicatrices,
   palabras contaminadas, frases recuperadas, encuentros, notas abiertas,
   palabras tocadas); `memoria.museo` es lo único global (la arqueología de
   crashes que cruza semillas). `usarViaje()` selecciona el viaje de la
   semilla activa; una semilla nueva arranca limpia. la migración desde v<4
   vuelca lo viejo al museo.
...
9. **formas de escritura** — bitácora, informe/diagnóstico, poema concreto,
   ...
   **disección exobiológica**, **forense de sonda**, **carta astronómica**,
   las formas narrativas — **tripulación**, **novela de a bordo**,
   **diálogo**, **fase de hallazgo** (el arco que avanza), **subtrama** —,
   las científicas de v7 — **informe ecológico**, **nota cosmológica**,
   **fisiología de la tripulación** —, las **poéticas** — **verso
   científico**, **haiku orbital**, **salmo de datos** — y las de v8, el
   cielo de cerca — **bitácora de plataforma**, **encuesta go/no-go**,
   **boletín de clima espacial**, **carta astral**, **horóscopo de a bordo**,
   **efemérides**, **cuenta regresiva** y **perfil de ascenso** — todas
...
11. **escenas** — más de treinta y cinco nodos, ahora también: documentos
    ...
    **muestra en cuarentena** (bioseguridad y la palabra *vivo*); la
    escena narrativa **guardia de a bordo** (la vida de la tripulación
    entre medición y medición); y las de v8, el **pacto de despegue**: la
    obra abre en **plataforma** (la nave en tierra), con **revisión de
    sistemas**, **lectura del cielo** (efemérides + clima + carta astral),
    **carta astral**, **bodega** (la carga y la caja sin manifiesto),
    **permiso de despegue** (la cuenta regresiva y el go/no-go) y **ascenso**
    (el max-Q y después). El vacío profundo sólo se abre tras pedir despegar.
12. **navegación** — `elegir` → `aplicarConsecuencias` →
    ...
    opción, y sólo al final la promesa del botón. El azar y la traición
    tiran del pool según el pacto (`poolNavegable()`): en tierra,
    `NAVEGABLES_TIERRA` (la torre); en vuelo, `NAVEGABLES` (el vacío).
    Cuando no hay destino fijado, la **gravedad narrativa** (`poolHaciaHito()`)
    inclina la deriva hacia el próximo hito del arco sin obligar: da rumbo al
    viaje para que no se pierda en el vacío.
12b. **el arco gatillado por hechos** — `memoria.hitos` (arco, despego,
    sonda, mensaje, muestra, contacto, omega) persiste entre ciclos.
    `marcarHito(nombre, faseMin, evento)` sube el techo del arco sólo cuando
    el lector cruza un umbral real, y cada escena verifica su precondición
    (la sonda exige `despegada`; el mensaje exige `sonda`; la muestra exige
    `mensaje`…). `faseDeHallazgo()` nunca sirve una fase por encima de ese
    techo: la nave no puede contar lo que no ha vivido. `objetivoDeArco()`
    traduce el estado del arco en un rumbo legible (plataforma y mapa).
