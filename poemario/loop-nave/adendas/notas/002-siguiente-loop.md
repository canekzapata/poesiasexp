...
- **WebAudio sin archivos**: osciladores y ruido filtrado; el ruido de
  fondo firmado por la semilla (misma semilla, mismo ruido).
- **la señal recibida suena**: cada bloque `senal` puede emitir su
  parpadeo como pulsos (morse defectuoso, nunca literal).
- **la caja negra leída en voz alta**: `speechSynthesis`, una voz
  rudimentaria, con las pausas mal puestas a propósito.
- **el earworm del loop**: una canción de cuna de 4 notas que se repite
  como derviche al volver al inicio; con cada ciclo pierde o gana una nota.
- regla: el sonido nunca decora. transmite, falla, insiste. botón de
  silencio discreto (la nave entiende el pudor).
