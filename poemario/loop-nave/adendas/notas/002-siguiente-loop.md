...
> cicatriz de la etapa 11 — más gamificación y trama (a pedido del lector):
> se hicieron cuatro cosas de la lista de pendientes. (1) GALERÍA DE VIAJES
> (`galeriaDeViajes`): lista las semillas jugadas con su avance (fase, vueltas,
> crashes, marca por hito) y deja zarpar en otra (`especial: "nuevaSemilla"`);
> accesible desde «analizar la misión». (2) CONTAMINACIÓN QUE SE PROPAGA
> (`propagarContaminacion`, `viaje.contagio`): cada N lecturas —N se acorta con
> la contaminación— una palabra secuestrada infecta a una vecina; se registra
> el árbol de contagio y sube `estado.contaminacion`. enganchada en
> `renderEscena` tras `desgastePaso`. (3) TRES RUTAS SECRETAS por umbral, que
> aparecen en el hub «analizar la misión»: astillero (`viaje.ciclos>=6`, las
> misiones muertas de todos los ciclos), diccionario herido
> (`viaje.palabrasTocadas>=60`, usa la huella nueva `viaje.palabrasHeridas`
> = {palabra: veces}, las más tocadas salen desvocalizadas) y la cuarta pared
> «la nave que escribe esta obra» (`memoria.museo.crashes>=10`, la nave abre
> motor.js; también se llega siguiendo la grieta desde el crash). corpus nuevo:
> `astillero`, `cuartaPared`, `diccionarioMarco`, y entradas de vocabulario.
> verificado en navegador (23 asserts nuevos + regresiones v9/v10): contagio,
> diccionario por dolor, desbloqueos por umbral, render de los cuatro cuartos,
> galería, y partida de 100 pasos sin errores de JS.
> pendiente aún: audio de fondo sembrado, la caja negra leída en voz alta,
> lector fantasma (instalación desatendida), edición impresa (tecla P: portada
> con la semilla, la ruta como constelación, colofón), la ruta-constelación que
> relee cada escena como la recuerda la nave, propagación del contagio ligada a
> las palabras REALMENTE vecinas del texto (hoy elige de `contaminables`), y
> mostrar el árbol de contagio como figura (hoy vive en la bitácora impresa).
> convendría también completar `fonts/` (faltan los CRT Ac437, las lineales
> A/B, el egipcio, el anatolio y unifont) —hoy degradan a fuentes de sistema.
