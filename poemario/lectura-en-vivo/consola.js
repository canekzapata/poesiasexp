...
const CONSOLA = (() => {
  ...
  async function despertar() {
    ...
    refs.portada.remove();
    /* La consola ya no empieza a correr debajo del prevuelo. Primero la
       ventana completa —escuchar la sala, probar las voces, mirar si la
       máquina aguanta— y la función arranca cuando el poeta lo dice.
       En instalación desatendida o ensayo comprimido se salta sola. */
    const hayPrevuelo = typeof CALIBRACION !== "undefined" &&
      !CALIBRACION.saltaPrevuelo && SONIDO.listo;
    if (hayPrevuelo && CALIBRACION.prevuelo(() => ir(0))) return;
    ir(0);
  ...
  function teclas(e) {
    ...
    /* Q cierra el micro. Con o sin Shift: cuando hay acople no se piensa
       en modificadores. R-27 sigue hablando; sólo se calla la sala. */
    if (K === "q") { aviso(SONIDO.alternarMicro() ? "micro abierto" : "MICRO CERRADO"); return; }
  ...
  function hud() {
    ...
    refs.hudMicro.textContent = ESTADO.audioOK
      ? (!ESTADO.micAbierto ? "micro: CERRADO · Q"
        : (ESTADO.micOK ? "micro: señal" : "micro: sin señal"))
      : "audio: dormido";
    refs.hudMicro.classList.toggle("muerto",
      ESTADO.audioOK && (!ESTADO.micOK || !ESTADO.micAbierto));
    ...
    refs.hudCapas.textContent =
      ...
      /* el HUD nunca miente: si la consola aflojó las visuales para
         salvar el audio, se dice */
      (typeof RENDIMIENTO !== "undefined" && RENDIMIENTO.apretado
