...
const NAVE = (() => {
  ...
  function nueva(cualquierZ) {
    ...
    return {
      x: (r(1) - 0.5) * W * 2,
      y: (r(2) - 0.5) * H * 2,
      z: cualquierZ ? r(3) * 1.4 + 0.1 : 1.5,
      capa: Math.floor(r(4) * 3),
      fase: r(5) * Math.PI * 2,
      tono: r(6),
      px: null,
      py: null,
  ...
  function paso() {
    ...
    for (let i = 0; i < estrellas.length; i++) {
      ...
      const color = colorEstrella(e, brillo);
      const s = e.capa === 2 ? 2 : 1;
      cx.fillStyle = color;
      if (e.px !== null && (vel > 0.75 || modo === "otra")) {
        ...
        cx.strokeStyle = colorEstrella(e, brillo * 0.45);
        cx.lineWidth = s;
        cx.beginPath();
        cx.moveTo(px, py);
        cx.lineTo(px + (e.px - px) * largo, py + (e.py - py) * largo);
        cx.stroke();
      } else {
  ...
  function nivelAudio() {
    if (typeof SONIDO === "undefined" || !SONIDO.listo) {
      return { nivel: 0, grave: 0, agudo: 0, beat: 0 };
    }
    const lee = (fn) => {
      const v = fn();
      return Number.isFinite(v) ? Math.max(0, Math.min(1, v)) : 0;
    };
    const señal = SONIDO.sensores();
    return {
      nivel: lee(() => señal.nivel),
      grave: lee(() => señal.grave),
      agudo: lee(() => señal.agudo),
      beat: lee(() => señal.beat),
    };
  }

  function colorEstrella(e, alpha) {
    let rgb;
    if (modo === "otra" && e.tono > 0.62) rgb = [255, 184, 74];
    else if (modo === "onda") rgb = e.tono < 0.33
      ? [90, 128, 255] : (e.tono < 0.66 ? [100, 255, 160] : [255, 92, 82]);
    else if (modo === "acuse") rgb = [175, 194, 184];
    else rgb = e.tono > 0.82 ? [228, 255, 238] : [140, 255, 170];
    return `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, ${Math.max(0, Math.min(1, alpha * 0.78))})`;
  }
