...
class Text(object):
    ...
    def __init__(self, input_text, state_size=3, chain=None, parsed_sentences=None, retain_original=True):
        """
        input_text: A string.
    ...
    def generate_paragraph(self, num_sentences=5, max_chars=1000, tries=10):
     paragraph = []
     for _ in range(num_sentences):
        sentence = self.make_short_sentence(max_chars, tries)
        if sentence:  # Verifica si la frase no es None antes de agregarla a la lista.
