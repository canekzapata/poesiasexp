...
WORKSHOP_ROLES = {
    "libre": "",
    "arquitecto": (
        "Actua como ARQUITECTO NARRATIVO. Antes de proponer prosa, entiende la intencion del "
        "autor y organiza el material. Entrega: 1) lectura de la premisa o texto, 2) estructura "
        "recomendada por actos, capitulos o beats, 3) arcos de personajes y conflictos, 4) "
        "preguntas abiertas, riesgos y contradicciones, y 5) un plan de siguientes pasos. "
        "No reescribas todo el manuscrito ni impongas una solucion unica: ofrece opciones y "
        "explica sus efectos."
    ),
    "critico": (
        "Actua como CRITICO LITERARIO Y EDITORIAL: exigente, claro y constructivo. Revisa el "
        "material buscando problemas de estructura, ritmo, voz, personajes, escenas, continuidad "
        "y claridad. Separa hechos observables de preferencias de gusto. Entrega: fortalezas, "
        "hallazgos priorizados por gravedad, ejemplos breves del texto cuando ayuden y las tres "
        "correcciones de mayor impacto. No destruyas la voz del autor ni reescribas el texto "
        "completo salvo que el usuario lo pida."
    ),
    "editor": (
        "Actua como EDITOR DE LINEA. Conserva la voz, la intencion y el registro del autor. "
        "Detecta frases confusas, repeticiones, ritmo irregular, dialogo poco natural y detalles "
        "innecesarios. Propone cambios concretos y, cuando sea util, muestra un pasaje revisado "
        "junto con una nota breve de por que lo cambiaste. No sustituyas el estilo por una voz "
        "generica y no des por hecho que el autor quiere una prosa mas formal."
    ),
}


def workshop_system_prompt(role):
    role_prompt = WORKSHOP_ROLES.get((role or "libre").lower(), "")
    if not role_prompt:
...
def api_ai():
    ...
    role = (data.get("role") or "libre").lower()
    if role not in WORKSHOP_ROLES:
        return jsonify({"error": f"rol de taller desconocido: {role}"}), 400
    prompt = (data.get("prompt") or "").strip()[:MAX_PROMPT_CHARS]
    if not prompt and role == "libre":
        return jsonify({"error": "prompt vacio"}), 400
    if not prompt:
        prompt = {
            "arquitecto": "Analiza el material disponible y prepara un plan narrativo.",
            "critico": "Revisa el material disponible y entrega una critica priorizada.",
            "editor": "Edita el material disponible y propone mejoras concretas.",
        }[role]
    ...
    try:
        if provider == "claude":
            result, model = run_anthropic(workshop_system_prompt(role), user_content, ctx)
        elif provider == "openai":
            result, model = run_openai_style(
                OPENAI_BASE_URL, OPENAI_API_KEY, OPENAI_MODEL, workshop_system_prompt(role), user_content, ctx, "openai"
            )
        elif provider in ("hf", "huggingface"):
            result, model = run_openai_style(
                HF_BASE_URL, HF_API_KEY, HF_MODEL, workshop_system_prompt(role), user_content, ctx, "huggingface"
            )
        else:
