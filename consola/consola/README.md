...
- **IA** — multi-proveedor: **Claude (Anthropic)**, **ChatGPT (OpenAI)** y
  **HuggingFace**. Incluye modos de taller **Arquitecto narrativo**, **Crítico editorial**
  y **Editor de línea**. Con "herramientas" activadas, el modelo puede llamar a:
...
- *"lista mis objetos y luego lee bienvenida.md y resumelo"* → usa `listar_objetos`
  y `leer_objeto`.
- *"analiza el capítulo y dame tres rutas de estructura"* → usa **Arquitecto narrativo**.
- *"encuentra problemas de ritmo y continuidad, sin reescribir"* → usa **Crítico editorial**.
- *"edita este párrafo conservando mi voz"* → usa **Editor de línea**.
...
| `/api/ai` | POST | IA con herramientas y modos de taller (`{provider,role,prompt,use_tools,...}`) |
