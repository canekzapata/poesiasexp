...
function dibujarPieza(ctx, p) {
  ...
  var lh = p.tam * 1.06, y0 = -(lineas.length - 1) / 2 * lh;
  for (var i = 0; i < lineas.length; i++) ctx.fillText(lineas[i], 0, y0 + i * lh);
  ctx.restore();
}
...
/* ---- el ciclo del lienzo: imprime -> reposa -> se pierde -> reimprime ---- */
function iniciarLienzo(piezasIniciales) {
  ...
  var W, H, dato, buf, piezas, fase, cont, umbral, fondoRGB, fondoStr;
  ...
  function imprimir() {
    ...
    piezas.forEach(function (p) { dibujarPieza(ctx, p); });
    dato = ctx.getImageData(0, 0, W, H); buf = dato.data;
  }
  function nuevoCiclo() {
    piezas = generarPiezas(false);
    umbral = azar(52, 72); fase = "reposo"; cont = 0; imprimir();
  }

  medir();
  piezas = piezasIniciales; umbral = azar(52, 72); fase = "reposo"; cont = 0; imprimir();
  var reposoFrames = entero(34, 60), perdidaFrames = entero(200, 300);

  function cuadro() {
    if (fase === "reposo") {
      if (++cont > reposoFrames) { fase = "perdida"; cont = 0; }
    } else {
      ...
      fundir(buf, fondoRGB, .010 + cont * 0.00006);
      ctx.putImageData(dato, 0, 0);
      umbral = Math.max(15, umbral - .12);
      if (++cont > perdidaFrames) nuevoCiclo();
    }
