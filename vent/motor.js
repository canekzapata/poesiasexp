...
function descriptorPieza(cxf, cyf, registrar) {
  ...
  return {
    ...
    aparece: azar(0, 1),                               // aparece por partes, no todo de golpe
...
function dibujarPiezaReveal(ctx, p, frame, spread, ox, oy, enHold, SCRAMBLE) {
  var t0 = p.aparece * spread;
  if (frame < t0) return;                                       // aun no aparece (por partes)
  var settled = p.typewriter ? Math.min(p.total, Math.floor((frame - t0) * p.cps)) : p.total;
  var apAlfa = p.typewriter ? 1 : Math.min(1, (frame - t0) / 7);   // los no-typewriter surgen con un pop
  ctx.save();
  ...
  var L = SCRAMBLE.length, bucket = Math.floor(frame / p.cadencia), idx = 0;
...
/* ---- ventisca: los pixeles se desplazan por el campo de viento (advection),
        arrastrados por las rafagas, hasta dispersarse. cada pixel toma el color
        del pixel de donde "sopla" el viento; nada de pixel-sort "batido" ---- */
function advectar(src, dst, W, H, ffx, ffy, gw, GS, amp) {
  for (var y = 0; y < H; y++) {
    var fila = ((y / GS) | 0) * gw, off = y * W;
    for (var x = 0; x < W; x++) {
      var gi = fila + ((x / GS) | 0);
      var sx = x - (ffx[gi] * amp | 0); if (sx < 0) sx = 0; else if (sx >= W) sx = W - 1;
      var sy = y - (ffy[gi] * amp | 0); if (sy < 0) sy = 0; else if (sy >= H) sy = H - 1;
      var d = (off + x) * 4, s = (sy * W + sx) * 4;
...
/* ---- el ciclo del lienzo:
        escritura  — el texto aparece POR PARTES y se escribe a velocidades muy
                     distintas (super rapido..lento); la cola cambia de
                     caracteres visuales a su propia cadencia
        viento     — mece el texto ya puesto y arrecia
        ventisca   — los pixeles se desplazan arrastrados por el viento hasta
                     dispersarse (nada de pixel-sort "batido")
        y reimprime otra tanda ---- */
function iniciarLienzo(piezasIniciales) {
  ...
  var W, H, fondoRGB, fondoStr, piezas, fase, t, E, V, P, spread;
  var srcData, dstData, GS = 8, gw, gh, ffx, ffy;
  ...
  function medir() {
    ...
    gw = Math.ceil(W / GS) + 1; gh = Math.ceil(H / GS) + 1;
    ffx = new Float32Array(gw * gh); ffy = new Float32Array(gw * gh);
  }
  ...
  function dibujarCampo(frame, windAmp, enHold) {
    ctx.globalAlpha = 1; ctx.fillStyle = fondoStr; ctx.fillRect(0, 0, W, H);
    var tt = frame * 0.02;
    piezas.forEach(function (p) {
      var f = flujo(p.x, p.y, tt);
      dibujarPiezaReveal(ctx, p, frame, spread, windAmp * f.x, windAmp * f.y, enHold, SCRAMBLE);
    });
  }
  /* campo de viento en malla gruesa (para no llamar trig por pixel) */
  function campoGrueso(tt) {
    for (var gy = 0; gy < gh; gy++) for (var gx = 0; gx < gw; gx++) {
      var f = flujo(gx * GS, gy * GS, tt), i = gy * gw + gx; ffx[i] = f.x; ffy[i] = f.y;
    }
  }
  function iniciarVentisca() { srcData = ctx.getImageData(0, 0, W, H); dstData = ctx.createImageData(W, H); }
  function duraciones() { spread = entero(90, 170); E = spread + entero(40, 90); V = entero(50, 100); P = entero(150, 230); }
  function nuevoCiclo() { piezas = generarPiezas(false); preparar(piezas); fase = "escritura"; t = 0; duraciones(); }

  medir();
  piezas = piezasIniciales; preparar(piezas);
  fase = "escritura"; t = 0; duraciones();

  function cuadro() {
    if (fase === "escritura") {                    // aparece por partes y se escribe
      dibujarCampo(t, 2 + (t / E) * 3, false);
      if (t > E) { fase = "viento"; t = 0; }
    } else if (fase === "viento") {                // el viento mece y arrecia
      dibujarCampo(E + t, 4 + (t / V) * 9, true);
      if (t > V) { iniciarVentisca(); fase = "ventisca"; t = 0; }
    } else {                                        // los pixeles se van con el viento
      campoGrueso(t * 0.05);
      var amp = 1.2 + (t / P) * 4.5;
      advectar(srcData.data, dstData.data, W, H, ffx, ffy, gw, GS, amp);
      fundir(dstData.data, fondoRGB, .012 + t * 0.00009);
      ctx.putImageData(dstData, 0, 0);
      var tmp = srcData; srcData = dstData; dstData = tmp;   // acumula el arrastre
      if (t > P) nuevoCiclo();
    }
