...
(function (root) {
   ...
   * ------------------------------------------------------------------ */
  var SESGOS = [
    { palabra: /r[ií]o|river/i,  arquitectura: "caverna",     paleta: "plano",     nota: "current" },
    { palabra: /luci[eé]rnaga|firefly/i, arquitectura: null,   paleta: "riso",      parpadeo: 3, nota: "intermittence" },
    { palabra: /virus/i,         arquitectura: "danado",      paleta: null,        nota: "contagion" },
    { palabra: /hueco|hollow/i,  arquitectura: null,          paleta: null,        porosidad: 0.16, nota: "porosity" },
    { palabra: /regresa|return/i,arquitectura: "concentrico", paleta: null,        nota: "turn" },
    { palabra: /noche|night/i,   arquitectura: null,          paleta: "noche",     nota: "darkness" }
  ];
  ...
  */
  ...
  function rasgosDe(mundo) {
    return [
      ["architecture", mundo.lab.arquitectura.nombre + (mundo.lab.baseDanada ? " (" + mundo.lab.baseDanada + ")" : "")],
      ["material", mundo.lab.materialDominante ? mundo.lab.materialDominante.nombre : "—"],
      ["palette", mundo.paleta.nombre],
      ["font", mundo.fuente.clave],
      ["levels", String(mundo.niveles)],
      ["level", String(mundo.nivel)],
      ["grid", mundo.lab.W + "x" + mundo.lab.H],
      ["bodies", mundo.monumentos ? String(mundo.monumentos.lista.length) : "0"],
      ["wardens", mundo.ronda ? (mundo.ronda.vivos().length + " of " + mundo.ronda.lista.length) : "0"],
      ["seed", mundo.semilla]
    ];
  }
  ...
  */
  var CONTROLES = [
    ["arrows / WASD", "walk and turn"],
    ["Q  E", "sidestep"],
    ["Space", "touch the wall ahead"],
    ["do nothing", "the wall becomes a phrase"],
    ["drag a wall", "rub it until it opens"],
    ["Enter", "descend through a threshold"],
    ["", ""],
    ["Tab", "change panel"],
    ...
    ["Alt + arrows", "swap it"],
    ["", ""],
    ["Esc", "pause proliferation"],
    ["R", "seed traits"],
    ["M", "speaker"],
    ["O", "forget"],
    ["P", "print traversal record"],
    ["Shift+S A T J V", "png · ans · txt · json · dump"],
    ["?", "this sheet"]
  ];
  ...
  function hoja(mundo, rej) {
    ...
    */
    ...
    rej.texto(x0 + 2, y0 + 1, "CONTROLS", 15, 0, ATTR.INTENSO, ORIGEN.INTERFAZ, ancho);
  ...
  */
   ...
   * ------------------------------------------------------------------ */
  var RUMBOS = ["east", "southeast", "south", "southwest", "west", "northwest", "north", "northeast"];
  ...
  function frase(mundo) {
    var f = CAM.frente(mundo);
    var oct = ((Math.round(mundo.dirAng / (Math.PI / 4)) % 8) + 8) % 8;
    var partes = ["You face " + RUMBOS[oct] + "."];
    if (f.macizo) partes.push("Ahead, a wall of " + (f.material ? f.material.nombre : "unknown material") + (f.roto ? ", perforated" : "") + ".");
    else partes.push("Ahead, an open passage.");
    if (mundo.lab.esUmbral(mundo.pos.x, mundo.pos.y)) partes.push("You are standing on a threshold.");
    var aqui = monumentoEn(mundo, mundo.pos.x, mundo.pos.y);
    if (aqui) partes.push("You are inside a written body: " + aqui.nombre + ".");
    else {
      var cerca = monumentoEn(mundo, f.x, f.y);
      if (cerca) partes.push("Ahead, a written body " + cerca.alto.toFixed(1) + " walls high.");
    }
    if (mundo.condensacion.estado === "legible" && mundo.condensacion.fraseVisible) {
      partes.push("The wall says: " + mundo.condensacion.fraseVisible + ".");
    } else if (mundo.condensacion.estado === "condensando") {
      partes.push("Something is arranging itself in the wall.");
    }
    if (mundo.ronda) {
      ...
      if (pegado.length) partes.push((pegado.length === 1 ? "One warden is" : pegado.length + " wardens are") + " very close.");
    }
    if (mundo.volcado.activo) partes.push("A region of space is dumping its internal state.");
    partes.push("Level " + mundo.nivel + ", " + mundo.pasos + " steps.");
    return partes.join(" ");
  }
  ...
  function iniciar() {
    ...
    function tecla(ev) {
      ...
      if (k === "Escape") {
        ...
        anotar(mundo, mundo.detenido ? "proliferation paused" : "proliferation resumed");
      } else if (k === "Tab") {
        ...
        root.MT_EXPORTAR.volcadoTxt(mundo.volcado.poema() || "no core dump has occurred in this session", semilla);
      } else if (k === "p" || k === "P") {
        ...
        anotar(mundo, on ? "speaker on" : "speaker off");
      } else if (k === "o" || k === "O") {
        olvidar();
        ...
        else if (bajar(mundo, ahora)) { sonido.umbral(); anotar(mundo, "threshold: level " + mundo.nivel); }
      } else {
        manejada = false;
      }
    ...
    function atrapar(ahora) {
      ...
      anotar(mundo, "you caught a warden: " + r.frase);
      if (r.devueltas) anotar(mundo, "it returned " + r.devueltas + " cells to your memory");
      if (!r.quedan) anotar(mundo, "none remain wandering this level");
      memoria.anotarFrase(r.frase);
      sonido.perforar();
      guardar();
      actualizarEspejo(true);
      mundo.volcado.quizas(ahora, mundo, "warden", 0.12);
    }
    ...
    function pasoAdelante(signo, ahora) {
      ...
      if (caminar(mundo, destino, ahora)) {
        ...
        if (cuerpo && cuerpo !== estado.ultimoCuerpo) {
          ...
          anotar(mundo, "you pass through " + cuerpo.nombre + " (" + cuerpo.especie + ")");
          sonido.muro(mundo.lab.material(mundo.pos.x, mundo.pos.y));
        } else if (!cuerpo) { estado.ultimoCuerpo = null; }
        if (mundo.lab.esUmbral(mundo.pos.x, mundo.pos.y)) anotar(mundo, "threshold underfoot");
        mundo.volcado.quizas(ahora, mundo, "steps", 0.006);
      } else {
        var contra = mundo.lab.material(destino.x, destino.y);
        anotar(mundo, "collision with " + contra.nombre);
        sonido.muro(contra);
      }
      moverse();
    }
    ...
    function gestoTocar(ahora) {
      ...
      if (r.vacio) { anotar(mundo, "nothing ahead"); }
      else if (r.inscripcion) { anotar(mundo, "inscription " + r.texto); sonido.muro(r.frente.material); }
      else if (r.palabra) { anotar(mundo, "the wall releases: " + r.palabra); memoria.anotarFrase(r.palabra); sonido.muro(r.material); }
      else if (r.ilumina) { anotar(mundo, "the " + r.material.nombre + " wall lights up"); sonido.muro(r.material); }
      else if (r.agrieta) { anotar(mundo, "the wall cracks"); sonido.muro(r.material); }
      else { anotar(mundo, "the wall does not respond; it is logged"); }
      mundo.volcado.quizas(ahora, mundo, "touch", 0.02);
      moverse();
    }
    ...
    function cerrarPanel(clave) {
      ...
      if (mundo.disposicion.cerrar(clave)) {
        ...
        guardar();
        anotar(mundo, (def ? def.titulo : clave) + " closed: " + (def ? def.facultad : "a faculty") + " is lost");
        sonido.panel(false);
        pintor.invalidar();
      }
    }
    ...
    function reabrirPanel(clave) {
      if (mundo.disposicion.abrir(clave)) {
        ...
        guardar();
        anotar(mundo, (PAN.DEFINICIONES[clave] ? PAN.DEFINICIONES[clave].titulo : clave) + " returns, but not what happened without it");
        sonido.panel(true);
        pintor.invalidar();
      }
    }
    ...
    function olvidar() {
      ...
      mundo.condensacion.reiniciar();
      anotar(mundo, "memory erased: the world knows nothing again");
      actualizarEspejo(true);
    }
    ...
    function abajo(ev) {
      ...
      if (c.x === rej.cols - 1 && c.y === 1) {
        anotar(mundo, sonido.alternar() ? "speaker on" : "speaker off");
        return;
      }
    ...
    function movido(ev) {
      ...
      if (estado.arrastre.tipo === "redim") {
        ...
        if (r && r.perforado) {
          anotar(mundo, "passage opened at " + r.x + "," + r.y + ": the architect did not make it");
          guardar();
          sonido.perforar();
          mundo.volcado.quizas(ahora, mundo, "perforation", 0.22);
        }
        moverse();
      }
    }
    ...
    function estadoExportable() {
      return {
        obra: "textmode-en", sitio: "poesiasexp",
        semilla: semilla,
    ...
    function bucle(ahora) {
      ...
      if (mundo.volcado.activo && !mundo.volcado.vigente(ahora)) {
        ...
        if (nuevo != null) anotar(mundo, "the region was left as another material");
      }
