...
(function (N) {
  ...
  function degradar() {
    ...
    if (C < 2 || !datos.rutas.length) return;
    const r = N.seed.make(datos.semilla, 'degradacion:' + C);
    // la memoria se degrada más cuando se perdieron bits y fragmentos
    const presion = Math.min(0.34, 0.03 * (C - 1) + 0.012 * datos.bits.perdidos + 0.02 * datos.fragmentos.filter(f => f.estado === 'perdido').length);
    for (let i = 1; i < datos.rutas.length; i++) {
      const reg = datos.rutas[i];
      if (reg.c !== 'verdadero') continue;
      if (r.raw() > presion) continue;
    ...
    // invención: con suficientes cargas la memoria afirma una página no visitada
    if (C >= 4) {
