...
const nodes = Array.from({ length: PAGE_COUNT }, (_, id) => {
  ...
  const graftCount = topology === "empty" ? 1 : topology === "textstorm" || topology === "garden" || topology === "charts" ? int(rng, 3, 5) : int(rng, 1, 3);
  return {
    id: pad(id),
    index: id,
    topology,
...
function buttonField(node, rng) {
  const tail = sample(rng, BUTTON_OPERATIONS.slice(3), BUTTON_OPERATIONS.length - 3);
  const operations = Array.from({ length: node.buttonCount }, (_, i) => i < 3 ? BUTTON_OPERATIONS[i] : tail[(i - 3) % tail.length]);
...
const fragmentCatalog = nodes.map(node => ({
  id: node.id,
  topology: node.topology,
  fontMode: node.fontMode,
  palette: node.palette,
  html: renderPageBody(node).body
}));
...
await Promise.all([
  ...
  write("js/fragmentos-paginas.js", `window.LAB_PAGE_FRAGMENTS=${JSON.stringify(fragmentCatalog).replace(/</g, "\\u003c")};\n`),
