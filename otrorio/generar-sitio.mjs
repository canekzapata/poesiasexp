...
function vanishingMapPage(node, rng) {
  ...
  const nodesHTML = points.map((point, i) => `<article class="diagram-node mortal" data-diagram-node data-mutable tabindex="0" style="--dx:${point.x}%;--dy:${point.y}%;--node-color:${point.color};--node-r:${int(rng, -9, 9)}deg"><span class="diagram-number">013.${String(i).padStart(2, "0")}</span><p>${esc(i % 2 ? point.label : pick(rng, LITERARY))}</p>${i < node.edges.length ? route(rng, node.edges[i], pick(rng, LINKS), "normal") : ""}</article>`).join("");
