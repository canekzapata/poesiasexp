...
const POLYGLOT = Object.freeze({
  englishJourney: [
    "You are not lost; the interface is withholding the scale.",
    "This route remembers the hesitation before the click.",
    "The map calls this progress because it cannot measure wonder.",
    "A window is a promise that borrowed the shape of a border.",
    "The reader moved; the page renamed the movement navigation.",
    "The interface offers a choice, then quietly changes what choice means.",
    "You have crossed three screens and one invisible threshold.",
    "The button performs confidence; its result remains provisional."
  ],
  frenchTongues: [
    "Six signes glissent sous six seuils sans savoir s'ils sortent.",
    "Trois très vieux traits tressent trente trajets trop étroits.",
    "La pluie plie la page; la page parle à la pluie pliée.",
    "Un lien lent lit l'autre lien, l'autre lien lentement le lie.",
    "Quatre cartes craquent quand le clic croque leur cadre."
  ],
  portugueseCurrent: [
    "(RÍO-RUÍDO: o cursor garimpa constelações de código.)",
    "(HYPER-VIAJANTE — link-lâmina, página-pampa, pixel-pirilampo.)",
    "(O mapa? mapa-máquina mastigando margens migratórias.)",
    "(CLIC! a palavra-canoa desce o arquivo amazônico-elétrico.)",
    "(subsubsolo do browser: ouro-erro, eco-ícone, rota-ruína.)"
  ],
  germanSystem: [
    "WEGSPEICHER / der Weg speichert den Schritt.",
    "SCHNITTSTELLENZWEIFEL: die Taste kennt ihre Folge nicht.",
    "PFADVERERBUNGSFEHLER / eine Route erbt den Rand.",
    "KLICKFOLGENARCHIV: der vorige Klick bleibt im nächsten Element.",
    "ZWISCHENRAUMORDNUNG / der Abstand klassifiziert die Stimme.",
    "Das Dokument klassifiziert seinen Leser."
  ],
  ancientGreek: [
    ["ποταμοῖσι τοῖσιν αὐτοῖσιν ἐμβαίνουσιν ἕτερα καὶ ἕτερα ὕδατα ἐπιρρεῖ.", "Para quienes entran en los mismos ríos, fluyen unas y otras aguas.", "Heráclito, DK B12"],
    ["ὁδὸς ἄνω κάτω μία καὶ ὡυτή.", "El camino hacia arriba y hacia abajo es uno y el mismo.", "Heráclito, DK B60"],
    ["φύσις κρύπτεσθαι φιλεῖ.", "La naturaleza ama ocultarse.", "Heráclito, DK B123"],
    ["ἦθος ἀνθρώπῳ δαίμων.", "El carácter es para el ser humano su destino.", "Heráclito, DK B119"]
  ]
});
...
].concat(NAHUATL.fragments, POLYGLOT.englishJourney, POLYGLOT.frenchTongues, POLYGLOT.portugueseCurrent, POLYGLOT.germanSystem);
const MICRO = [
...
].concat(NAHUATL.passages, POLYGLOT.englishJourney, POLYGLOT.portugueseCurrent, POLYGLOT.germanSystem);
const DIAGRAM_TEXT = [
...
].concat(NAHUATL.passages, POLYGLOT.englishJourney, POLYGLOT.frenchTongues, POLYGLOT.portugueseCurrent, POLYGLOT.germanSystem);
...
<head>
  ...
  <script defer src="../js/poliglotas.js"></script>
  <script defer src="../js/juegos.js"></script>
...
function portalDocument() {
  ...
  return `<!doctype html><html lang="es" style="--bg:${palette[0]};--fg:${palette[1]};--a:${palette[2]};--b:${palette[3]};--c:${palette[4]};--d:${palette[5]}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>otra internet dentro del mismo corpus</title><link rel="icon" href="favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="css/base.css"><script>window.LAB_PAGE={worldSeed:${JSON.stringify(WORLD_SEED)},topology:"portal",topologyLabel:"umbral sonoro"};</script><script defer src="js/corpus.js"></script><script defer src="js/acontecimientos.js"></script><script defer src="js/reliquias.js"></script><script defer src="js/tlahtolli.js"></script><script defer src="js/poliglotas.js"></script><script defer src="js/sonido.js"></script></head><body><main class="portal"><div class="portal-grid">${cells}</div><h1 class="portal-title outline">${esc(nodes[0].word)}<br>${esc(nodes[1].word)}</h1><form class="seed-form" id="seed-form"><input name="seed" value="" aria-label="semilla"><button type="submit">otra internet no usada</button><button type="button" id="sound-entry">entrar con sonido</button><a class="route" href="mapa.html?seed=${encodeURIComponent(WORLD_SEED)}">mapa que miente</a><a class="route" href="reliquias.html?seed=${encodeURIComponent(WORLD_SEED)}">amoxtli de reliquias</a></form></main><script>(function(){var params=new URLSearchParams(location.search);function freshSeed(){var entropy="";try{var bytes=new Uint32Array(2);crypto.getRandomValues(bytes);entropy=Array.from(bytes,function(n){return n.toString(36)}).join("")}catch(error){entropy=Math.random().toString(36).slice(2)}return "rio-"+Date.now().toString(36)+"-"+entropy}var initial=params.get("seed")||freshSeed();window.LAB_PAGE.worldSeed=initial;try{if(!params.get("seed")){var current=new URL(location.href);current.searchParams.set("seed",initial);history.replaceState(null,"",current.href)}}catch(error){}var form=document.getElementById("seed-form");form.seed.value=initial;document.querySelectorAll("a[href]").forEach(function(a){try{var url=new URL(a.getAttribute("href"),location.href);url.searchParams.set("seed",initial);a.href=url.href}catch(error){}});form.addEventListener("submit",function(e){e.preventDefault();var s=this.seed.value.trim()||freshSeed();var all=Array.from(document.querySelectorAll(".portal-grid a"));var unique=Array.from(new Map(all.map(function(a){return[a.dataset.target,a]})).values());var memory={};try{memory=JSON.parse(localStorage.getItem("laberinto:"+s)||"{}")}catch(error){}var visited=memory.visited||[];var recent=memory.recent||[];var pool=unique.filter(function(a){return visited.indexOf(a.dataset.target)<0&&recent.indexOf(a.dataset.target)<0});if(!pool.length)pool=unique.filter(function(a){return recent.indexOf(a.dataset.target)<0});if(!pool.length)pool=unique;var link=pool[Math.floor(Math.random()*pool.length)];var destination=new URL(link.getAttribute("href"),location.href);destination.searchParams.set("seed",s);location.href=destination.href})})();</script></body></html>`;
}
...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>amoxtli de reliquias</title><link rel="icon" href="favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="css/base.css"><script>window.LAB_PAGE={worldSeed:${JSON.stringify(WORLD_SEED)},topology:"reliccodex",topologyLabel:"códice de reliquias"};</script><script defer src="js/corpus.js"></script><script defer src="js/acontecimientos.js"></script><script defer src="js/reliquias.js"></script><script defer src="js/tlahtolli.js"></script><script defer src="js/poliglotas.js"></script></head>
<body class="relic-codex-page"><header class="relic-codex-header"><p class="micro" lang="nah">AMOXTLI / ΛΕΊΨΑΝΟΝ / TLAHTOLLI</p><h1>códice de reliquias</h1><p>No son puntos ni aparecen por entrar. Son frases, residuos y pequeñas pruebas que un gesto encontró sin saber que las buscaba. Algunas parejas náhuatl–griego hacen resonar una cámara que el URL, por sí solo, no puede abrir.</p><nav><a class="route" href="index.html?seed=${encodeURIComponent(WORLD_SEED)}">volver al río</a><a class="route" href="mapa.html?seed=${encodeURIComponent(WORLD_SEED)}&amp;view=used">mapa que aún omite</a><a class="route" href="acontecimientos.html?seed=${encodeURIComponent(WORLD_SEED)}">acontecimientos / PDF</a></nav></header><main id="relic-codex" class="relic-codex" aria-live="polite"></main><footer class="relic-sources"><p>Las formas en náhuatl clásico y griego antiguo llevan glosas editoriales propias en español. Las líneas de Homero, Píndaro y Nono indican el archivo exacto del corpus <i>poemario</i>.</p><a href="https://historicas.unam.mx/publicaciones/publicadigital/libros/obras_leon_portilla/339/339_04_08_apendicei.pdf">Cantares mexicanos / textos citados en su original náhuatl · UNAM</a><a href="https://gdn.iib.unam.mx/">Gran Diccionario Náhuatl · UNAM</a><a href="https://github.com/canekzapata/poesiasexp/tree/main/poemario">Corpus griego incorporado desde poemario/</a><a href="https://archive.org/details/fragmentsofworko00hera">Fragmentos de Heráclito / edición griega de Bywater</a></footer><script>addEventListener('DOMContentLoaded',function(){LabRelics.render(document.getElementById('relic-codex'))});</script></body></html>`;
}

function chamberDocument() {
  return `<!doctype html>
<html lang="es" style="--bg:#04080c;--fg:#d8cfb8;--a:#ff4fa3;--b:#63a7ff;--c:#d5ff4f;--d:#ff7b32">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>cámara epigráfica bajo el río</title><link rel="icon" href="favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="css/base.css"><script>window.LAB_PAGE={worldSeed:${JSON.stringify(WORLD_SEED)},topology:"epigraphic",topologyLabel:"cámara epigráfica"};</script><script defer src="js/corpus.js"></script><script defer src="js/acontecimientos.js"></script><script defer src="js/reliquias.js"></script><script defer src="js/camara.js"></script></head>
<body class="epigraphic-chamber"><header class="chamber-header"><p class="micro">TRES RESONANCIAS / NINGUNA CONTRASEÑA</p><h1>cámara<br>epigráfica</h1><p>Las inscripciones no traducen una lengua a la otra: conservan el conflicto. Toca la piedra vacía para producir otra incisión; toca una pareja para desplazarla.</p><nav><a href="reliquias.html?seed=${encodeURIComponent(WORLD_SEED)}">volver al amoxtli</a><a href="mapa.html?seed=${encodeURIComponent(WORLD_SEED)}&amp;view=used">salir por el mapa usado</a><a href="index.html?seed=${encodeURIComponent(WORLD_SEED)}">otro río, misma semilla</a></nav></header><main id="chamber-stone" class="chamber-stone" aria-live="polite"></main></body></html>`;
}
...
const manifest = {
  ...
  relics: "seed-local attention thresholds; impossible before eight meaningful gestures across two pages; three semantic Nahuatl–Greek resonances open an epigraphic chamber",
  entrySeed: "fresh random seed when index has no seed query",
  languageCharacters: { en: "persistent journey and UX/UI memory", fr: "progressively deforms labels", pt: "cannibalizes words from visited pages", de: "classifies the DOM and disputes the UX voice", nah: "text engine and resonance half", grc: "poemario relic stratum and resonance half" },
  languageApparitions: "hover never speaks; French never floats; other apparitions require three pages, twelve gestures, 120-second cooldown and a 1.6 percent deterministic gate",
  localScores: { cellular: "persistence", charts: "legible noise", destruction: "fertile loss", forms: "valid contradictions", diagrams: "improper relations", cannibal: "borrowed organs" },
  nahuatl: { variety: NAHUATL.variety, sources: NAHUATL.sources },
  ancientGreek: { source: "Heraclitus plus poemario/homeriliad.txt, homerody.txt, nonnus.txt and pindar.txt", glosses: "Spanish, editorial", chamber: "semantic resonance with Nahuatl" },
...
await Promise.all([
  ...
  write("camara.html", chamberDocument()),
