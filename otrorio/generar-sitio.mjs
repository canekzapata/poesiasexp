...
const PAGE_COUNT = Number.isFinite(requestedPages)
  ? Math.max(24, Math.min(96, Math.floor(requestedPages)))
  : WORLD_SEED === "otra-agua-2026" ? 68 : 44 + Math.floor(worldRng() * 37);
...
const nodes = Array.from({ length: PAGE_COUNT }, (_, id) => {
  ...
  const rhythm = topology === "destruction" || topology === "timepoem" ? "slow" : topology === "index" ? "click" : topology === "field" || topology === "cellulararchive" ? "scroll" : pick(rng, RHYTHMS);
  const noiseLevel = topology === "garden" ? 2.8 : topology === "textstorm" ? 2.45 : topology === "charts" ? 2.15 : topology === "cellularloom" ? 1.9 : Number((.7 + rng() * 1.25).toFixed(2));
  const cannibalIntensity = topology === "cannibal" ? int(rng, 28, 44) : topology === "empty" ? 0 : topology === "charts" || topology === "cellularloom" ? int(rng, 7, 14) : int(rng, 2, 8);
  return {
    id: pad(id),
    index: id,
    topology,
    ...
    noiseLevel,
    buttonCount: topology === "form" ? int(rng, 10, 16) : 0,
    cannibalIntensity,
    ...
    zapping: topology !== "timepoem" && id > 2 && (id % 11 === 7 || chance(rng, .1)),
...
<body class="${className}" data-topology="${node.topology}" data-font="${node.fontMode}">
  ...
  ${body}
  ${node.topology === "form" ? buttonField(node, rng) : ""}
...
<body class="memory-map-page">
  <header class="memory-map-header">
    ...
    <nav><a class="route" href="index.html?seed=${encodeURIComponent(WORLD_SEED)}">volver al umbral</a><a class="route" href="acontecimientos.html?seed=${encodeURIComponent(WORLD_SEED)}">archivo / PDF</a></nav>
...
<body class="chronicle-page">
  <header class="chronicle-header">
    ...
    <nav><a class="route" href="index.html?seed=${encodeURIComponent(WORLD_SEED)}">volver al río</a><a class="route" href="mapa.html?seed=${encodeURIComponent(WORLD_SEED)}&amp;view=used">volver por el mapa</a></nav>
