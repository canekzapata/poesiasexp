...
const PAGE_COUNT = Number.isFinite(requestedPages)
  ...
  : WORLD_SEED === "otra-agua-2026" ? 52 : 32 + Math.floor(worldRng() * 41);
...
const TOPOLOGIES = [
  ...
  "textstorm", "ascii", "garden", "charts", "cannibal"
];
...
const BUTTON_OPERATIONS = ["plus5", "noise", "chart", "sound", "block", "rain", "cultivate", "scale", "color", "archive"];
const BUTTON_LABELS = {
...
const nodes = Array.from({ length: PAGE_COUNT }, (_, id) => {
  ...
  const cannibalIntensity = topology === "cannibal" ? int(rng, 22, 36) : topology === "empty" ? 0 : int(rng, 1, 6);
  return {
    id: pad(id),
    index: id,
    topology,
