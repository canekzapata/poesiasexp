(function (global) {
  ...
  function readMemory() {
    try {
      return Object.assign({ visited: [], visits: {}, durations: {}, residues: [], windows: 0, births: 0, zaps: 0 }, JSON.parse(localStorage.getItem(memoryKey) || "{}"));
    } catch (error) {
      return { visited: [], visits: {}, durations: {}, residues: [], windows: 0, births: 0, zaps: 0 };
    }
  }
  ...
  function orderedPages(memory, view) {
    if (view !== "used") return manifest.pages.slice();
  ...
  function makeRow(page, memory, position) {
    ...
    species.textContent = page.topologyLabel;
  ...
  function renderList(view) {
    ...
    status.textContent = memory.visited.length + " usadas / " + manifest.pages.length + " existentes / " + Number(memory.zaps || 0) + " zappings";
  }
  ...
  function drawGraph() {
    ...
    points = manifest.pages.map(function (page, index) {
      var angle = index * 2.3999632297;
      var distance = 24 + Math.sqrt((index + 1) / manifest.pages.length) * radius;
    ...
    points.forEach(function (point) {
      ...
      ctx.fillRect(point.x - (active ? 5 : 2), point.y - (active ? 5 : 2), active ? 10 : 4, active ? 10 : 4);
      if (active) {
