(function (global) {
  ...
  function cellularPanels() {
    if (!global.LabAutomata) return;
    var panels = Array.prototype.slice.call(document.querySelectorAll(".cellular-panel"));
    if (!panels.length) return;
    var css = getComputedStyle(document.documentElement);
    var palette = ["--bg", "--fg", "--a", "--b", "--c", "--d"].map(function (name) { return css.getPropertyValue(name).trim(); });
    var modes = ["wolfram", "conway", "cyclic", "typographic"];
    var stops = [];

    function paintPanel(panel, index) {
      var mode = panel.dataset.mode || modes[index % modes.length];
      var stop = global.LabAutomata.paint(panel, {
        mode: mode,
        rule: Number(panel.dataset.rule || config.cellularRule || 90),
        cell: Number(panel.dataset.cell || config.cellSize || 7),
        density: Number(panel.dataset.density || config.cellDensity || .18),
        states: 6,
        palette: palette,
        words: ((C.escrituras || {}).lluvia || ["RÍO", "href", "cae"]),
        rng: R ? R.rngFrom(R.seed + ":panel:" + config.id + ":" + index + ":" + mode) : rng,
        animated: !matchMedia("(prefers-reduced-motion: reduce)").matches && index % 3 === 0
      });
      stops[index] = stop;
    }

    panels.forEach(function (panel, index) {
      paintPanel(panel, index);
      panel.addEventListener("click", function () {
        if (stops[index]) stops[index]();
        var current = modes.indexOf(panel.dataset.mode || "wolfram");
        panel.dataset.mode = modes[(current + 1 + index) % modes.length];
        paintPanel(panel, index);
        record("automata", "el panel " + index + " cambió a " + panel.dataset.mode);
      });
    });
    global.__stopAutomata = function () { stops.forEach(function (stop) { if (stop) stop(); }); };
    record("automata", panels.length + " autómatas comparten la página", "paneles-automata:" + config.id);
  }

  function timepoem() {
    var world = document.querySelector(".timepoem-world");
    if (!world) return;
    cellularPanels();
    var lines = Array.prototype.slice.call(world.querySelectorAll("[data-time-line]"));
    var countNode = world.querySelector("[data-poem-count]");
    var secondsNode = world.querySelector("[data-poem-seconds]");
    var interval = Math.max(1800, Number(config.poemInterval || 5600));
    var storageKey = "otrorio.poema." + (R ? R.seed : "otra-agua") + "." + config.id;
    var state = { revealed: 0, seconds: 0, remainder: 0 };
    try { state = Object.assign(state, JSON.parse(localStorage.getItem(storageKey) || "{}")); } catch (error) { /* empezar de nuevo también es tiempo */ }

    function save() {
      try { localStorage.setItem(storageKey, JSON.stringify(state)); } catch (error) { /* el poema puede olvidar */ }
    }

    function reveal(index, returning) {
      var line = lines[index];
      if (!line) return false;
      line.textContent = String(index + 1).padStart(2, "0") + "  " + line.dataset.line;
      line.classList.add("is-readable");
      if (!returning) record("poema", "la demora reveló la línea " + (index + 1), "poema:" + config.id + ":" + index);
      return true;
    }

    for (var i = 0; i < Math.min(lines.length, Number(state.revealed || 0)); i += 1) reveal(i, true);
    if (countNode) countNode.textContent = String(state.revealed || 0);
    if (secondsNode) secondsNode.textContent = String(state.seconds || 0);
    var last = Date.now();
    var timer = global.setInterval(function () {
      var now = Date.now();
      var elapsed = now - last;
      last = now;
      if (document.hidden || state.revealed >= lines.length) return;
      state.seconds += elapsed / 1000;
      state.remainder += elapsed;
      if (state.remainder >= interval) {
        state.remainder -= interval;
        if (reveal(state.revealed, false)) state.revealed += 1;
      }
      if (countNode) countNode.textContent = String(state.revealed);
      if (secondsNode) secondsNode.textContent = String(Math.floor(state.seconds));
      save();
      if (state.revealed >= lines.length) {
        global.clearInterval(timer);
        record("poema", "el poema terminó de aparecer", "poema-completo:" + config.id);
      }
    }, 1000);
    global.addEventListener("pagehide", function () { global.clearInterval(timer); save(); }, { once: true });
  }
  ...
  function destruction() {
    ...
    function residueFor(node, i, remembered) {
      if (!node || !node.isConnected) return;
      var residue = document.createElement("span");
      residue.className = "residue persistent-scar" + (remembered ? " is-remembered" : "");
      residue.dataset.absent = node.dataset.mortalId || i;
      residue.textContent = "<" + node.tagName.toLowerCase() + " data-ausente=\"" + (node.dataset.mortalId || i) + "\">";
      node.replaceWith(residue);
    }
    mortals.forEach(function (node, i) {
      if (R && R.isScarred && R.isScarred(node.dataset.mortalId)) { residueFor(node, i, true); return; }
      node.addEventListener("click", function (event) {
        if (event.target.closest && event.target.closest("a")) return;
        event.preventDefault(); event.stopPropagation();
        if (R && R.rememberScar) R.rememberScar(node.dataset.mortalId);
        record("destruccion", "la lectura dejó una cicatriz persistente " + (node.dataset.mortalId || i));
        if (global.LabRelics) global.LabRelics.discover("ceniza", "cicatriz " + (node.dataset.mortalId || i), true);
        node.classList.add("dying");
        global.setTimeout(function () { residueFor(node, i, false); }, 520);
      });
  ...
  function init() {
    ...
    var handlers = {
      ...
      cellularloom: cellularPanels, cellulararchive: cellularPanels,
      timepoem: timepoem,
      diagram: function () { if (global.LabDiagrams) global.LabDiagrams.init(); },
      diagramtext: function () { if (global.LabDiagrams) global.LabDiagrams.init(); },
      oracleparty: function () { form(); if (global.LabDiagrams) global.LabDiagrams.init(); },
      vanishingmap: function () { destruction(); if (global.LabDiagrams) global.LabDiagrams.init(); }
    };
