(function (global) {
  ...
  var lastScroll = 0;
  var timer = null;
  var budget = Math.min(28, Math.max(3, Math.ceil(Number(config.cannibalIntensity || 2) / 2)));
  ...
  function targets() {
    return Array.prototype.slice.call(document.querySelectorAll("body *")).filter(function (node) {
      if (!node.parentNode || node.closest(".permanent-routes, .lab-meta")) return false;
      if (node.matches("script, style, link, meta, source, option, [data-indestructible]")) return false;
      return true;
    });
  }
  ...
  function digestCanvas(target, donor, rng) {
    var mode = pick(rng, donor.charts && donor.charts.length ? donor.charts : (global.LabCharts && global.LabCharts.modes), "glitch");
    if (target.matches(".cellular-canvas, .cellular-panel") && typeof global.__stopAutomata === "function") global.__stopAutomata();
    target.classList.add("cannibalized-canvas");
    target.dataset.cannibalCanvas = donor.id;
    target.dataset.chartMode = mode;
    if (global.LabCharts) global.LabCharts.draw(target, mode, signature());
    var container = target.closest("figure, section, main") || target;
    chromatize(container, donor.palette, serial);
    return target;
  }

  function digestFrame(target, donor, rng) {
    var hrefs = (donor.hrefs || []).filter(function (href) { return /\.html(?:[?#]|$)/.test(href); });
    var href = pick(rng, hrefs, "./" + donor.id + ".html");
    if (global.LabRoutes) href = global.LabRoutes.carrySeed(href, { cannibal: donor.id });
    target.removeAttribute("data-src");
    target.src = href;
    target.dataset.cannibalFrame = donor.id;
    target.title = "marco digerido de " + donor.id;
    chromatize(target.parentElement || target, donor.palette, serial);
    return target;
  }

  function digestLink(target, donor, rng) {
    var hrefs = (donor.hrefs || []).filter(function (href) { return href && href.charAt(0) !== "#"; });
    var href = pick(rng, hrefs, "./" + donor.id + ".html");
    target.href = global.LabRoutes ? global.LabRoutes.carrySeed(href, { cannibal: donor.id }) : href;
    target.textContent = decode(pick(rng, donor.texts, donor.id)).slice(0, 90);
    target.dataset.cannibalLink = donor.id;
    return copySkin(target, donor, rng);
  }

  function digestControl(target, donor, rng) {
    var text = decode(pick(rng, donor.texts, donor.id)).slice(0, 70);
    if (target.matches("button")) target.textContent = text;
    else if (target.matches("input")) { target.value = text; target.placeholder = donor.id + " / valor digerido"; }
    else if (target.matches("textarea")) target.value = text + "\n" + decode(pick(rng, donor.code, ""));
    else if (target.matches("select")) target.title = text;
    target.dataset.cannibalControl = donor.id;
    return copySkin(target, donor, rng);
  }

  function wrapTarget(target, donor, rng) {
    if (target.matches("tr, td, tbody, table, form, fieldset, legend") || target.closest("table")) return copySkin(target, donor, rng);
    var wrapper = document.createElement(pick(rng, ["section", "aside", "blockquote", "div"], "div"));
    target.replaceWith(wrapper);
    wrapper.appendChild(target);
    wrapper.dataset.cannibalWrapper = donor.id;
    chromatize(wrapper, donor.palette, serial);
    return wrapper;
  }
  ...
  function bite(reason) {
    ...
    var operations = ["skin", "tag", "digest", "split", "chart", "text", "wrap"];
    var operation;
    if (target.matches("canvas")) operation = "canvas";
    else if (target.matches("iframe")) operation = "frame";
    else if (target.matches("a[href]")) operation = "link";
    else if (target.matches("button, input, textarea, select")) operation = "control";
    else operation = operations[(hash32(donor.id + config.id + serial) + Math.floor(rng() * operations.length)) % operations.length];
    serial += 1;
    var result;
    if (operation === "canvas") result = digestCanvas(target, donor, rng);
    else if (operation === "frame") result = digestFrame(target, donor, rng);
    else if (operation === "link") result = digestLink(target, donor, rng);
    else if (operation === "control") result = digestControl(target, donor, rng);
    else if (operation === "skin") result = copySkin(target, donor, rng);
    ...
    else if (operation === "wrap") result = wrapTarget(target, donor, rng);
    else {
  ...
  function init() {
    ...
    else if (config.topology !== "empty" && rngFrom(signature() + ":initial")() < .72) bite("carga inicial");
